...
const DOMAIN_NAME = process.env.MANAGED_DOMAIN || 'easysecurity.in'
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'start') {
      ...
      // Auto-add TXT record to Vercel DNS
    ...
    if (action === 'finalize') {
      ...
      // Save to certificates table for dashboard
      await sb.from('certificates').upsert({
        user_id: row.user_id, session_id: sessionId,
        domain: row.domain, cert_pem: certPem,
      ...
      return res.json({
        ...
        fullchain: certPem, privateKey: privateKeyPem,
        domain: row.domain, expiresAt,
