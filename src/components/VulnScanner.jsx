...
function ProtoRow({ proto }) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:8, padding:'7px 0',
      ...
      {icon}
      <span style={{ fontSize:13, flex:1, color:'#0f172a' }}>{proto.name}</span>
      <span style={{ fontSize:11, color: proto.status==='good'?'#16a34a':proto.status==='bad'?'#dc2626':'#d97706' }}>
        {proto.note}
      </span>
    </div>
  )
...
function VulnRow({ v }) {
  const icon = v.status === 'safe'
    ...
    : v.status === 'vulnerable'
    ? <XCircle      size={13} color="#dc2626" style={{ flexShrink:0 }}/>
    : <Info         size={13} color="#0e7fc0" style={{ flexShrink:0 }}/>
  const label = v.status === 'safe' ? { text:'Not vulnerable', color:'#16a34a' }
    : v.status === 'vulnerable' ? { text:'Vulnerable', color:'#dc2626' }
    : { text:'Manual check', color:'#0e7fc0' }
  return (
    <div style={{ display:'flex', alignItems:'center', gap:8, padding:'7px 0',
      borderBottom:'0.5px solid rgba(15,23,42,0.06)' }}>
      {icon}
      <span style={{ fontSize:13, flex:1, color:'#0f172a' }}>{v.name}</span>
  ...
  return (
    <div style={{ marginTop:12 }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between',
        padding:'10px 14px', background:'#fafaf9', border:'0.5px solid rgba(15,23,42,0.08)',
        borderRadius: result ? '8px 8px 0 0' : 8 }}>
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          <Bug size={14} color="#64748b"/>
          <span style={{ fontSize:13, fontWeight:500, color:'#374151' }}>SSL Vulnerability Scan</span>
      ...
      {error && (
        <div style={{ padding:'8px 14px', background:'#fef2f2', border:'0.5px solid #fecaca',
          borderTop:'none', borderRadius:'0 0 8px 8px', fontSize:12, color:'#dc2626' }}>
          {error}
        </div>
      )}
      ...
      {result && expanded && (
        <div style={{ border:'0.5px solid rgba(15,23,42,0.08)', borderTop:'none',
          ...
          <CipherBar label="Strong" count={result.ciphers.strong} total={totalCiphers} color="#10b981"/>
          <CipherBar label="Medium" count={result.ciphers.medium} total={totalCiphers} color="#f59e0b"/>
