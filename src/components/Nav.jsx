      ...
      {sm && mob && (
        <div style={{borderTop:`1px solid ${LN}`,background:W}}>
          <div style={{padding:'10px 14px 16px',display:'flex',flexDirection:'column',gap:1}}>
            ...
            <button onClick={()=>{nav('/auth');setMob(false)}} style={{display:'flex',alignItems:'center',justifyContent:'center',background:BK,color:W,border:'none',padding:'10px 12px',borderRadius:3,fontSize:13,fontWeight:600,cursor:'pointer',fontFamily:F}}>Get started</button>
          </div>
        </div>
      )}
    </nav>
