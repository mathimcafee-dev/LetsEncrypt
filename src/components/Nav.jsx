...
const BG='#0a0a0a', BG3='#f3f3f3', LINE='rgba(0,0,0,0.06)', LINE2='rgba(0,0,0,0.12)'
const T1   = '#0a0a0a', T2='rgba(0,0,0,0.52)', T3='rgba(0,0,0,0.26)'
...
export default function Nav({ nav, page }) {
  ...
  return (
    <nav style={{position:'sticky',top:0,zIndex:200,background:'rgba(255,255,255,0.97)',backdropFilter:'blur(10px)',WebkitBackdropFilter:'blur(10px)',borderBottom:`1px solid ${LINE}`,fontFamily:F}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',height:52,padding:`0 clamp(16px,4vw,40px)`}}>
        <div onClick={()=>nav('/')} style={{display:'flex',alignItems:'center',gap:7,cursor:'pointer',flexShrink:0,userSelect:'none'}}>
          <div style={{width:22,height:22,background:'#0a0a0a',borderRadius:4,display:'flex',alignItems:'center',justifyContent:'center'}}>
        ...
        {!isMobile&&<div style={{display:'flex',alignItems:'center',gap:1}}>
          ...
          <div ref={intelRef} style={{position:'relative'}}>
            ...
            {intelOpen&&(
              <div style={{position:'absolute',top:'calc(100% + 6px)',left:'50%',transform:'translateX(-50%)',background:'#ffffff',border:'1px solid rgba(0,0,0,0.12)'`1px solid ${LINE2}`,borderRadius:4,padding:'5px',minWidth:195,zIndex:300,boxShadow:'0 8px 32px rgba(0,0,0,0.6)'}}>
                {intel.map(item=>(
        ...
        {isMobile&&<button onClick={()=>setMobileOpen(o=>!o)} style={{background:'rgba(0,0,0,0.05)',border:`1px solid ${LINE}`,borderRadius:3,width:32,height:32,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',color:T2,padding:0}}>
          {mobileOpen?<X size={15}/>:<Menu size={15}/>}
        </button>}
      </div>

      {isMobile&&mobileOpen&&(
        <div style={{borderTop:`1px solid ${LINE}`,background:'#ffffff'}}>
          <div style={{padding:'10px 14px 16px',display:'flex',flexDirection:'column',gap:1}}>
            ...
            <button onClick={()=>{nav('/auth');setMobileOpen(false)}} style={{display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(0,0,0,0.05)',color:T2,border:`1px solid ${LINE}`,padding:'10px 12px',borderRadius:3,fontSize:13,fontWeight:500,cursor:'pointer',fontFamily:F,marginBottom:6}}>Sign in</button>
