...
function Pill({ state }) {
  const map = { pending:{bg:'rgba(192,57,43,0.1)',color:'#9a918a',label:'Pending'}, running:{bg:'rgba(192,57,43,0.1)',color:'#c0392b',label:'In progress...'}, done:{bg:'rgba(192,57,43,0.1)',color:'#5edb8a',label:'Done'}, error:{bg:'rgba(192,57,43,0.12)',color:'#c0392b',label:'Failed'}, skipped:{bg:'rgba(192,57,43,0.1)',color:'#b5aea8',label:'Skipped'} }
...
function StepRow({step,title,subtitle,state}) {
  const icons = {
    pending:<span style={{width:22,height:22,borderRadius:'50%',background:'rgba(192,57,43,0.15)',display:'inline-flex',alignItems:'center',justifyContent:'center',fontSize:11,color:'#b5aea8',fontWeight:700}}>{step}</span>,
    ...
    skipped:<span style={{width:22,height:22,borderRadius:'50%',background:'rgba(192,57,43,0.15)',display:'inline-flex',alignItems:'center',justifyContent:'center',fontSize:11,color:'#b5aea8'}}>.</span>,
  }
  return (
    <div style={{display:'flex',gap:14,padding:'14px 0',borderBottom:'1px solid #f1f5f9'}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        {subtitle && <div style={{fontSize:11,color:'#b5aea8',marginTop:2}}>{subtitle}</div>}
      </div>
    </div>
  )
}
...
function Field({label,hint,required,children}) {
  return (
    <div style={{marginBottom:14}}>
      <label style={{display:'block',fontSize:11,fontWeight:600,color:'#d4cdc6',marginBottom:5,textTransform:'uppercase',letterSpacing:'0.4px'}}>
        {label}{required && <span style={{color:'#c0392b',marginLeft:3}}>*</span>}
      </label>
      {children}
      {hint && <div style={{fontSize:11,color:'#9a918a',marginTop:4}}>{hint}</div>}
    </div>
  )
}
...
function ServerTypeCard({icon,title,description,selected,onClick}) {
  return (
    <button onClick={onClick} style={{flex:1,textAlign:'left',padding:'12px 14px',borderRadius:8,cursor:'pointer',fontFamily:'inherit',border:selected?'2px solid #2563eb':'1.5px solid #e2e8f0',background:selected?'rgba(192,57,43,0.1)':'white',transition:'all 0.15s'}}>
      ...
      <div style={{fontSize:11,color:'#9a918a',lineHeight:1.5}}>{description}</div>
    </button>
  )
}
      ...
      <div style={{background:'white',borderRadius:14,width:'100%',maxWidth:520,maxHeight:'90vh',overflowY:'auto',boxShadow:'0 24px 64px rgba(0,0,0,0.18)'}}>
        <div style={{padding:'20px 24px 16px',borderBottom:'1px solid #f1f5f9',display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,background:'white',zIndex:10,borderRadius:'14px 14px 0 0'}}>
          <div>
            ...
            <div style={{fontSize:11,color:'#b5aea8',marginTop:2,fontFamily:'monospace'}}>{cert.domain}</div>
          </div>
          <button onClick={onClose} style={{background:'none',border:'none',cursor:'pointer',color:'#b5aea8',padding:4}}><X size={16}/></button>
        </div>
        <div style={{padding:'20px 24px 24px'}}>
          <div style={{background:'transparent',border:'1px solid #e2e8f0',borderRadius:8,padding:'10px 14px',marginBottom:20,display:'flex',alignItems:'center',gap:12}}>
            ...
            <div><div style={{fontSize:12,fontWeight:600,color:'transparent'}}>Active Paid SSL Certificate</div><div style={{fontSize:11,color:'#9a918a'}}>Fullchain ready | Private key in CertVault | Expires {expStr}</div></div>
          </div>

          {phase==='select_type' && (<>
            <div style={{fontSize:11,fontWeight:700,color:'#d4cdc6',textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:12}}>Select Server Type</div>
            ...
            {serverType==='vps' && <div style={{background:'rgba(192,57,43,0.1)',border:'1px solid #F2C4BC',borderRadius:8,padding:'12px 14px',marginBottom:16,display:'flex',gap:8,alignItems:'flex-start'}}>
              ...
              <div><div style={{fontSize:12,fontWeight:600,color:'#ff8c7a'}}>SSLVault Agent Required</div><div style={{fontSize:11,color:'#ff8c7a',marginTop:2,lineHeight:1.5}}>Go to DNS &amp; Servers &#8210; Servers tab &#8210; Install Agent. Once running it automatically polls and installs certificates.</div></div>
            </div>}
          ...
          {phase==='configure' && serverType==='cpanel' && (<>
            <div style={{fontSize:11,fontWeight:700,color:'#d4cdc6',textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:14}}>Configure cPanel Server</div>
            {savedServers.length>0 && <div style={{marginBottom:14}}>
              <label style={{display:'block',fontSize:11,fontWeight:600,color:'#d4cdc6',marginBottom:5,textTransform:'uppercase',letterSpacing:'0.4px'}}>Saved Servers</label>
            ...
            {(useNew||savedServers.length===0) && <div style={{background:'transparent',border:'1px solid #e2e8f0',borderRadius:8,padding:'16px',marginTop:4}}>
              <div style={{fontSize:11,fontWeight:700,color:'#d4cdc6',marginBottom:14,textTransform:'uppercase',letterSpacing:'0.4px'}}>New cPanel Server</div>
              ...
              <Field label='cPanel API Token' required hint='Create in cPanel Security > Manage API Tokens. Never expires.'>
                <div style={{position:'relative'}}>
                  ...
                  <button onClick={()=>setTokenVis(v=>!v)} style={{position:'absolute',right:10,top:'50%',transform:'translateY(-50%)',background:'none',border:'none',cursor:'pointer',color:'#9a918a',padding:0}}>{tokenVis?<EyeOff size={14}/>:<Eye size={14}/>}</button>
                </div>
              </Field>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:10,marginTop:4}}>
                <label style={{display:'flex',alignItems:'flex-start',gap:8,cursor:'pointer'}}><input type='checkbox' checked={saveServer} onChange={e=>setSaveServer(e.target.checked)} style={{marginTop:2}}/><span style={{fontSize:12,color:'#d4cdc6',lineHeight:1.5}}>Save this server for future one-click installs</span></label>
                <label style={{display:'flex',alignItems:'flex-start',gap:8,cursor:'pointer'}}><input type='checkbox' checked={autoInstallFuture} onChange={e=>setAutoInstallFuture(e.target.checked)} style={{marginTop:2}}/><span style={{fontSize:12,color:'#d4cdc6',lineHeight:1.5}}>Auto-install when cert is reissued or renewed</span></label>
              </div>
            </div>}
            <div style={{marginTop:16,marginBottom:20,background:'rgba(192,57,43,0.1)',border:'1px solid rgba(192,57,43,0.3)',borderRadius:8,padding:'12px 14px'}}>
              <div style={{fontSize:11,fontWeight:700,color:'#5edb8a',marginBottom:8}}>What happens during installation</div>
              {['SSLVault connects to your cPanel server over HTTPS (no SSH needed)','Uploads the 3-block fullchain PEM - end-entity + intermediate + root CA','Calls cPanel rebuild_mail_sni to activate SSL across all services','Enables automatic HTTP to HTTPS redirect','Verifies the live domain is serving your new certificate'].map((t,i)=>(<div key={i} style={{display:'flex',gap:6,marginBottom:4,fontSize:11,color:'#5edb8a'}}><span style={{color:'#5edb8a',flexShrink:0}}>&#10004;</span>{t}</div>))}
            </div>
            <div style={{display:'flex',gap:8}}>
              <button onClick={()=>setPhase('select_type')} style={{padding:'10px 16px',borderRadius:8,border:'1px solid #e2e8f0',background:'white',cursor:'pointer',fontFamily:'inherit',fontSize:13,color:'#d4cdc6'}}>Back</button>
          ...
          {(phase==='running'||phase==='done'||phase==='error') && (<>
            <div style={{fontSize:11,fontWeight:700,color:'#d4cdc6',textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:4}}>Installation Progress</div>
            <div>
            ...
            {phase==='done' && <div style={{marginTop:16,background:'rgba(192,57,43,0.1)',border:'1px solid rgba(192,57,43,0.3)',borderRadius:8,padding:'14px'}}>
              <div style={{display:'flex',gap:8,alignItems:'center',marginBottom:10}}><CheckCircle size={16} color='#16a34a'/><span style={{fontSize:13,fontWeight:700,color:'#5edb8a'}}>Certificate installed successfully</span></div>
              <div style={{display:'grid',gridTemplateColumns:'auto 1fr',gap:'4px 12px',fontSize:11,color:'#5edb8a',lineHeight:1.8,marginBottom:8}}>
                ...
                <span style={{fontWeight:600}}>Status</span><span style={{color:'#5edb8a',fontWeight:600}}>✓ Active on cPanel</span>
              </div>
              {autoInstallFuture&&<div style={{fontSize:10,color:'#5edb8a',opacity:0.8}}>Future reissues and renewals will be installed automatically.</div>}
            </div>}
            <div style={{display:'flex',gap:8,marginTop:16}}>
              {phase==='error' && <button onClick={()=>{setPhase('configure');setBusy(false)}} style={{flex:1,padding:'10px',border:'1px solid #e2e8f0',borderRadius:8,background:'white',cursor:'pointer',fontFamily:'inherit',fontSize:13,color:'#d4cdc6'}}>Try again</button>}
