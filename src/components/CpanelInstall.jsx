...
function Pill({ state }) {
  const map = { pending:{bg:'rgba(192,57,43,0.1)',color:'#b0a8a0',label:'Pending'}, running:{bg:'rgba(192,57,43,0.1)',color:'#c0392b',label:'In progress...'}, done:{bg:'rgba(192,57,43,0.1)',color:'#5edb8a',label:'Done'}, error:{bg:'rgba(192,57,43,0.12)',color:'#c0392b',label:'Failed'}, skipped:{bg:'rgba(192,57,43,0.1)',color:'#b5aea8',label:'Skipped'} }
...
function Field({label,hint,required,children}) {
  return (
    <div style={{marginBottom:14}}>
      <label style={{display:'block',fontSize:11,fontWeight:600,color:'#e8e0d8',marginBottom:5,textTransform:'uppercase',letterSpacing:'0.4px'}}>
        {label}{required && <span style={{color:'#c0392b',marginLeft:3}}>*</span>}
      </label>
      {children}
      {hint && <div style={{fontSize:11,color:'#b0a8a0',marginTop:4}}>{hint}</div>}
    </div>
  )
}
...
function ServerTypeCard({icon,title,description,selected,onClick}) {
  return (
    <button onClick={onClick} style={{flex:1,textAlign:'left',padding:'12px 14px',borderRadius:8,cursor:'pointer',fontFamily:'inherit',border:selected?'2px solid #2563eb':'1.5px solid #e2e8f0',background:selected?'rgba(192,57,43,0.1)':'white',transition:'all 0.15s'}}>
      ...
      <div style={{fontSize:11,color:'#b0a8a0',lineHeight:1.5}}>{description}</div>
    </button>
  )
}
      ...
      <div style={{background:'white',borderRadius:14,width:'100%',maxWidth:520,maxHeight:'90vh',overflowY:'auto',boxShadow:'0 24px 64px rgba(0,0,0,0.18)'}}>
        ...
        <div style={{padding:'20px 24px 24px'}}>
          <div style={{background:'transparent',border:'1px solid #e2e8f0',borderRadius:8,padding:'10px 14px',marginBottom:20,display:'flex',alignItems:'center',gap:12}}>
            ...
            <div><div style={{fontSize:12,fontWeight:600,color:'transparent'}}>Active Paid SSL Certificate</div><div style={{fontSize:11,color:'#b0a8a0'}}>Fullchain ready | Private key in CertVault | Expires {expStr}</div></div>
          </div>

          {phase==='select_type' && (<>
            <div style={{fontSize:11,fontWeight:700,color:'#e8e0d8',textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:12}}>Select Server Type</div>
          ...
          {phase==='configure' && serverType==='cpanel' && (<>
            <div style={{fontSize:11,fontWeight:700,color:'#e8e0d8',textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:14}}>Configure cPanel Server</div>
            {savedServers.length>0 && <div style={{marginBottom:14}}>
              <label style={{display:'block',fontSize:11,fontWeight:600,color:'#e8e0d8',marginBottom:5,textTransform:'uppercase',letterSpacing:'0.4px'}}>Saved Servers</label>
            ...
            {(useNew||savedServers.length===0) && <div style={{background:'transparent',border:'1px solid #e2e8f0',borderRadius:8,padding:'16px',marginTop:4}}>
              <div style={{fontSize:11,fontWeight:700,color:'#e8e0d8',marginBottom:14,textTransform:'uppercase',letterSpacing:'0.4px'}}>New cPanel Server</div>
              ...
              <Field label='cPanel API Token' required hint='Create in cPanel Security > Manage API Tokens. Never expires.'>
                <div style={{position:'relative'}}>
                  ...
                  <button onClick={()=>setTokenVis(v=>!v)} style={{position:'absolute',right:10,top:'50%',transform:'translateY(-50%)',background:'none',border:'none',cursor:'pointer',color:'#b0a8a0',padding:0}}>{tokenVis?<EyeOff size={14}/>:<Eye size={14}/>}</button>
                </div>
              </Field>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:10,marginTop:4}}>
                <label style={{display:'flex',alignItems:'flex-start',gap:8,cursor:'pointer'}}><input type='checkbox' checked={saveServer} onChange={e=>setSaveServer(e.target.checked)} style={{marginTop:2}}/><span style={{fontSize:12,color:'#e8e0d8',lineHeight:1.5}}>Save this server for future one-click installs</span></label>
                <label style={{display:'flex',alignItems:'flex-start',gap:8,cursor:'pointer'}}><input type='checkbox' checked={autoInstallFuture} onChange={e=>setAutoInstallFuture(e.target.checked)} style={{marginTop:2}}/><span style={{fontSize:12,color:'#e8e0d8',lineHeight:1.5}}>Auto-install when cert is reissued or renewed</span></label>
              </div>
            </div>}
            ...
            <div style={{display:'flex',gap:8}}>
              <button onClick={()=>setPhase('select_type')} style={{padding:'10px 16px',borderRadius:8,border:'1px solid #e2e8f0',background:'white',cursor:'pointer',fontFamily:'inherit',fontSize:13,color:'#e8e0d8'}}>Back</button>
          ...
          {(phase==='running'||phase==='done'||phase==='error') && (<>
            <div style={{fontSize:11,fontWeight:700,color:'#e8e0d8',textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:4}}>Installation Progress</div>
            <div>
            ...
            <div style={{display:'flex',gap:8,marginTop:16}}>
              {phase==='error' && <button onClick={()=>{setPhase('configure');setBusy(false)}} style={{flex:1,padding:'10px',border:'1px solid #e2e8f0',borderRadius:8,background:'white',cursor:'pointer',fontFamily:'inherit',fontSize:13,color:'#e8e0d8'}}>Try again</button>}
