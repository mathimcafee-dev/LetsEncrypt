...
const MANDATES = [
  ...
  { year: '2029', label: 'Mar 2029', days: 47,  col: '#c0392b' },
]
...
function slaCol(s) { return s >= 80 ? '#4ade80' : s >= 50 ? '#f59e0b' : '#c0392b' }
...
function Tick({ ok }) {
  return ok
    ...
    : <XCircle size={13} color="rgba(255,255,255,0.15)" strokeWidth={2} style={{ flexShrink:0 }}/>
}
...
export default function ComplianceCentre({ nav, user }) {
  ...
  // Styles
  var card = { background:'rgba(255,255,255,0.04)', border:'1px solid rgba(192,57,43,0.18)', borderRadius:10, padding:'16px 18px' }
      ...
      <div style={{ maxWidth:1100, margin:'0 auto', padding:'28px 24px 60px' }}>
        ...
        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:28, flexWrap:'wrap', gap:12 }}>
          <div>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:5 }}>
              ...
              {hasSla && <span style={{ fontSize:10, fontWeight:700, padding:'2px 8px', borderRadius:99, background:'rgba(192,57,43,0.15)', color:'#c0392b', letterSpacing:'.06em' }}>{(sub.plan||'PREMIUM').toUpperCase()}</span>}
            </div>
            <div style={{ fontSize:12, color:'rgba(240,237,232,0.35)' }}>47-Day mandate readiness + SLA compliance + certificate health</div>
          </div>
          <div style={{ display:'flex', gap:8 }}>
            {hasSla && (
              <button onClick={generateReport} disabled={genLoad} style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 16px', background:'rgba(192,57,43,0.12)', border:'1px solid rgba(192,57,43,0.3)', borderRadius:8, color:'#f0ede8', fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:F }}>
                <FileText size={13}/>{genLoad ? 'Generating...' : 'Generate report'}
              </button>
            )}
            <button onClick={load} style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 14px', background:'transparent', border:'1px solid rgba(240,237,232,0.12)', borderRadius:8, color:'rgba(240,237,232,0.5)', fontSize:12, cursor:'pointer', fontFamily:F }}>
              <RefreshCw size={12}/>Refresh
            </button>
          </div>
        </div>

        {msg && <div style={{ background:'rgba(74,222,128,0.08)', border:'1px solid rgba(74,222,128,0.25)', borderRadius:8, padding:'10px 14px', color:'#4ade80', fontSize:12, marginBottom:20 }}>{msg}</div>}
        ...
        <div style={{ display:'grid', gridTemplateColumns: hasSla ? '1fr 1fr 1fr' : '1fr 1fr', gap:12, marginBottom:20 }}>
          ...
          <div style={{ ...card, display:'flex', alignItems:'center', gap:16 }}>
            ...
            <div>
              ...
              <div style={{ fontSize:12, color:'rgba(240,237,232,0.4)', marginTop:4 }}>
              ...
              {MANDATES.map(function(m) {
                ...
                return (
                  <div key={m.year} style={{ display:'flex', alignItems:'center', gap:8 }}>
                    ...
                    <span style={{ fontSize:10, color:'rgba(240,237,232,0.3)', marginLeft:'auto' }}>{count}/{rows.length} ready</span>
                  </div>
                )
              })}
            </div>
          ...
          {/* No SLA - upsell card */}
          {!hasSla && (
            <div style={{ ...card, border:'1px solid rgba(192,57,43,0.25)', display:'flex', flexDirection:'column', justifyContent:'center' }}>
              ...
              <button onClick={function(){ if(nav) nav('/pricing') }} style={{ alignSelf:'flex-start', padding:'7px 16px', background:'#c0392b', border:'none', borderRadius:7, color:'#fff', fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:F }}>Upgrade to Premium</button>
            </div>
          )}
        </div>
        ...
        <div style={{ ...card, padding:0, overflow:'hidden', marginBottom:20 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 16px', borderBottom:'1px solid rgba(192,57,43,0.15)' }}>
            <span style={{ fontSize:12, fontWeight:600, color:'#f0ede8' }}>Certificate Coverage</span>
          ...
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            <thead>
              <tr style={{ background:'rgba(192,57,43,0.06)' }}>
                <th style={{ ...th, width:'22%' }} onClick={function(){ toggleSort('domain') }}>
                  <span style={{ display:'flex', alignItems:'center', gap:3 }}>Domain{sortKey==='domain'&&(sortAsc?<ChevronUp size={9} color="#c0392b"/>:<ChevronDown size={9} color="#c0392b"/>)}</span>
                </th>
                <th style={{ ...th, width:'11%' }} onClick={function(){ toggleSort('expiry') }}>
                  <span style={{ display:'flex', alignItems:'center', gap:3 }}>Expires{sortKey==='expiry'&&(sortAsc?<ChevronUp size={9} color="#c0392b"/>:<ChevronDown size={9} color="#c0392b"/>)}</span>
                </th>
                <th style={{ ...th, width:'7%' }}>Days</th>
                <th style={{ ...th, width:'7%' }}>Validity</th>
                <th style={{ ...th, width:'9%' }} onClick={function(){ toggleSort('score') }}>
                  <span style={{ display:'flex', alignItems:'center', gap:3 }}>Readiness{sortKey==='score'&&(sortAsc?<ChevronUp size={9} color="#c0392b"/>:<ChevronDown size={9} color="#c0392b"/>)}</span>
                </th>
            ...
            <tbody>
              {sorted.length === 0 ? (
                <tr><td colSpan={12} style={{ padding:28, textAlign:'center', color:'rgba(240,237,232,0.3)', fontSize:13 }}>No active certificates</td></tr>
                ...
                return [
                  <tr key={c.id} onClick={function(){ setExpanded(isExpanded ? null : c.id) }}
                    style={{ borderTop:'1px solid rgba(192,57,43,0.08)', cursor:'pointer', transition:'background .1s',
                    ...
                    <td style={{ padding:'10px 12px', fontSize:13, color:'#f0ede8', fontFamily:MONO }}>{c.domain}</td>
                    <td style={{ padding:'10px 12px', fontSize:11, color:'rgba(240,237,232,0.5)' }}>{fmtDate(c.expires_at)}</td>
                    <td style={{ padding:'10px 12px', fontSize:13, fontWeight:600, color:dlCol }}>{Math.max(0,dl)}</td>
                    <td style={{ padding:'10px 12px', fontSize:11, color:'rgba(240,237,232,0.4)' }}>{v !== null ? v+'d' : 'N/A'}</td>
                  ...
                  isExpanded && (
                    <tr key={c.id + '-expanded'} style={{ background:'rgba(192,57,43,0.04)' }}>
                      <td colSpan={12} style={{ padding:'12px 16px', borderTop:'1px solid rgba(192,57,43,0.1)' }}>
                        <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:8 }}>
                          {[
                            ...
                            return (
                              <div key={ch.label} style={{ background:'rgba(255,255,255,0.03)', borderRadius:8, padding:'10px 12px', border:'1px solid ' + (ch.ok ? 'rgba(74,222,128,0.15)' : 'rgba(248,113,113,0.15)') }}>
                                <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:4 }}>
                                  <Tick ok={ch.ok}/>
                                  <span style={{ fontSize:11, fontWeight:600, color: ch.ok ? '#4ade80' : '#f87171' }}>+{ch.pts} pts</span>
                                </div>
                                <div style={{ fontSize:11, color:'rgba(240,237,232,0.7)', lineHeight:1.4 }}>{ch.label}</div>
        ...
        {hasSla && (
          <div>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}>
              <span style={{ fontSize:12, fontWeight:600, color:'#f0ede8' }}>Compliance Reports</span>
              <span style={{ fontSize:11, color:'rgba(240,237,232,0.35)' }}>SOC2 / ISO 27001 audit evidence</span>
            </div>
            {reports.length === 0 ? (
              <div style={{ ...card, textAlign:'center', padding:'20px', color:'rgba(240,237,232,0.3)', fontSize:13 }}>
                ...
                {reports.map(function(r) {
                  ...
                  return (
                    <div key={r.id} style={{ ...card, display:'flex', alignItems:'center', gap:12 }}>
                      <div style={{ width:40, height:40, borderRadius:8, background:'rgba(192,57,43,0.12)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                        <FileText size={16} color="#c0392b"/>
                      </div>
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ fontSize:12, fontWeight:600, color:'#f0ede8' }}>{r.report_month}</div>
                      ...
                      {r.report_url && (
                        <a href={r.report_url} target="_blank" rel="noreferrer"
                          style={{ display:'flex', alignItems:'center', gap:4, fontSize:11, color:'#c0392b', textDecoration:'none', fontWeight:600, flexShrink:0 }}>
