    ...
    <div style={{ padding:'28px 28px 60px', fontFamily:"system-ui,-apple-system,'Segoe UI',sans-serif" }}>
      ...
      <div style={{ background:'transparent', border:'0.5px solid #e8edf2', borderRadius:8, padding:'min(28px,5vw) min(32px,4vw)', marginBottom:16, borderLeft:'3px solid #c0392b' }}>
        <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:16 }}>
          ...
          <span style={{ fontSize:10, fontWeight:500, color:'rgba(240,237,232,0.45)', textTransform:'uppercase', letterSpacing:'.5px' }}>Mission</span>
        </div>
        <p style={{ fontSize:20, fontWeight:500, color:'#f0ede8', lineHeight:1.5, letterSpacing:'-.3px', margin:'0 0 14px', maxWidth:680 }}>
          The web should be encrypted by default — and managing that encryption should never be a barrier.
        </p>
        <p style={{ fontSize:13, color:'rgba(240,237,232,0.6)', lineHeight:1.8, margin:'0 0 10px', maxWidth:720 }}>
        ...
        <p style={{ fontSize:13, color:'rgba(240,237,232,0.6)', lineHeight:1.8, margin:0, maxWidth:720 }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(min(300px,100%),1fr))', gap:12, marginBottom:16 }}>
        {PILLARS.map(({ icon:Icon, title, body }) => (
          <div key={title} style={{ background:'transparent', border:'0.5px solid #e8edf2', borderRadius:8, padding:'20px 22px' }}>
            ...
            <div style={{ fontSize:13, fontWeight:500, color:'#f0ede8', marginBottom:6 }}>{title}</div>
            <div style={{ fontSize:12, color:'rgba(240,237,232,0.6)', lineHeight:1.7 }}>{body}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ background:'transparent', border:'0.5px solid #e8edf2', borderRadius:8, padding:'20px 24px', marginBottom:16 }}>
        <div style={{ fontSize:10, fontWeight:500, color:'rgba(240,237,232,0.45)', textTransform:'uppercase', letterSpacing:'.5px', marginBottom:14 }}>Stack & infrastructure</div>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(180px,1fr))', gap:8 }}>
          {[
            ...
            <div key={k} style={{ padding:'10px 12px', background:'transparent', borderRadius:6, border:'0.5px solid rgba(192,57,43,0.08)' }}>
              <div style={{ fontSize:9, fontWeight:500, color:'rgba(240,237,232,0.45)', textTransform:'uppercase', letterSpacing:'.4px', marginBottom:4 }}>{k}</div>
              <div style={{ fontSize:12, color:'#f0ede8', fontFamily:"'JetBrains Mono',monospace" }}>{v}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display:'flex', gap:10 }}>
        <button onClick={() => nav('/developer')} style={{ display:'inline-flex', alignItems:'center', gap:6, background:'#f0ede8', color:'#f0ede8', border:'none', borderRadius:6, padding:'9px 16px', fontSize:12, fontWeight:500, cursor:'pointer', fontFamily:'inherit' }}>
          <Code2 size={13}/> Meet the developer
        </button>
        <button onClick={() => nav('/contact')} style={{ display:'inline-flex', alignItems:'center', gap:6, background:'transparent', color:'#f0ede8', border:'0.5px solid #e8edf2', borderRadius:6, padding:'9px 16px', fontSize:12, fontWeight:500, cursor:'pointer', fontFamily:'inherit' }}>
