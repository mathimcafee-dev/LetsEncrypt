...
function StatusPill({ status }) {
  const map = {
    online:  { bg: '#f0fdf4', color: '#15803d', label: 'Online',   dot: '#16a34a' },
    offline: { bg: '#fef2f2', color: '#b91c1c', label: 'Offline',  dot: '#dc2626' },
    never:   { bg: '#f8fafc', color: '#64748b', label: 'Never',    dot: '#94a3b8' },
    unknown: { bg: '#fffbeb', color: '#b45309', label: 'Unknown',  dot: '#d97706' },
  }
...
function JobBadge({ status }) {
  ...
  const map = {
    ...
    queued:  { bg: '#fffbeb', color: '#b45309' },
    claimed: { bg: '#eff6ff', color: '#1d4ed8' },
  }
      ...
      <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 420, zIndex: 50,
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 1,
          ...
          {[
            ...
            <div key={label} style={{ background: 'var(--v2-surface-3)', padding: '10px 14px' }}>
              <div style={{ fontSize: 10, color: 'var(--v2-text-3)', marginBottom: 3 }}>{label}</div>
              <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--v2-text)' }}>{val}</div>
            </div>
          ))}
        </div>
              ...
              <div key={label} style={{ marginBottom: 10 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 500,
                    color: value >= danger ? '#dc2626' : value >= warn ? '#d97706' : '#16a34a',
                ...
                <div style={{ height: 5, borderRadius: 3, background: 'var(--v2-border)', overflow: 'hidden' }}>
                  <div style={{ height: '100%', borderRadius: 3, transition: 'width .6s',
                    ...
                    background: value >= danger ? '#dc2626' : value >= warn ? '#d97706' : '#16a34a' }}/>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ flex: 1, padding: '14px 20px' }}>
          {loading ? (
            ...
            !jobs?.length ? (
              <div style={{ fontSize: 12, color: 'var(--v2-text-3)', textAlign: 'center', padding: '24px 0' }}>
              ...
              <div key={job.id} style={{ display: 'flex', gap: 10, padding: '8px 0',
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 10, color: 'var(--v2-text-3)', marginTop: 2 }}>
                    ...
                    {job.error_message && (
                      <span style={{ color: '#dc2626' }}> · {job.error_message.slice(0, 60)}</span>
                    )}
                  </div>
                </div>
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
              {[
                ...
                <div key={label} style={{ display: 'flex', justifyContent: 'space-between',
                  ...
                  <span style={{ fontSize: 12, color: 'var(--v2-text-3)' }}>{label}</span>
                  <span style={{ fontSize: 12, color: 'var(--v2-text)', fontWeight: 500,
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)',
        ...
        <p style={{ fontSize: 13, color: 'var(--v2-text-2)', marginBottom: 8 }}>
      ...
      <div className="v2-container" style={{ maxWidth: 1100 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 20 }}>
          {[
            ...
            { val: highLoad.length,label: 'High load',    color: '#d97706', filter: 'all' },
        ...
        {/* ── Table ── */}
        {loading ? (
          <div style={{ textAlign: 'center', padding: '56px 0', color: 'var(--v2-text-3)' }}>
          ...
          <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--v2-text-3)', fontSize: 13 }}>
                ...
                <div key={agent.id}
                  ...
                  <div style={{ minWidth: 0, display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ width: 32, height: 32, borderRadius: 8, flexShrink: 0,
                      background: status === 'online' ? '#f0fdf4' : status === 'offline' ? '#fef2f2' : '#f8fafc',
                  ...
                  {/* Certs */}
                  <div style={{ fontSize: 12, color: 'var(--v2-text-2)', fontWeight: 500 }}>
                    {agent.certs_managed ?? '—'}
                  </div>
