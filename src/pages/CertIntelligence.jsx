...
const URGENCY = {
  ...
  warning:  { label: 'Warning',    color: '#f5f0eb', bg: 'rgba(239,68,68,0.08)', border: 'rgba(192,57,43,0.25)', dot: '#f0ede8' },
  upcoming: { label: 'Upcoming',   color: '#f5f0eb', bg: 'transparent', border: 'rgba(192,57,43,0.3)', dot: '#f0ede8' },
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
        ...
        <h2 style={{ fontSize:17, fontWeight: 800, color: '#f5f0eb', letterSpacing: '-0.3px', margin: 0 }}>{title}</h2>
      </div>
      ...
      <Card>
        <div style={{ display: 'grid', gridTemplateColumns: '28px 2fr 1fr 1fr 1fr 1fr 80px',
          ...
          <div style={{ display:'flex', alignItems:'center', cursor:'pointer' }} onClick={toggleAll}>
            {selected.size > 0 && selected.size === certs.length
              ? <CheckSquare size={13} style={{ color:'#f5f0eb' }}/>
              : <Square size={13} style={{ color:'#9a918a' }}/>}
          </div>
            ...
            <div key={cert.id} style={{ display: 'grid', gridTemplateColumns: '28px 2fr 1fr 1fr 1fr 1fr 80px',
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                  ...
                  <span style={{ fontSize:12, fontWeight: 600, fontFamily: 'monospace', color: '#f5f0eb',
        ...
        {certs.length > 0 && (
          <div style={{ padding: '8px 16px', fontSize:11, color: 'var(--v2-text-3)', borderTop: '0.5px solid var(--v2-border)' }}>
            ...
            {selected.size > 0 && <span style={{ color:'#f5f0eb', marginLeft:8 }}>· {selected.size} selected</span>}
          </div>
        )}
      </Card>
              ...
              <div style={{ padding:'16px 20px 20px', display:'flex', flexDirection:'column', gap:10 }}>
                ...
                {caUrl && (
                  <button onClick={() => { window.open(caUrl, '_blank', 'noopener'); setRenewModal(null) }}
                    ...
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:13, fontWeight:700, color:'#f5f0eb', marginBottom:3 }}>Renew via {caName}</div>
        ...
        <div style={{ position:'fixed', inset:0, zIndex:1000, display:'flex',
          ...
          <div style={{ background:'var(--v2-surface)', borderRadius:14, width:'100%', maxWidth:400,
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:12 }}>
              ...
              <div>
                <div style={{ fontSize:15, fontWeight:700, color:'#f5f0eb' }}>
      ...
      <Card style={{ padding: '18px 20px', marginBottom: 20 }}>
        ...
        {conns.length === 0 ? (
          <div style={{ fontSize:13, color: 'var(--v2-text-3)' }}>
            ...
            <span style={{ color: '#f5f0eb', cursor: 'pointer', textDecoration: 'underline' }}
    ...
    <div>
      ...
      <Card style={{ padding: '18px 20px', marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <div style={{ fontSize:13, fontWeight: 700, color: '#f5f0eb', marginBottom: 4 }}>Portfolio cost analysis</div>
      ...
      {opps.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(180px,1fr))', gap: 10, marginBottom: 20 }}>
          ...
          <Card style={{ padding: '14px 16px', borderLeft: '3px solid #c0392b' }}>
            <div style={{ fontSize:26, fontWeight: 800, color: '#f5f0eb', letterSpacing: '-0.5px', marginBottom: 4 }}>{consolidation.length}</div>
          ...
          <Card style={{ padding: '14px 16px', borderLeft: '3px solid #f07059' }}>
            <div style={{ fontSize:26, fontWeight: 800, color: '#f5f0eb', letterSpacing: '-0.5px', marginBottom: 4 }}>{duplicates.length}</div>
      ...
      {consolidation.length > 0 && (
        <>
          <div style={{ fontSize:13, fontWeight: 700, color: '#f5f0eb', marginBottom: 10 }}>
          ...
          <Card style={{ marginBottom: 16 }}>
            ...
            {consolidation.map((opp, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr auto',
                ...
                onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                <div>
                  <div style={{ fontSize:12, fontWeight: 600, fontFamily: 'monospace', color: '#f5f0eb',
          ...
          <Card>
            {duplicates.map((opp, i) => (
              <div key={i} style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12,
                ...
                <AlertTriangle size={14} color="#e07060" style={{ flexShrink: 0 }}/>
                <div style={{ flex: 1 }}>
                  <span style={{ fontSize:12, fontWeight: 600, fontFamily: 'monospace', color: '#f5f0eb' }}>{opp.domain}</span>
...
export default function CertIntelligence({ nav }) {
  ...
  const TABS = [
    { id: 'timeline',      label: 'Expiry Timeline',        icon: Activity,    color: '#f5f0eb' },
    { id: 'shadow',        label: 'Shadow IT Scanner',      icon: Search,      color: '#f5f0eb' },
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '28px 24px' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
            ...
            <h1 style={{ fontSize:22, fontWeight: 800, color: '#f5f0eb', letterSpacing: '-0.5px', margin: 0 }}>
