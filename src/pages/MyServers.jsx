...
const CA_DEFS = {
  digicert:{ name:'DigiCert CertCentral', color:'#c0392b', bg:'rgba(31,92,78,0.09)', border:'rgba(0,0,0,0.1)', logo:'DC',
  ...
  sslcom:{ name:'SSL.com', color:'#ffffff', bg:'rgba(31,92,78,0.08)', border:'rgba(31,92,78,0.2)', logo:'SL',
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.7)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding:24 }}>
      <div style={{ background:'#1a0404', borderRadius:14, width:'100%', maxWidth:460, border:'1px solid rgba(31,92,78,0.2)', boxShadow:'0 24px 64px rgba(0,0,0,0.6)', overflow:'hidden' }}>
        ...
        <div style={{ padding:24 }}>
          ...
          {error && <div style={{ fontSize:12, color:'#c0392b', marginBottom:12, padding:'8px 12px', background:'rgba(31,92,78,0.09)', borderRadius:6 }}>{error}</div>}
                ...
                {agents.length === 0 ? (
                  <div style={{
                    ...
                    <button onClick={() => setShowAddServer(true)} style={{
                      ...
                      background:'#1f5c4e', color:'#ffffff', fontSize:13,
              ...
              <div>
                ...
                {cpanelCreds.length === 0 ? (
                  <div style={{ textAlign:'center', padding:'min(40px,5vw) min(24px,4vw)', background:'var(--v2-surface)', borderRadius:12, border:'1px solid rgba(0,0,0,0.1)' }}>
                    ...
                    <button onClick={() => setShowAddHosting(true)} style={{
                      padding:'9px 18px', borderRadius:8, border:'none',
                      background:'#1f5c4e', color:'#ffffff', fontSize:13, fontWeight:600,
                      cursor:'pointer', fontFamily:'inherit',
                      display:'inline-flex', alignItems:'center', gap:6,
                    }}>
              ...
              <div>
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
                  ...
                  <div style={{display:'flex',gap:8}}>
                    ...
                    <button onClick={()=>setShowAddCA(true)}
                      style={{display:'flex',alignItems:'center',gap:6,padding:'8px 16px',fontSize:12,fontWeight:600,background:'#1f5c4e',border:'none',borderRadius:8,cursor:'pointer',fontFamily:'inherit',color:'#ffffff'}}>
                ...
                {caConns.length===0?(
                  <div style={{textAlign:'center',padding:'60px 32px',background:'var(--v2-surface)',borderRadius:12,border:'1px solid rgba(0,0,0,0.1)'}}>
                    ...
                    <button onClick={()=>setShowAddCA(true)} style={{padding:'9px 20px',fontSize:13,background:'#1f5c4e',border:'none',borderRadius:8,cursor:'pointer',fontFamily:'inherit',color:'#ffffff',fontWeight:600,display:'inline-flex',alignItems:'center',gap:6}}><Plus size={13}/> Connect CA</button>
                  </div>
                ):(
                      ...
                      return(
                        <div key={conn.id}>
                          <div style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr 110px 130px',minWidth:600,padding:'14px 18px',alignItems:'center',borderBottom:i<caConns.length-1?'1px solid rgba(31,92,78,0.08)':'none'}}>
                            ...
                            <div><span style={{display:'inline-flex',alignItems:'center',gap:4,fontSize:11,fontWeight:600,padding:'3px 8px',borderRadius:4,background:isActive?'transparent':'rgba(31,92,78,0.09)',color:isActive?'#4ade80':'#f87171',border:`1px solid ${isActive?'rgba(74,222,128,0.3)':'rgba(248,113,113,0.3)'}`}}><span style={{width:5,height:5,borderRadius:'50%',background:isActive?'#4ade80':'#f87171'}}/>{isActive?'Connected':'Error'}</span></div>
                            <div style={{display:'flex',gap:5}}>
                              <button onClick={()=>caSync(conn.id)} disabled={caSyncing===conn.id}
                                style={{padding:'5px 10px',fontSize:11,background:'rgba(31,92,78,0.07)',border:'1px solid rgba(0,0,0,0.1)',borderRadius:6,cursor:'pointer',fontFamily:'inherit',color:'#e8e0d8',display:'flex',alignItems:'center',gap:4}}>
                          ...
                          {res&&(<div style={{padding:'8px 18px',borderTop:'1px solid rgba(31,92,78,0.08)',background:res.ok?'rgba(74,222,128,0.05)':'rgba(31,92,78,0.07)',display:'flex',alignItems:'center',gap:7}}>
                            {res.ok?<Check size={12} style={{color:'#16a068'}}/>:<AlertCircle size={12} style={{color:'#c0392b'}}/>}
                  ...
                  <div style={{padding:'18px 22px 22px'}}>
                    {!addCa?(
                      ...
                      <div>
                        ...
                        {addError&&(<div style={{background:'rgba(31,92,78,0.09)',border:'1px solid rgba(248,113,113,0.3)',borderRadius:7,padding:'8px 12px',marginBottom:12,fontSize:12,color:'#c0392b',display:'flex',gap:6,alignItems:'flex-start'}}><AlertTriangle size={12} style={{flexShrink:0,marginTop:1}}/>{addError}</div>)}
                        <div style={{display:'flex',gap:8,marginTop:4}}>
                          ...
                          <button onClick={caSaveConn} disabled={addSaving} style={{flex:1,padding:'9px',fontSize:13,background:'#1f5c4e',border:'none',borderRadius:8,cursor:'pointer',fontFamily:'inherit',color:'#ffffff',fontWeight:600,display:'flex',alignItems:'center',justifyContent:'center',gap:6}}>
                  ...
                  <div style={{padding:'18px 22px 22px'}}>
                    {!importResult?(
                      <>
                        ...
                        <button onClick={caImport} disabled={importing||!pemText.trim()} style={{width:'100%',padding:'9px',fontSize:13,background:'#1f5c4e',border:'none',borderRadius:8,cursor:'pointer',fontFamily:'inherit',color:'#ffffff',fontWeight:600,display:'flex',alignItems:'center',justifyContent:'center',gap:6}}>
              ...
              return(
                <div style={{position:'fixed',inset:0,zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20,background:'rgba(10,10,20,0.7)',backdropFilter:'blur(4px)'}}>
                  <div style={{background:'#1a0a0a',borderRadius:14,width:'100%',maxWidth:400,padding:'24px',boxShadow:'0 24px 64px rgba(0,0,0,0.5)',border:'1px solid rgba(31,92,78,0.2)'}}>
                    ...
                    {n>0&&(<label style={{display:'flex',alignItems:'center',gap:10,cursor:'pointer',padding:'10px 12px',borderRadius:8,marginBottom:14,background:caDelCerts?'rgba(31,92,78,0.08)':'rgba(255,255,255,0.03)',border:`1px solid ${caDelCerts?'rgba(31,92,78,0.2)':'rgba(0,0,0,0.07)'}`,transition:'all .15s'}}>
