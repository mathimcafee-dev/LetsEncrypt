...
import {
  ...
  Zap, Lock, Clock, Upload, FileText, Info, Eye, EyeOff, ExternalLink, ChevronRight, AlertTriangle
} from 'lucide-react'
...
const _SB_UNUSED = import.meta.env.VITE_SUPABASE_URL || 'https://frthcwkntciaakqsppss.supabase.co'
      ...
      <div style={{
        ...
        {caTab==='ca'&&(
          <div>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
              <span style={{fontSize:13,color:'#e8e0d8'}}>{connections.length} connection{connections.length!==1?'s':''} · {caCertCount} certificates tracked</span>
              <div style={{display:'flex',gap:8}}>
                <button onClick={()=>{setShowImport(true);setPemText('');setImportResult(null)}}
                  style={{display:'flex',alignItems:'center',gap:6,padding:'8px 14px',fontSize:13,background:'var(--v2-surface)',border:'1px solid rgba(192,57,43,0.25)',borderRadius:8,cursor:'pointer',fontFamily:'inherit',color:'#e8e0d8',fontWeight:500}}>
                  <Upload size={14}/> Import PEM</button>
                <button onClick={()=>setShowAddCA(true)}
                  style={{display:'flex',alignItems:'center',gap:6,padding:'8px 16px',fontSize:13,fontWeight:600,background:'#c0392b',border:'none',borderRadius:8,cursor:'pointer',fontFamily:'inherit',color:'#ffffff'}}>
                  <Plus size={14}/> Connect CA</button>
              </div>
            </div>
            {connections.length===0?(
              <div style={{background:'var(--v2-surface)',border:'0.5px solid var(--v2-border)',borderRadius:8,padding:'60px 32px',textAlign:'center'}}>
                ...
                <div style={{fontSize:13,color:'#b0a8a0',maxWidth:420,margin:'0 auto 20px'}}>Connect DigiCert CertCentral or Sectigo SCM to import your external certificate portfolio.</div>
                <button onClick={()=>setShowAddCA(true)} style={{padding:'9px 20px',fontSize:13,background:'#c0392b',border:'none',borderRadius:8,cursor:'pointer',fontFamily:'inherit',color:'#ffffff',fontWeight:600,display:'inline-flex',alignItems:'center',gap:6}}><Plus size={14}/> Connect CA</button>
              </div>
            ):(
              <div style={{background:'var(--v2-surface)',border:'0.5px solid var(--v2-border)',borderRadius:8,overflowX:'auto'}}>
                <div style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr 120px 140px',minWidth:650,padding:'10px 20px',background:'var(--v2-surface-2)',borderBottom:'0.5px solid rgba(255,255,255,0.08)'}}>
                  {['Certificate authority','Label','Certs','Status','Actions'].map(h=>(<div key={h} style={{fontSize:11,fontWeight:600,color:'#b0a8a0',textTransform:'uppercase',letterSpacing:'0.5px'}}>{h}</div>))}
                </div>
                {connections.map((conn,i)=>{
                  ...
                  return(
                    <div key={conn.id} style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr 120px 140px',minWidth:650,padding:'14px 20px',alignItems:'center',borderBottom:i<connections.length-1?'0.5px solid var(--v2-border)':'none',background:'var(--v2-surface)'}}>
                      <div style={{display:'flex',alignItems:'center',gap:12}}>
                        <div style={{width:34,height:34,borderRadius:7,background:def.color,display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:700,color:'#ffffff',flexShrink:0}}>{def.logo}</div>
                        <div><div style={{fontSize:13,fontWeight:500,color:'#ffffff'}}>{def.name}</div><div style={{fontSize:11,color:'#b0a8a0',marginTop:1}}>Certificate authority</div></div>
                      </div>
                      <div style={{fontSize:13,color:'#e8e0d8'}}>{conn.label||'—'}</div>
                      <div style={{fontSize:13,color:'#e8e0d8'}}>{conn.cert_count??'—'}</div>
                      <div><span style={{display:'inline-flex',alignItems:'center',gap:5,fontSize:12,fontWeight:500,padding:'4px 10px',borderRadius:4,background:isActive?'transparent':'rgba(192,57,43,0.12)',color:isActive?'#f0ede8':'#a93226',border:`0.5px solid ${isActive?'rgba(192,57,43,0.3)':'rgba(192,57,43,0.25)'}`}}><span style={{width:6,height:6,borderRadius:'50%',background:isActive?'#f0ede8':'#f87171'}}/>{isActive?'Connected':'Error'}</span></div>
                      <div style={{display:'flex',gap:6,alignItems:'center'}}>
                        <button onClick={()=>doSync(conn.id)} disabled={syncing===conn.id}
                          style={{padding:'5px 10px',fontSize:12,background:'var(--v2-surface-2)',border:'1px solid rgba(192,57,43,0.25)',borderRadius:6,cursor:'pointer',fontFamily:'inherit',color:'#e8e0d8',display:'flex',alignItems:'center',gap:4}}>
                          <RefreshCw size={12} style={{animation:syncing===conn.id?'spin .8s linear infinite':'none'}}/>{syncing===conn.id?'Syncing':'Sync'}</button>
                        <button onClick={()=>setDelConn(conn.id)} style={{width:28,height:28,display:'flex',alignItems:'center',justifyContent:'center',background:'none',border:'0.5px solid #fecaca',borderRadius:5,cursor:'pointer',color:'#f87171'}}><Trash2 size={12}/></button>
                      </div>
                    </div>
                  )
                })}
                {Object.entries(syncResult).filter(([,r])=>r).map(([id,r])=>(
                  <div key={id} style={{padding:'10px 20px',borderTop:'0.5px solid var(--v2-border)',background:r.ok?'transparent':'rgba(192,57,43,0.12)',display:'flex',alignItems:'center',gap:8}}>
                    {r.ok?<Check size={13} style={{color:'#ffffff'}}/>:<AlertCircle size={13} style={{color:'#f87171'}}/>}
                    <span style={{fontSize:12,color:r.ok?'#f0ede8':'#f87171'}}>{r.ok?`Sync complete — ${r.imported||0} certificates imported`:r.error}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        {showAddCA&&(
          <div style={{position:'fixed',inset:0,zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20,background:'rgba(15,23,42,0.5)',backdropFilter:'blur(4px)'}}>
            <div style={{background:'rgba(255,255,255,0.03)',borderRadius:14,width:'100%',maxWidth:480,boxShadow:'0 24px 64px rgba(0,0,0,0.18)',border:'0.5px solid var(--v2-border)'}}>
              <div style={{padding:'18px 22px 14px',borderBottom:'0.5px solid rgba(255,255,255,0.08)',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                <div style={{fontSize:15,fontWeight:500}}>Connect a CA</div>
                <button onClick={()=>{setShowAddCA(false);setAddCa(null)}} style={{background:'none',border:'0.5px solid var(--v2-border)',borderRadius:6,cursor:'pointer',color:'#b0a8a0',padding:'4px 6px'}}><X size={14}/></button>
              </div>
              <div style={{padding:'18px 22px 22px'}}>
                {!addCa?(
                  <div style={{display:'flex',flexDirection:'column',gap:8}}>
                    <div style={{fontSize:12,color:'#e8e0d8',marginBottom:4,lineHeight:1.6}}>Choose a CA. SSLVault pulls your existing certificates for monitoring. <strong style={{color:'#ffffff'}}>No private keys needed.</strong></div>
                    {Object.entries(CA_DEFS).map(([key,def])=>(
                      <div key={key} onClick={()=>openAdd(key)} style={{padding:'14px 16px',borderRadius:10,border:'0.5px solid var(--v2-border)',background:'rgba(255,255,255,0.03)',cursor:'pointer',display:'flex',alignItems:'center',gap:12,transition:'all .15s'}}
                        ...
                        <div style={{width:36,height:36,borderRadius:8,background:def.bg,border:`0.5px solid ${def.border}`,display:'flex',alignItems:'center',justifyContent:'center',fontWeight:700,fontSize:12,color:def.color,flexShrink:0}}>{def.logo}</div>
                        <div style={{flex:1}}><div style={{fontSize:13,fontWeight:500,color:'#ffffff'}}>{def.name}</div><div style={{fontSize:11,color:'#b0a8a0',marginTop:2}}>{def.desc}</div></div>
                  ...
                  <div>
                    <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:16,padding:'10px 12px',borderRadius:8,background:'var(--v2-surface-2)',border:'0.5px solid var(--v2-border)'}}>
                      <div style={{width:30,height:30,borderRadius:7,background:CA_DEFS[addCa].bg,display:'flex',alignItems:'center',justifyContent:'center',fontWeight:700,fontSize:11,color:CA_DEFS[addCa].color,flexShrink:0}}>{CA_DEFS[addCa].logo}</div>
                      <div style={{flex:1}}><div style={{fontSize:13,fontWeight:500,color:'#ffffff'}}>{CA_DEFS[addCa].name}</div></div>
                      {CA_DEFS[addCa].docs&&(<a href={CA_DEFS[addCa].docs} target="_blank" rel="noopener noreferrer" style={{fontSize:11,color:'#b0a8a0',display:'flex',alignItems:'center',gap:3}}>Docs <ExternalLink size={11}/></a>)}
                    </div>
                    <div style={{marginBottom:12}}><label style={{fontSize:10,fontWeight:700,textTransform:'uppercase',letterSpacing:'0.5px',color:'#b0a8a0',display:'block',marginBottom:5}}>Label</label>
                      <input value={addLabel} onChange={e=>setAddLabel(e.target.value)} placeholder="e.g. Production DigiCert" style={{width:'100%',padding:'9px 12px',borderRadius:7,fontSize:13,background:'var(--v2-surface-2)',border:'1px solid rgba(192,57,43,0.25)',color:'#e8e0d8',fontFamily:'inherit',boxSizing:'border-box'}}/></div>
                    {CA_DEFS[addCa].fields.map(f=>(
                      <div key={f.key} style={{marginBottom:12}}>
                        <label style={{fontSize:10,fontWeight:700,textTransform:'uppercase',letterSpacing:'0.5px',color:'#b0a8a0',display:'block',marginBottom:5}}>{f.label}{f.required?' *':''}</label>
                        <div style={{position:'relative'}}>
                          <input type={f.type==='password'&&!showPwd[f.key]?'password':'text'} value={addFields[f.key]||''} onChange={e=>setAddFields(p=>({...p,[f.key]:e.target.value}))} placeholder={f.placeholder}
                            style={{width:'100%',padding:'9px 12px',borderRadius:7,fontSize:13,background:'var(--v2-surface-2)',border:'1px solid rgba(192,57,43,0.25)',color:'#e8e0d8',fontFamily:'inherit',boxSizing:'border-box',paddingRight:f.type==='password'?36:12}}/>
                          {f.type==='password'&&(<button type="button" onClick={()=>setShowPwd(p=>({...p,[f.key]:!p[f.key]}))} style={{position:'absolute',right:10,top:'50%',transform:'translateY(-50%)',background:'none',border:'none',cursor:'pointer',color:'#b0a8a0'}}>{showPwd[f.key]?<EyeOff size={13}/>:<Eye size={13}/>}</button>)}
                        </div>
                      </div>
                    ))}
                    {addError&&(<div style={{background:'rgba(192,57,43,0.12)',border:'0.5px solid #fecaca',borderRadius:7,padding:'9px 12px',marginBottom:12,fontSize:12,color:'#f87171',display:'flex',gap:7,alignItems:'flex-start'}}><AlertTriangle size={13} style={{flexShrink:0,marginTop:1}}/>{addError}</div>)}
                    <div style={{display:'flex',gap:8}}>
                      <button onClick={()=>setAddCa(null)} style={{padding:'9px 16px',fontSize:13,background:'var(--v2-surface-2)',border:'1px solid rgba(192,57,43,0.25)',borderRadius:8,cursor:'pointer',fontFamily:'inherit',color:'#e8e0d8'}}>Back</button>
                      <button onClick={saveConnection} disabled={addSaving} style={{flex:1,padding:'9px',fontSize:13,background:'#c0392b',border:'none',borderRadius:8,cursor:'pointer',fontFamily:'inherit',color:'#ffffff',fontWeight:600,display:'flex',alignItems:'center',justifyContent:'center',gap:6}}>
        ...
        {showImport&&(
          <div style={{position:'fixed',inset:0,zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20,background:'rgba(15,23,42,0.5)',backdropFilter:'blur(4px)'}}>
            <div style={{background:'rgba(255,255,255,0.03)',borderRadius:14,width:'100%',maxWidth:480,boxShadow:'0 24px 64px rgba(0,0,0,0.18)',border:'0.5px solid var(--v2-border)'}}>
              <div style={{padding:'18px 22px 14px',borderBottom:'0.5px solid rgba(255,255,255,0.08)',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                <div><div style={{fontSize:15,fontWeight:500}}>Import certificate</div><div style={{fontSize:11,color:'#b0a8a0',marginTop:2}}>Paste cert PEM — domain, expiry &amp; issuer extracted automatically</div></div>
                <button onClick={()=>setShowImport(false)} style={{background:'none',border:'0.5px solid var(--v2-border)',borderRadius:6,cursor:'pointer',color:'#b0a8a0',padding:'4px 6px'}}><X size={14}/></button>
              </div>
              <div style={{padding:'18px 22px 22px'}}>
                {!importResult?(
                  <>
                    <div style={{background:'var(--v2-surface-2)',border:'0.5px solid var(--v2-border)',borderRadius:8,padding:'10px 12px',marginBottom:14,display:'flex',gap:8,alignItems:'flex-start'}}><Info size={13} style={{color:'#b0a8a0',flexShrink:0,marginTop:1}}/><div style={{fontSize:12,color:'#e8e0d8',lineHeight:1.6}}><strong>Only the certificate PEM is needed</strong> for tracking. Private key not required.</div></div>
                    <div style={{marginBottom:16}}><label style={{fontSize:10,fontWeight:700,textTransform:'uppercase',letterSpacing:'0.5px',color:'#b0a8a0',display:'block',marginBottom:6}}>Certificate PEM *</label>
                      <textarea rows={8} placeholder={'-----BEGIN CERTIFICATE-----
MIIFaz...
-----END CERTIFICATE-----'} value={pemText} onChange={e=>setPemText(e.target.value)} style={{width:'100%',padding:'9px 12px',borderRadius:7,fontSize:11,background:'var(--v2-surface-2)',border:'1px solid rgba(192,57,43,0.25)',color:'#e8e0d8',fontFamily:'monospace',resize:'vertical',boxSizing:'border-box'}}/></div>
                    <button onClick={doImport} disabled={importing||!pemText.trim()} style={{width:'100%',padding:'9px',fontSize:13,background:'#c0392b',border:'none',borderRadius:8,cursor:'pointer',fontFamily:'inherit',color:'#ffffff',fontWeight:600,display:'flex',alignItems:'center',justifyContent:'center',gap:6}}>
                  ...
                  <div style={{textAlign:'center',padding:'10px 0'}}>
                    ...
                    <div style={{display:'flex',gap:8}}>
                      <button onClick={()=>{setPemText('');setImportResult(null)}} style={{flex:1,padding:'9px',fontSize:13,background:'var(--v2-surface-2)',border:'1px solid rgba(192,57,43,0.25)',borderRadius:8,cursor:'pointer',fontFamily:'inherit',color:'#e8e0d8'}}>Import another</button>
                      <button onClick={()=>setShowImport(false)} style={{flex:1,padding:'9px',fontSize:13,background:'#c0392b',border:'none',borderRadius:8,cursor:'pointer',fontFamily:'inherit',color:'#ffffff',fontWeight:600}}>Done</button>
                    </div>
                  </div>
                ):(
                  <div style={{textAlign:'center',padding:'10px 0'}}>
                    ...
                    <button onClick={()=>setImportResult(null)} style={{padding:'9px 20px',fontSize:13,background:'var(--v2-surface-2)',border:'1px solid rgba(192,57,43,0.25)',borderRadius:8,cursor:'pointer',fontFamily:'inherit',color:'#e8e0d8'}}>Try again</button>
                  </div>
                )}
              </div>
            </div>
        ...
        {delConn&&(()=>{
          ...
          return(
            <div style={{position:'fixed',inset:0,zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20,background:'rgba(15,23,42,0.5)',backdropFilter:'blur(4px)'}}>
              <div style={{background:'rgba(255,255,255,0.03)',borderRadius:14,width:'100%',maxWidth:400,padding:'24px',boxShadow:'0 24px 64px rgba(0,0,0,0.18)',border:'0.5px solid var(--v2-border)'}}>
                ...
                <div style={{fontSize:13,color:'#e8e0d8',marginBottom:16,lineHeight:1.6}}>This CA connection will be disconnected and will no longer sync.{connCertCount>0&&<span style={{color:'#ffffff',fontWeight:500}}> {connCertCount} certificate{connCertCount!==1?'s':''} are linked.</span>}</div>
                {connCertCount>0&&(
                  <label style={{display:'flex',alignItems:'center',gap:10,cursor:'pointer',padding:'10px 12px',borderRadius:8,marginBottom:16,background:delCerts?'rgba(192,57,43,0.12)':'var(--v2-surface-2)',border:`0.5px solid ${delCerts?'rgba(192,57,43,0.25)':'var(--v2-border)'}`,transition:'all .15s'}}>
                    <input type="checkbox" checked={delCerts} onChange={e=>setDelCerts(e.target.checked)} style={{width:14,height:14,accentColor:'#f87171',flexShrink:0}}/>
                    <div><div style={{fontSize:12,fontWeight:600,color:delCerts?'#f87171':'#e8e0d8'}}>Also delete {connCertCount} imported certificate{connCertCount!==1?'s':''}</div><div style={{fontSize:11,color:'#b0a8a0',marginTop:1}}>{delCerts?'Certificates will be removed from inventory':'Certificates remain but stop syncing'}</div></div>
                  </label>
                )}
                <div style={{display:'flex',gap:8}}>
                  <button onClick={()=>{setDelConn(null);setDelCerts(true)}} style={{flex:1,padding:'9px',fontSize:13,background:'var(--v2-surface-2)',border:'1px solid rgba(192,57,43,0.25)',borderRadius:8,cursor:'pointer',fontFamily:'inherit',color:'#e8e0d8'}}>Cancel</button>
                  <button onClick={()=>deleteConn(delConn)} style={{flex:1,background:'#f87171',color:'#ffffff',border:'none',borderRadius:8,padding:'9px',cursor:'pointer',fontFamily:'inherit',fontWeight:600,fontSize:13,display:'flex',alignItems:'center',justifyContent:'center',gap:6}}>
                    <Trash2 size={13}/>{delCerts&&connCertCount>0?`Remove + delete ${connCertCount} certs`:'Remove connection'}
                  </button>
                </div>
              </div>
            </div>
...
export default function MyServers({ user }) {
  ...
  const [caTab,        setCaTab]       = useState('servers')
