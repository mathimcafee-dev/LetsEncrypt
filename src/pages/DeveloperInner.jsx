...
export default function DeveloperInner({ nav }) {
  return (
    <div style={{ padding:'28px 28px 60px', fontFamily:"system-ui,-apple-system,'Segoe UI',sans-serif" }}>
      ...
      {/* Profile card */}
      <div style={{ background:'white', border:'0.5px solid #e8edf2', borderRadius:8, padding:'28px 32px', marginBottom:14, borderLeft:'3px solid #1a56db' }}>
        <div style={{ display:'grid', gridTemplateColumns:'auto 1fr', gap:24, alignItems:'start' }}>
          ...
          <div>
            ...
            <div style={{ display:'flex', gap:8 }}>
              <a href="mailto:mathimcafee@gmail.com" style={{ display:'inline-flex', alignItems:'center', gap:6, background:'#1a56db', color:'white', textDecoration:'none', borderRadius:6, padding:'7px 13px', fontSize:12, fontWeight:500 }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            {CERTS.map(c => (
              <div key={c} style={{ display:'flex', alignItems:'flex-start', gap:10, fontSize:12, color:'#525252' }}>
                <Award size={13} color="#1a56db" style={{ flexShrink:0, marginTop:1 }}/>
                {c}
              </div>
            ))}
          </div>
        ...
        <div style={{ background:'white', border:'0.5px solid #e8edf2', borderRadius:8, padding:'20px 22px' }}>
          ...
          <div style={{ display:'flex', flexWrap:'wrap', gap:6 }}>
            {SKILLS.map(s => (
              <span key={s} style={{ fontSize:11, fontWeight:500, color:'#525252', background:'#fafafa', border:'0.5px solid #e8edf2', borderRadius:4, padding:'4px 9px' }}>{s}</span>
            ))}
          </div>
        </div>
        ...
        <div style={{ background:'white', border:'0.5px solid #e8edf2', borderRadius:8, padding:'20px 22px', gridColumn:'span 2' }}>
          ...
          <p style={{ fontSize:12, color:'#525252', lineHeight:1.8, margin:0 }}>
