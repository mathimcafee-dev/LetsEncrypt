...
const DNS_GUIDES = {
  godaddy:{label:'GoDaddy',steps:['Login to DNS Management','Add New Record → Type: TXT','Name: _acme-challenge, Value: (paste below), TTL: 600','Save']},
...
export default function Generate({ user, nav }) {
  ...
  const download = (content, filename) => { const a=document.createElement('a'); a.href=URL.createObjectURL(new Blob([content],{type:'text/plain'})); a.download=filename; a.click() }
  ...
  const call = async (action) => {
    ...
    try { return JSON.parse(text) } catch(e) { return { error: text || 'Server error' } }
  }
  ...
  const finalize = async () => {
    ...
    if (!result) { setError('Not ready yet. Click Issue again.'); return }
    setCertResult(result); setStep(3)
  }

  const resetAll = () => { setStep(0); setDomain(''); setChallenge(null); setCertResult(null); setError('') }

  return (
    <div className="container" style={{padding:'40px 24px 80px'}}>
      <div style={{marginBottom:40}}>
        <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:12}}>
          <div style={{width:44,height:44,background:'linear-gradient(135deg,#38bdf8,#0ea5e9)',borderRadius:12,display:'flex',alignItems:'center',justifyContent:'center',fontSize:20}}>🔒</div>
          <div>
            <h1 style={{fontSize:28,fontWeight:800,letterSpacing:'-0.5px'}}>Generate Free SSL</h1>
            <p style={{color:'var(--text3)',fontSize:14}}>Powered by Let's Encrypt · 90-day validity</p>
          </div>
          <label style={{marginLeft:'auto',display:'flex',alignItems:'center',gap:8,fontSize:12,color:'var(--text3)',cursor:'pointer'}}>
            <input type="checkbox" checked={staging} onChange={e=>setStaging(e.target.checked)} style={{width:'auto'}}/>Staging (test mode)
          </label>
        </div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:24}}>
          <div className="card">
            ...
            <div style={{marginBottom:16}}>
              ...
              <input placeholder="example.com or *.example.com" value={domain} onChange={e=>setDomain(e.target.value.replace(/^https?:\/\//,'').replace(/\/.*/,''))} onKeyDown={e=>e.key==='Enter'&&startOrder()} autoFocus/>
              <p style={{fontSize:11,color:'var(--text3)',marginTop:6}}>Wildcards (*.example.com) supported</p>
            </div>
            <div style={{background:'var(--surface2)',border:'1px solid var(--border)',borderRadius:'var(--radius-sm)',padding:'14px 16px',marginBottom:16}}>
              {[['🌐','Access to your DNS settings'],['⏱','2–5 minutes for DNS propagation'],['🔑','Private key stays in your browser']].map(([icon,text])=>(
                <div key={text} style={{display:'flex',gap:10,fontSize:12,color:'var(--text3)',marginBottom:6}}><span>{icon}</span><span>{text}</span></div>
              ))}
            </div>
            ...
            <button className="btn btn-primary btn-lg" onClick={startOrder} disabled={loading||!domain.trim()} style={{width:'100%',justifyContent:'center'}}>
              {loading?<><span className="spinner"/> Preparing...</>:'🔒 Generate Free SSL →'}
            </button>
            {!user&&<p style={{fontSize:12,color:'var(--text3)',textAlign:'center',marginTop:12}}>You need to <button onClick={()=>nav('auth')} style={{background:'none',border:'none',color:'var(--accent)',cursor:'pointer',fontWeight:600}}>sign in</button> to generate certificates</p>}
          </div>
          <div className="card">
            <h2 style={{fontWeight:700,fontSize:18,marginBottom:20}}>About Free SSL</h2>
            {[["🏛","Let's Encrypt","The world's largest free CA. Trusted by all browsers."],['📅','90-day Validity','Free forever. Renew anytime.'],['🔒','DNS-01 Validation','Proves domain ownership. Supports wildcards.'],['✅','Wildcard Support','*.example.com covers all subdomains.'],['📦','All Formats','Download cert, private key, full chain.']].map(([icon,title,desc])=>(
              <div key={title} style={{display:'flex',gap:14,marginBottom:16}}>
                <span style={{fontSize:20,flexShrink:0}}>{icon}</span>
                <div><p style={{fontWeight:700,fontSize:14,marginBottom:3}}>{title}</p><p style={{fontSize:12,color:'var(--text3)',lineHeight:1.6}}>{desc}</p></div>
              </div>
            ))}
          </div>
        </div>
            ...
            <div style={{background:'rgba(56,189,248,0.05)',border:'1px solid rgba(56,189,248,0.15)',borderRadius:'var(--radius-sm)',padding:16,marginBottom:16}}>
              ...
              {[['Type','TXT'],['Name / Host','_acme-challenge'],['Value',challenge.txtValue],['TTL','300']].map(([l,v])=>(
                <div key={l} style={{display:'flex',alignItems:'center',gap:10,background:'var(--surface2)',borderRadius:8,padding:'10px 12px',border:'1px solid var(--border)',marginBottom:8}}>
                  ...
                  <button onClick={()=>copy(v,l)} style={{background:'none',border:'none',cursor:'pointer',color:copied===l?'var(--green)':'var(--text3)',padding:4,fontSize:14}}>{copied===l?'✓':'📋'}</button>
                </div>
              ))}
            </div>
      ...
      {step===2&&(
        <div className="card" style={{maxWidth:560}}>
          ...
          <p style={{color:'var(--text2)',fontSize:14,lineHeight:1.7,marginBottom:24}}>Click below to generate your certificate. This will generate a 2048-bit RSA key pair, submit a CSR to Let's Encrypt, and return your signed certificate files.</p>
          {polling&&<div className="alert alert-info" style={{marginBottom:16}}><span className="spinner" style={{marginRight:8,borderTopColor:'var(--accent)'}}/> Issuing certificate... up to 30 seconds</div>}
          {error&&<div className="alert alert-error" style={{marginBottom:16}}>{error}</div>}
          <div style={{display:'flex',gap:12}}>
      ...
      {step===3&&certResult&&(
        <div>
          <div className="alert alert-success" style={{marginBottom:24,fontSize:15,fontWeight:600}}>🎉 SSL Certificate issued for <strong>{certResult.domain}</strong>!</div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20,marginBottom:20}}>
            <div className="card">
              ...
              <div style={{display:'flex',gap:8}}>
                <button className="btn btn-primary btn-sm" onClick={()=>download(certResult.cert,`${certResult.domain}-cert.pem`)}>⬇ Download</button>
                <button className="btn btn-secondary btn-sm" onClick={()=>copy(certResult.cert,'cert')}>{copied==='cert'?'✓ Copied':'📋 Copy'}</button>
              </div>
            </div>
            <div className="card">
              ...
              <div style={{display:'flex',gap:8}}>
                <button className="btn btn-primary btn-sm" onClick={()=>download(certResult.privateKey,`${certResult.domain}-key.pem`)}>⬇ Download</button>
                <button className="btn btn-secondary btn-sm" onClick={()=>copy(certResult.privateKey,'key')}>{copied==='key'?'✓ Copied':'📋 Copy'}</button>
              </div>
            </div>
          </div>
          {certResult.fullchain&&(
            <div className="card" style={{marginBottom:20}}>
              ...
              <button className="btn btn-primary btn-sm" onClick={()=>download(certResult.fullchain,`${certResult.domain}-fullchain.pem`)}>⬇ Download fullchain.pem</button>
            </div>
          )}
          <div style={{display:'flex',gap:12,flexWrap:'wrap'}}>
            <button className="btn btn-primary" onClick={()=>{download(certResult.cert,`${certResult.domain}-cert.pem`);setTimeout(()=>download(certResult.privateKey,`${certResult.domain}-key.pem`),300);setTimeout(()=>download(certResult.fullchain,`${certResult.domain}-fullchain.pem`),600)}}>⬇ Download All Files</button>
            <button className="btn btn-secondary" onClick={()=>nav('dashboard')}>🛡 Go to Dashboard</button>
