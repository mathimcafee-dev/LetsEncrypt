...
function GradeBadge({ grade, size = 'sm' }) {
  ...
  const s = GRADE_STYLE[grade] || { bg:'#f3f4f6', color:'#374151' }
  return (
    <span style={{ fontSize: size==='sm'?9:11, fontWeight:700, padding: size==='sm'?'1px 5px':'2px 7px',
...
function RiskBadge({ level }) {
  ...
  const s = level==='critical'?{bg:'#fee2e2',color:'#991b1b'}:level==='high'?{bg:'#ffedd5',color:'#9a3412'}:{bg:'#fef9c3',color:'#854d0e'}
  return (
    <span style={{ fontSize:9, fontWeight:600, padding:'1px 5px', borderRadius:3, background:s.bg, color:s.color }}>
      {level} risk
    </span>
  )
}
...
function PostureCard({ certs, nav }) {
  const graded = certs.filter(c => c.tls_grade)
  if (graded.length === 0) return null
  const counts = { A:0, B:0, C:0, D:0, F:0 }
  let topGrade = 'F'
  for (const c of graded) {
    if (c.tls_grade && counts[c.tls_grade] !== undefined) counts[c.tls_grade]++
  }
  const order = ['A','B','C','D','F']
  for (const g of order) { if (counts[g] > 0) { topGrade = g; break } }
  const gs = GRADE_STYLE[topGrade] || { bg:'#f3f4f6', color:'#374151' }
  const risk = certs.filter(c => c.risk_level === 'critical' || c.risk_level === 'high').length

  return (
    <div style={{ background:'white', border:'0.5px solid #e5e7eb', borderRadius:10, padding:'14px 16px',
      borderTop:'2px solid #10b981', marginBottom:14 }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
          <Shield size={14} color="#10b981"/>
          <span style={{ fontSize:11, fontWeight:700, color:'#374151', textTransform:'uppercase', letterSpacing:'0.4px' }}>TLS Posture</span>
        </div>
        <button onClick={() => nav('/audit-log')} style={{ fontSize:11, color:'#10b981', background:'none', border:'none', cursor:'pointer', fontWeight:500 }}>
          View audit log →
        </button>
      </div>
      <div style={{ display:'flex', alignItems:'center', gap:12 }}>
        <div style={{ width:44, height:44, borderRadius:8, background:gs.bg, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
          <span style={{ fontSize:22, fontWeight:800, color:gs.color }}>{topGrade}</span>
        </div>
        <div style={{ flex:1 }}>
          <div style={{ display:'flex', gap:6, flexWrap:'wrap', marginBottom:4 }}>
            {order.map(g => counts[g] > 0 && (
              <span key={g} style={{ fontSize:11, padding:'1px 7px', borderRadius:4, background:GRADE_STYLE[g].bg, color:GRADE_STYLE[g].color, fontWeight:500 }}>
                {counts[g]} {g}
              </span>
            ))}
          </div>
          <div style={{ fontSize:11, color:'#6b7280' }}>
            {graded.length} of {certs.length} certs scanned
            {risk > 0 && <span style={{ color:'#ef4444', fontWeight:500, marginLeft:6 }}>· {risk} at risk</span>}
          </div>
        </div>
        <button onClick={() => nav('/renewal-rules')} style={{ fontSize:11, color:'#374151', background:'#f9fafb', border:'1px solid #e5e7eb', borderRadius:6, padding:'5px 10px', cursor:'pointer', fontWeight:500, whiteSpace:'nowrap' }}>
          Renewal rules
        </button>
      </div>
    </div>
  ...
  return (
    <div style={{ marginBottom:20 }}>
      ...
      <div style={{ position:'relative', height:28, borderRadius:6, overflow:'hidden', background:'#1a1a2e', border:'1px solid rgba(255,255,255,0.07)' }}>
        ...
        <div style={{ position:'absolute', top:0, bottom:0, left: pctNow+'%', width:2, background:'white',
          boxShadow:'0 0 8px 2px rgba(255,255,255,0.6)', zIndex:3 }}/>
      ...
      <style>{`@keyframes stripe-move { from { background-position: 0 0; } to { background-position: 40px 0; } }`}</style>
    </div>
  )
}
...
function CertDetail({ cert, onClose, onDelete, onInstall, nav, onRefresh }) {
  ...
  return (
    <div className="v2-detail" style={{ position:'sticky', top:20 }}>
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:14, gap:12 }}>
        <div style={{ minWidth:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:4, flexWrap:'wrap' }}>
            ...
            <GradeBadge grade={cert.tls_grade} size="md"/>
          </div>
      ...
      {/* TLS posture detail */}
      {cert.tls_grade && (
        <div style={{ background:'#f8fafc', border:'0.5px solid #e2e8f0', borderRadius:8, padding:'10px 12px', marginBottom:14 }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:6 }}>
            <span style={{ fontSize:11, fontWeight:600, color:'#374151' }}>TLS Posture</span>
            <GradeBadge grade={cert.tls_grade} size="md"/>
          </div>
          <div style={{ fontSize:11, color:'#6b7280' }}>Score: {cert.tls_score || '—'}/100 · Last checked: {cert.tls_checked_at ? fmtDate(cert.tls_checked_at) : 'never'}</div>
          {cert.tls_details && (
            <div style={{ display:'flex', flexWrap:'wrap', gap:4, marginTop:6 }}>
              {Object.entries(cert.tls_details).map(([k, v]) => (
                <span key={k} style={{ fontSize:10, padding:'1px 6px', borderRadius:3, background: v.pass?'#d1fae5':'#fee2e2', color: v.pass?'#065f46':'#991b1b' }}>
                  {v.pass?'✓':'✗'} {k.replace(/_/g,' ')}
                </span>
              ))}
            </div>
          )}
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:8, marginBottom:16 }}>
        {[
          ...
          cert.tls_grade ? { label:'TLS grade', value: cert.tls_grade } : null,
          cert.key_algorithm ? { label:'Key algorithm', value: cert.key_algorithm } : null,
          cert.key_size_bits ? { label:'Key size', value: cert.key_size_bits + ' bits' } : null,
    ...
    <div className={'v2-list-row status-'+s.dot+(selected?' selected':'')} onClick={onClick}>
      ...
      <div className="v2-row-body">
        <div className="v2-row-title-line">
          ...
          <StatusPill days={days} status={cert.status}/>
          <GradeBadge grade={cert.tls_grade}/>
          <RiskBadge level={cert.risk_level}/>
          {cert.private_key_pem && (
  ...
  return (
    <div style={{ background:'#f0f4f8', minHeight:'100vh' }}>
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'32px 28px 80px' }}>
        ...
        {/* P1: TLS Posture card */}
        <PostureCard certs={certs} nav={nav}/>
        ...
        <div style={{ marginTop:24 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10 }}>
            {[
              ...
              { icon:<RotateCcw size={15}/>,   label:'Renewal rules',      desc:'Automate renewals with smart rules',            action:() => nav('/renewal-rules') },
              { icon:<FileText size={15}/>,    label:'Audit log',          desc:'View all cert events and history',              action:() => nav('/audit-log') },
              ...
              <button key={label} onClick={action}
                ...
                <div style={{ color:'#10b981', marginBottom:8 }}>{icon}</div>
