...
function ProgressBar({ days, max = 365 }) {
  ...
  const color = days == null ? '#ffffff' : days < 0 ? '#f85149' : days < 14 ? '#f85149' : days < 30 ? '#ffffff' : '#ffffff'
  return (
    <div style={{ height:4, borderRadius:999, background:'#161b22', overflow:'hidden' }}>
...
function ImportedCertsSection({ certs, onDelete }) {
  ...
  return (
    <div style={{ marginTop:20, border:'1px solid rgba(20,184,166,0.2)', borderRadius:14, overflow:'hidden', background:'#161b22', boxShadow:'0 2px 8px rgba(0,0,0,0.04)' }}>
      <button onClick={() => setOpen(o => !o)}
        style={{ width:'100%', padding:'14px 18px', display:'flex', alignItems:'center', gap:10,
          background:'#161b22', border:'none', cursor:'pointer', fontFamily:'inherit', textAlign:'left' }}>
        <span style={{ fontSize:11, fontWeight:700, color:'#8b949e', textTransform:'uppercase', letterSpacing:'0.5px', flex:1 }}>
          CA Connector — Imported Certificates ({certs.length})
        </span>
        <span style={{ fontSize:10, color:'#484f58', marginRight:4 }}>Tracked only · not managed by SSLVault</span>
      ...
      {open && (
        <div>
          {certs.map(c => {
            ...
            const dotColor = isExpired ? '#f85149' : isExpiring ? '#ffffff' : '#3fb950'
            return (
              <div key={c.id} style={{ padding:'12px 18px', borderTop:'0.5px solid #e6fbf5',
                display:'flex', alignItems:'center', gap:12, background:'#161b22' }}>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontSize:13, fontWeight:600, color:'#ffffff', display:'flex', alignItems:'center', gap:8 }}>
                    ...
                    {isExpired && <span style={{ fontSize:10, fontWeight:600, padding:'1px 6px', borderRadius:4, background:'rgba(248,81,73,0.1)', color:'#f85149' }}>EXPIRED</span>}
                  </div>
                ...
                <button onClick={() => onDelete(c.id)}
                  style={{ background:'none', border:'none', cursor:'pointer', color:'#484f58', padding:4,
...
function DvPendingCard({ order, onRefresh }) {
  ...
  return (
    <div style={{ background:'rgba(239,68,68,0.08)', border:'1px solid #F2C4BC', borderRadius:10, padding:'14px 16px', marginBottom:12 }}>
      <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:10 }}>
        <Clock size={13} color="#f07059" style={{ flexShrink:0 }}/>
      ...
      <div style={{ background:'#161b22', borderRadius:8, padding:'10px 14px', marginBottom:10, fontSize:11, fontFamily:'monospace' }}>
        <div style={{ display:'grid', gridTemplateColumns:'60px 1fr auto', gap:'6px 10px', alignItems:'center' }}>
          <span style={{ color:'#8b949e', fontWeight:700, textTransform:'uppercase', fontSize:9 }}>Name</span>
          <span style={{ color:'#484f58', wordBreak:'break-all' }}>
            {hasDcv ? dcvName : <span style={{ color:'rgba(255,255,255,0.6)' }}>⟳ Fetching...</span>}
          </span>
          {hasDcv && <CopyBtn text={dcvName}/>}
          <span style={{ color:'#8b949e', fontWeight:700, textTransform:'uppercase', fontSize:9 }}>Type</span>
          <span style={{ color:'#34d399' }}>TXT</span><span/>
          <span style={{ color:'#8b949e', fontWeight:700, textTransform:'uppercase', fontSize:9 }}>Value</span>
          <span style={{ color:'#d29922', wordBreak:'break-all' }}>
            {hasDcv ? dcvValue : <span style={{ color:'rgba(255,255,255,0.6)' }}>⟳ Fetching...</span>}
          </span>
          {hasDcv && <CopyBtn text={dcvValue}/>}
          <span style={{ color:'#8b949e', fontWeight:700, textTransform:'uppercase', fontSize:9 }}>TTL</span>
          <span style={{ color:'#ffffff' }}>300</span><span/>
        </div>
      </div>
      <div style={{ display:'flex', gap:8, flexWrap:'wrap', alignItems:'center' }}>
        <button onClick={autoDns} disabled={addingDns || !hasDcv}
          style={{ display:'flex', alignItems:'center', gap:6, background:'#161b22', color:'#ffffff',
        ...
        <button onClick={checkStatus} disabled={checking}
          style={{ display:'flex', alignItems:'center', gap:6, background:'#161b22',
            border:'1px solid rgba(255,255,255,0.12)', color:'#ffffff', borderRadius:6,
        ...
        {!confirmDismiss ? (
          <button onClick={() => setConfirmDismiss(true)}
            style={{ display:'flex', alignItems:'center', gap:5, background:'#161b22',
              border:'1px solid #fca5a5', color:'#f85149', borderRadius:6,
          ...
          <div style={{ display:'flex', gap:6, alignItems:'center', marginLeft:'auto' }}>
            ...
            <button onClick={dismissOrder} disabled={dismissing}
              style={{ background:'#f85149', color:'#ffffff', border:'none', borderRadius:6,
            ...
            <button onClick={() => setConfirmDismiss(false)}
              style={{ background:'#161b22', color:'#ffffff', border:'1px solid rgba(255,255,255,0.12)', borderRadius:6,
                padding:'6px 12px', fontSize:11, fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>
              Cancel
            </button>
          </div>
        )}
        {msg && <span style={{ fontSize:11, color: msg.startsWith('✅') ? '#ffffff' : msg.startsWith('❌') ? '#f85149' : '#C45A4A' }}>{msg}</span>}
      </div>
    </div>
  )
}
...
function RingGauge({ days, total, expiresAt, issuedAt }) {
  ...
  const color     = isExpired ? '#f85149' : isWarn ? '#ffffff' : '#3fb950'
    ...
    <div style={{ flexShrink:0 }}>
      ...
      <div style={{ position:'relative', width:82, height:82 }}>
        <svg width="82" height="82" viewBox="0 0 82 82" style={{ transform:'rotate(-90deg)' }}>
          <circle cx="41" cy="41" r={r} fill="none" stroke="#e6fbf5" strokeWidth="8"/>
        ...
        <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column',
          ...
          <div style={{ fontSize:18, fontWeight:800, color: isExpired?'#f85149':color,
            fontFamily:'monospace', lineHeight:1, letterSpacing:'-0.5px',
            transition:'all 0.3s' }}>
            {isExpired ? '!' : timeLeft.d}
          </div>
          <div style={{ fontSize:8, color:'#484f58', marginTop:2, letterSpacing:'0.4px', textTransform:'uppercase' }}>
            days
          </div>
        </div>
      </div>
        ...
        <div style={{ display:'flex', justifyContent:'center', gap:2, marginTop:5 }}>
          {[
            ...
            <div key={label} style={{ display:'flex', flexDirection:'column', alignItems:'center',
              background:'#161b22', borderRadius:5, padding:'2px 5px', minWidth:28 }}>
              <span style={{ fontSize:11, fontWeight:800, fontFamily:'monospace',
                color: isWarn ? '#ffffff' : '#ffffff', lineHeight:1,
                transition:'all 0.3s' }}>{val}</span>
              <span style={{ fontSize:7, color:'rgba(255,255,255,0.12)', letterSpacing:'0.3px' }}>{label}</span>
            </div>
          ))}
        </div>
      )}
...
function ValidityTimeline({ issuedAt, expiresAt, orderPeriodMonths = 12 }) {
  ...
  const certColor = isExpired ? '#f85149' : isWarn ? '#ffffff' : '#3fb950'
  ...
  return (
    <div style={{ padding:'16px 18px', borderBottom:'1px solid rgba(255,255,255,0.08)' }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
        <span style={{ fontSize:10, fontWeight:700, color:'#484f58', textTransform:'uppercase', letterSpacing:'0.6px' }}>
          Validity timeline
        </span>
        <span style={{ fontSize:11, fontWeight:600,
          color: isExpired ? '#f85149' : isWarn ? '#ffffff' : '#3fb950' }}>
          {isExpired
            ? '⚠ Certificate expired'
            : isWarn
      ...
      <div style={{ position:'relative', height:20, borderRadius:6, overflow:'hidden',
        background:'#161b22', border:'1px solid rgba(63,185,80,0.2)' }}>
        ...
        <div style={{ position:'absolute', top:0, bottom:0, width:3,
          ...
          background:'#161b22', borderRadius:2, zIndex:5,
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(min(220px,100%),1fr))', marginTop:8 }}>
        <div>
          <div style={{ fontSize:10, fontWeight:600, color:'#8b949e', fontFamily:'monospace' }}>
            {fmtShort(issuedAt)}
          </div>
          <div style={{ fontSize:9, color:'#484f58', marginTop:2 }}>SSL valid from</div>
        </div>
        <div style={{ textAlign:'center' }}>
          <div style={{ fontSize:10, fontWeight:700,
            color: isWarn ? '#ffffff' : isExpired ? '#f85149' : '#ffffff',
            fontFamily:'monospace' }}>
            {fmtShort(expiresAt)}
          </div>
          <div style={{ fontSize:9, color:'#484f58', marginTop:2 }}>SSL valid till</div>
        </div>
        <div style={{ textAlign:'right' }}>
          <div style={{ fontSize:10, fontWeight:700, color:'#ffffff', fontFamily:'monospace' }}>
            {fmtShort(subEnd)}
          </div>
          <div style={{ fontSize:9, color:'#484f58', marginTop:2 }}>Subscription ends</div>
        </div>
      </div>
    </div>
  )
...
const CertHistory = forwardRef(function CertHistory({ cert, session }, ref) {
  ...
  const statusBadge = (s) => {
    const map = {
      issued:   { bg:'#dcfce7', color:'#3fb950', label:'Issued' },
      active:   { bg:'#dcfce7', color:'#3fb950', label:'Active' },
      installed:{ bg:'#dcfce7', color:'#3fb950', label:'Installed' },
      pending:  { bg:'#fef9c3', color:'#854d0e', label:'Pending DV' },
      pending_validation: { bg:'#fef9c3', color:'#854d0e', label:'Pending DV' },
      failed:   { bg:'rgba(239,68,68,0.08)', color:'#f85149', label:'Failed' },
    }
    const t = map[s] || { bg:'#ffffff', color:'#8b949e', label: s||'—' }
  ...
  const stepIcon = (status) => {
    if (status === 'done')    return <span style={{color:'#ffffff',fontWeight:700}}>✓</span>
    if (status === 'error')   return <span style={{color:'#f85149',fontWeight:700}}>✗</span>
    if (status === 'active')  return <span style={{display:'inline-block',width:12,height:12,borderRadius:'50%',border:'2px solid #0d9488',borderTopColor:'transparent',animation:'spin .7s linear infinite',verticalAlign:'middle'}}/>
              ...
              <div key={i} style={{display:'flex',alignItems:'flex-start',gap:10}}>
                ...
                <div>
                  <div style={{
                    ...
                    color: step.status==='done' ? '#ffffff' : step.status==='error' ? '#f85149' : step.status==='active' ? '#ffffff' : '#7A9E9B',
                  }}>
                    {step.label}
                  </div>
                  {step.detail && (
      ...
      <div style={{ display:'flex', borderBottom:'1px solid var(--v2-border)', marginBottom:14 }}>
        {[['reissues','Reissue History'], ['renewals','Renewals']].map(([k,l]) => (
          <button key={k} onClick={() => setTab(k)} style={{
            ...
            color: tab===k ? '#ffffff' : 'var(--v2-text-3)',
            borderBottom: tab===k ? '2px solid #0d9488' : '2px solid transparent',
            marginBottom:-1
          }}>
            {l}
            {k==='reissues' && reissues.length > 0 && (
              <span style={{ marginLeft:5, fontSize:10, fontWeight:700, padding:'1px 5px', borderRadius:10,
                background: tab===k ? 'rgba(255,255,255,0.1)' : 'transparent', color: tab===k ? '#ffffff' : 'rgba(255,255,255,0.5)' }}>
                {reissues.length}
              </span>
            )}
          ...
          {reissues.length === 0 ? (
            ...
            <div key={r.id} style={{ border:'1px solid var(--v2-border)', borderRadius:8, marginBottom:8, overflow:'hidden' }}>
              ...
              <div onClick={() => toggleExpand(r.id)}
                ...
                <div style={{ width:26, height:26, borderRadius:6, background:'#161b22',
                  display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                  <span style={{ fontSize:11, fontWeight:700, color:'#ffffff' }}>R{i+1}</span>
                </div>
                <div style={{ flex:1, minWidth:0 }}>
                ...
                <div style={{ borderTop:'1px solid var(--v2-border)', padding:'12px 14px', background:'var(--v2-surface)' }}>
                  ...
                  {r.cert_pem && (
                    <div>
                      ...
                      <div style={{ background:'#161b22', borderRadius:6, padding:'10px 12px', position:'relative' }}>
                        <pre style={{ fontSize:10, color:'#484f58', margin:0, overflow:'hidden', maxHeight:80, fontFamily:'monospace', whiteSpace:'pre-wrap', wordBreak:'break-all' }}>
                  ...
                  {(r.auto_install_status || r.auto_install_error) && (
                    <div style={{ marginTop:10, padding:'8px 10px', borderRadius:6,
                      background: r.auto_install_status==='success' ? '#111111' : 'rgba(248,81,73,0.1)',
                      border: '1px solid '+(r.auto_install_status==='success' ? '#A8E6DE' : '#fecaca') }}>
                      <div style={{ fontSize:11, fontWeight:600, color: r.auto_install_status==='success' ? '#3fb950' : '#f85149' }}>
                        Auto-install: {r.auto_install_status}
                      </div>
                      {r.auto_install_error && <div style={{ fontSize:11, color:'#f85149', marginTop:2 }}>{r.auto_install_error}</div>}
                    </div>
                  )}
                </div>
              )}
            ...
            <div key={r.id} style={{ padding:'10px 14px', borderRadius:8, marginBottom:8,
              ...
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6 }}>
                ...
                <span style={{ fontSize:10, fontWeight:600, padding:'2px 8px', borderRadius:4,
                  background: r.status==='active' ? 'rgba(63,185,80,0.1)' : '#111111',
                  color: r.status==='active' ? '#3fb950' : 'rgba(255,255,255,0.7)' }}>{r.status}</span>
              </div>
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:6 }}>
      <button onClick={run} disabled={scanning}
        style={{ display:'flex', alignItems:'center', gap:5, background:'#161b22', color:'#ffffff',
      ...
      {result && (
        <span style={{ fontSize:11, color: result.ok ? '#3fb950' : '#f85149',
...
// ── PQC Readiness row ─────────────────────────────────────────────────
const PQC_RISK_MAP = {
  ready:  { color:'#3fb950', bg:'#111111', border:'#A8E6DE', label:'PQC Ready',     icon:'✓' },
  ...
  high:   { color:'#f85149', bg:'#fef2f2', border:'#fecaca', label:'High risk',      icon:'✗' },
}
    ...
    <div style={{ border:'0.5px solid var(--v2-border)', borderRadius:10, overflow:'hidden', marginBottom:6 }}>
      <div onClick={()=>risk&&setOpen(o=>!o)}
        ...
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          <div style={{ width:34, height:34, borderRadius:8, background:'rgba(239,68,68,0.08)',
            display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
            <Shield size={16} color="#f07059"/>
          </div>
          <div>
          ...
          {!noPem && (
            <button onClick={runCheck} disabled={checking}
              style={{ display:'flex', alignItems:'center', gap:4, fontSize:11,
                color:'#ffffff', background:'rgba(239,68,68,0.08)', border:'0.5px solid #F2C4BC',
...
function TlsPostureRow({ cert, onRefresh }) {
  ...
  const gradeColor = { A:'#3fb950', B:'#65a30d', C:'#ffffff', D:'#ea580c', F:'#f85149' }
  const grade = cert.tls_grade
  const score = cert.tls_score || 0
  const color = gradeColor[grade] || 'rgba(255,255,255,0.7)'
  ...
  return (
    <div style={{ border:'0.5px solid var(--v2-border)', borderRadius:10, overflow:'hidden', marginBottom:6 }}>
      ...
      <div onClick={() => grade && setOpen(o => !o)}
        ...
        onMouseLeave={e => e.currentTarget.style.background='var(--v2-bg)'}>
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          <div style={{ width:34, height:34, borderRadius:8, background:'#161b22',
            display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
          ...
          <button onClick={runCheck} disabled={checking}
            style={{ display:'flex', alignItems:'center', gap:4, fontSize:11,
              color:'#ffffff', background:'#161b22', border:'1px solid rgba(63,185,80,0.2)',
              borderRadius:6, padding:'4px 10px', cursor:checking?'wait':'pointer',
              fontFamily:'inherit', fontWeight:500, flexShrink:0 }}>
            {checking
              ? <><RefreshCw size={11} style={{ animation:'spin .8s linear infinite' }}/> Checking…</>
          ...
          {Object.entries(result).map(([key, check]) => (
            <div key={key} style={{ display:'flex', alignItems:'flex-start', gap:10,
              ...
              <div style={{ width:18, height:18, borderRadius:'50%', flexShrink:0, marginTop:1,
                background: check.pass ? '#111111' : '#fef2f2',
                display:'flex', alignItems:'center', justifyContent:'center' }}>
                {check.pass
              ...
              <div style={{ flex:1 }}>
                <div style={{ fontSize:12, fontWeight:500,
                  color: check.pass ? 'var(--v2-text)' : '#f85149' }}>
...
function CertDetail({ cert, onClose, onDelete, onInstall, onCpanel, nav, onRefresh, session }) {
  ...
  const isExpired = (days ?? 0) < 0
  const isWarn    = (days ?? 0) >= 0 && (days ?? 0) < 30
  const statusColor = isExpired ? '#f85149' : isWarn ? '#ffffff' : '#3fb950'
  const statusBg    = isExpired ? '#fef2f2' : isWarn ? 'rgba(239,68,68,0.08)' : '#111111'
    ...
    return (
      <button onClick={() => { try { navigator.clipboard.writeText(text) } catch(e) {} setOk(true); setTimeout(() => setOk(false), 1800) }}
        style={{ background:'none', border:'0.5px solid var(--v2-border)', borderRadius:4, cursor:'pointer',
          padding:'2px 6px', fontSize:10, color: ok ? '#3fb950' : 'var(--v2-text-3)',
          fontFamily:'inherit', transition:'all 0.12s', display:'inline-flex', alignItems:'center', gap:3,
          ...(ok ? { borderColor:'#A8E6DE', background:'#161b22' } : {}) }}>
  ...
  return (
    <div style={{ background:'#161b22', border:'1px solid rgba(63,185,80,0.2)', borderRadius:14,
      overflow:'hidden', boxShadow:'0 4px 20px rgba(255,255,255,0.08)',
      position:'sticky', top:20, animation:'fadeSlideUp 0.3s ease both' }}>

      {/* ── Hero header ─── */}
      <div style={{ padding:'20px 20px 16px', background:'#161b22',
        borderBottom:'0.5px solid #e6fbf5' }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:6 }}>
              ...
              <button onClick={onClose}
                style={{ background:'#161b22', border:'1px solid rgba(63,185,80,0.2)', cursor:'pointer',
                  color:'#484f58', padding:'5px', borderRadius:7, display:'flex', flexShrink:0,
                  marginLeft:8, transition:'all .15s' }}
                onMouseEnter={e=>{e.currentTarget.style.background='rgba(239,68,68,0.08)';e.currentTarget.style.color='#f85149';e.currentTarget.style.borderColor='#fecaca'}}
                onMouseLeave={e=>{e.currentTarget.style.background='rgba(255,255,255,0.06)';e.currentTarget.style.color='rgba(255,255,255,0.6)';e.currentTarget.style.borderColor='rgba(255,255,255,0.2)'}}>
            ...
            <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
              ...
              <span style={{ fontSize:10, fontWeight:600, padding:'3px 10px', borderRadius:20,
                background:'#161b22', color:'#185FA5', border:'0.5px solid #B5D4F4' }}>
                RapidSSL
              </span>
              <span style={{ fontSize:10, fontWeight:600, padding:'3px 10px', borderRadius:20,
                background:'#161b22', color:'rgba(255,255,255,0.6)', border:'1px solid rgba(63,185,80,0.2)' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(140px,1fr))',
        borderBottom:'0.5px solid #e6fbf5', background:'#161b22' }}>
        {(() => {
          // ── Date logic ─────────────────────────────────────────────────
          //
          ...
          return [
            ...
            },
            {
              ...
              bottom: 'Auto-reissue',
              color:  reissueOverdue ? '#f85149' : '#ffffff',
              mono:   false,
              tip:    reissueDate
            ...
            },
            {
              ...
              bottom: 'Auto-renewal',
              color:  renewalOverdue ? '#f85149' : '#ffffff',
              mono:   false,
              tip:    renewalDate
            ...
            <div key={i} title={tip||''} style={{ padding:'12px 16px',
              borderRight: i < 2 ? '0.5px solid #e6fbf5' : 'none',
              ...
              <div style={{ display:'flex', alignItems:'center', gap:3, marginTop:4 }}>
                <span style={{ fontSize:10, color:'#484f58', textTransform:'uppercase', letterSpacing:'0.4px' }}>{bottom}</span>
                {tip && <span style={{ fontSize:11, color:'rgba(255,255,255,0.12)', lineHeight:1 }}>ⓘ</span>}
              </div>
            </div>
          ))
        })()}
      ...
      {/* ── Tabs ─── */}
      <div style={{ display:'flex', padding:'0 16px', borderBottom:'0.5px solid #e6fbf5', gap:0 }}>
        {['details','actions','files','history'].map(t => (
          <button key={t} onClick={() => setActiveTab(t)}
            style={{ fontSize:11, fontWeight:700, padding:'11px 14px',
              background:'none', border:'none', cursor:'pointer', fontFamily:'inherit',
              color: activeTab===t ? '#ffffff' : 'rgba(255,255,255,0.38)',
        ...
        <div style={{ padding:'14px 18px' }}>
          {refreshMsg && (
            <div style={{ fontSize:12, padding:'8px 10px', borderRadius:6, marginBottom:12,
              background: refreshMsg.includes('Error') ? '#fef2f2' : '#111111',
              color: refreshMsg.includes('Error') ? '#f85149' : '#3fb950',
              border:'0.5px solid '+(refreshMsg.includes('Error') ? '#fecaca' : '#A8E6DE') }}>
              {refreshMsg}
            </div>
          )}
          <ActionRow icon={RotateCcw} iconColor="#16a34a" iconBg="#ccfbf1"
            hoverBorder="#A8E6DE" hoverBg="#ccfbf1"
          ...
          {cert._daysToRenewal != null && cert._daysToRenewal <= 30 && (
            <ActionRow icon={RefreshCw} iconColor="#0d9488" iconBg="#ccfbf1"
              hoverBorder="#A8E6DE" hoverBg="#ccfbf1"
              title="Renew for another year" subtitle="New GGS order · new cert row"
              onClick={() => certHistoryRef.current?.doAction('renew')}
              disabled={!session}/>
          )}
          <ActionRow icon={Server} iconColor="#f07059" iconBg="#fde8e4"
            hoverBorder="#F2C4BC" hoverBg="#fde8e4"
            title="Install on server" subtitle="VPS agent or cPanel wizard"
            onClick={() => onInstall(cert)}/>
          <ActionRow icon={RefreshCw} iconColor="#f07059" iconBg="#fde8e4"
            hoverBorder="#F2C4BC" hoverBg="#fde8e4"
              ...
              {cancelConfirm && !cancelMsg && (
                <div style={{ display:'flex', alignItems:'center', gap:8, marginTop:4 }}>
                  ...
                  <button onClick={doCancel} disabled={cancelling}
                    style={{ fontSize:11, fontWeight:600, color:'#ffffff', background:'#DC2626',
          ...
          <div style={{ display:'flex', justifyContent:'flex-end', marginTop:4 }}>
            {!delConfirm
              ...
              : <button onClick={() => onDelete(cert.id)}
                  style={{ fontSize:11, color:'#ffffff', background:'#f85149', border:'none',
      ...
      {activeTab === 'files' && (
        <div style={{ padding:'14px 18px' }}>
          ...
          {cert.cert_pem && (
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between',
              ...
              <div style={{ display:'flex', alignItems:'center', gap:9 }}>
                <div style={{ width:30, height:30, borderRadius:7, background:'#161b22',
                  display:'flex', alignItems:'center', justifyContent:'center' }}>
          ...
          {/* private key */}
          {cert.tls_grade && (
            <span style={{ fontSize:9, fontWeight:700, color: cert.tls_grade==='A'?'#3fb950':cert.tls_grade==='B'?'#65a30d':cert.tls_grade==='C'?'#ffffff':'#f85149',
              background: cert.tls_grade==='A'?'#111111':cert.tls_grade==='B'?'#f7fee7':cert.tls_grade==='C'?'rgba(239,68,68,0.08)':'#fef2f2',
          ...
          {cert.pqc_risk && (
            <span title={cert.key_algorithm || 'PQC check'} style={{ fontSize:9, fontWeight:700,
              color: cert.pqc_risk==='ready'?'#3fb950':cert.pqc_risk==='low'?'#ffffff':cert.pqc_risk==='medium'?'#ffffff':'#f85149',
              background: cert.pqc_risk==='ready'?'#111111':cert.pqc_risk==='low'?'#111111':cert.pqc_risk==='medium'?'rgba(239,68,68,0.08)':'#fef2f2',
          ...
          {cert.private_key_pem && (
            <div style={{ border:'0.5px solid var(--v2-border)', borderRadius:8,
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between',
                padding:'10px 12px' }}>
                <div style={{ display:'flex', alignItems:'center', gap:9 }}>
                  <div style={{ width:30, height:30, borderRadius:7, background:'rgba(248,81,73,0.1)',
                    display:'flex', alignItems:'center', justifyContent:'center' }}>
                  ...
                  <div>
                    <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                      ...
                      <span style={{ fontSize:9, fontWeight:600, padding:'2px 6px', borderRadius:3,
                        background:'rgba(239,68,68,0.08)', color:'#f85149', letterSpacing:'0.3px' }}>SENSITIVE</span>
                    </div>
                    <div style={{ fontSize:10, color:'var(--v2-text-3)', marginTop:1 }}>
                      {keyCopied && keyTimer > 0
                        ? <span style={{ color:'#f85149' }}>Copied — clears in {keyTimer}s</span>
                ...
                <div style={{ display:'flex', gap:6, alignItems:'center' }}>
                  {keyCopied && keyTimer > 0 && (
                    <div style={{ height:3, width:80, background:'rgba(239,68,68,0.08)', borderRadius:3, overflow:'hidden' }}>
                      <div style={{ height:'100%', background:'#f85149', borderRadius:3,
...
function DomainGroup({ primary, versions, index, selected, onSelect }) {
  ...
  const dotColor = s.dot==='green'?'#ffffff':s.dot==='amber'?'#ffffff':s.dot==='red'?'#f85149':'#d4d4d4'
  ...
  return (
    <div style={{ background:'var(--v2-surface)',
      border:`0.5px solid ${selected===primary.id ? '#ffffff' : 'var(--v2-border)'}`,
      ...
      <div onClick={() => onSelect(primary.id)}
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:3, flexWrap:'wrap' }}>
            ...
            <span style={{ fontSize:10, fontWeight:600, padding:'2px 8px', borderRadius:20,
              background: s.dot==='green'?'#111111':s.dot==='amber'?'#FAEEDA':'#FCEBEB',
              color: s.dot==='green'?'#0F6E56':s.dot==='amber'?'#854F0B':'#A32D2D' }}>
              {days != null ? `${days}d left` : primary.status}
            </span>
            {primary.install_method && (
              <span style={{ fontSize:10, padding:'2px 8px', borderRadius:20,
                background: primary.install_method==='cpanel'?'#000000':'rgba(255,255,255,0.7)',
                color: primary.install_method==='cpanel'?'#8B6914':'#ffffff',
                border:`0.5px solid ${primary.install_method==='cpanel'?'#D4C9B0':'rgba(255,255,255,0.7)'}`,
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
          <div style={{ textAlign:'right' }}>
            ...
            <div style={{ fontSize:12, fontWeight:600, color:'#ffffff' }}>{fmtDate(primary.expires_at)}</div>
          </div>
      ...
      {expanded && (
        <div style={{ background:'#161b22', padding:'10px 14px 14px' }}>
          ...
          {subscriptions.map((subVersions, si) => {
            ...
            // Active original → dark teal #0d9488
            // Renewal         → mint teal #0d9488
            // Older           → muted cream
            const borderColor = isActiveSub && !isRenewal ? '#ffffff'
              : isRenewal ? '#ffffff'
              : '#C5BDAD'

            const headerBg    = isActiveSub && !isRenewal ? '#111111'
              : isRenewal ? '#111111'
              : '#000000'
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:7, marginBottom:5 }}>
                      <span style={{ fontSize:10, fontWeight:600, textTransform:'uppercase',
                        letterSpacing:'0.4px', color: isRenewal?'#ffffff': isActiveSub?'#ffffff':'#6B5A3E' }}>
                        Subscription {subNumberMap[subNewest.ggs_order_id || subNewest.id] || (si + 1)} · {subLabel}
                      </span>
                      {isActiveSub && (
                        <span style={{ fontSize:10, fontWeight:600, padding:'2px 8px', borderRadius:20,
                          background:isRenewal?'#ffffff':'#ffffff', color:'#ffffff' }}>
                          {isRenewal ? '↻ Active renewal' : 'Active'}
                        </span>
                      )}
                      {ggsOrder && (
                        <span style={{ fontSize:10, padding:'2px 7px', borderRadius:20,
                          background:'#161b22', color:'#ffffff', border:`0.5px solid ${headerBorder}`,
                          fontFamily:'monospace' }}>
                          GGS #{ggsOrder}
                        </span>
                      )}
                        ...
                        <div style={{ flex:1, height:6, background:'rgba(255,255,255,0.6)', borderRadius:6,
                          ...
                          {isActiveSub && subPct > 0 && subPct < 100 && (
                            <div style={{ position:'absolute', left:`${subPct}%`, top:-3,
                              width:2, height:12, background:'#161b22', borderRadius:1 }}/>
                          )}
                          {/* Auto-reissue dot marker */}
                          {isActiveSub && (
                            <div style={{ position:'absolute', right:'8%', top:-2,
                              width:7, height:7, borderRadius:'50%', background:'#161b22',
                    ...
                    {isActiveSub && subStart && (
                      <div style={{ display:'flex', gap:14, marginTop:4 }}>
                        <div style={{ display:'flex', alignItems:'center', gap:4 }}>
                          <div style={{ width:6, height:6, borderRadius:'50%', background:'#161b22' }}/>
                  ...
                  return (
                    <div key={v.id}
                      ...
                      style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 14px',
                        ...
                        background: isLive ? '#111111' : selected===v.id ? '#111111' : 'var(--v2-surface)',
                        cursor:'pointer',
                      ...
                      <div style={{ width:28, height:28, borderRadius:8, flexShrink:0,
                        display:'flex', alignItems:'center', justifyContent:'center',
                        background: isLive ? '#ffffff' : wasInstalled ? '#ffffff' : neverInstalled && vi===0 ? '#111111' : '#ffffff',
                        border: isLive ? 'none' : wasInstalled ? '1.5px solid #E5E7EB' : vi===0 ? '1.5px dashed #0d9488' : '1.5px solid #E5E7EB',
                        ...
                        {!isLive && !wasInstalled && vi===0 && <Clock size={11} color="#0d9488"/>}
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:2 }}>
                          <span style={{ fontSize:10, fontWeight:600, padding:'1px 7px', borderRadius:20,
                            background: isLive ? '#ffffff' : wasInstalled ? '#ffffff' : vi===0 ? '#111111' : '#ffffff',
                            color: isLive ? '#000000' : wasInstalled ? '#6B7280' : vi===0 ? '#ffffff' : '#9CA3AF',
                      ...
                      {(() => {
                        ...
                        return (
                          <span style={{ fontSize:10, fontWeight:600, padding:'2px 8px', borderRadius:20,
                            background: d==null?'var(--v2-bg)':d>30?'#111111':d>=0?'#FAEEDA':'#FCEBEB',
...
function CertRow({ cert, selected, onClick, index }) {
  ...
  const dotColor = s.dot==='green'?'#3fb950':s.dot==='amber'?'#ffffff':s.dot==='red'?'#f85149':'#d4d4d4'
  const iconBg   = s.dot==='green'?'#ffffff':s.dot==='amber'?'#ffffff':s.dot==='red'?'#f85149':'rgba(255,255,255,0.7)'
  return (
    <div onClick={onClick}
      ...
      <div style={{ position:'relative', flexShrink:0 }}>
        <div style={{ width:38, height:38, borderRadius:10, background:iconBg,
          display:'flex', alignItems:'center', justifyContent:'center',
          fontSize:12, fontWeight:700, color:'#ffffff', letterSpacing:'0.5px' }}>
          {initials}
        </div>
      ...
      <div style={{ flex:1, minWidth:0 }}>
        <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:4 }}>
          ...
          {cert._healthGrade && (() => {
            ...
            const gc = g==='A+'||g==='A' ? '#3fb950' : g==='B' ? '#ffffff' : g==='C' ? '#ca8a04' : g==='D' ? '#ffffff' : '#f85149'
            const gb = g==='A+'||g==='A' ? '#111111' : g==='B' ? '#111111' : g==='C' ? '#fefce8' : g==='D' ? 'rgba(239,68,68,0.08)' : '#fef2f2'
            return (
          ...
          {cert.tls_grade && (
            <span style={{ fontSize:9, fontWeight:700, padding:'2px 5px', borderRadius:4,
              color: cert.tls_grade==='A'?'#3fb950':cert.tls_grade==='B'?'#65a30d':cert.tls_grade==='C'?'#ffffff':'#f85149',
              background: cert.tls_grade==='A'?'#111111':cert.tls_grade==='B'?'#f7fee7':cert.tls_grade==='C'?'rgba(239,68,68,0.08)':'#fef2f2',
        ...
        <div style={{ display:'flex', gap:8, alignItems:'center', fontSize:11, color:'#484f58' }}>
          ...
          return (
            <div style={{ marginTop:5, display:'flex', alignItems:'center', gap:5 }}>
              ...
              <span style={{
                ...
                background: method==='agent' ? '#111111' : '#000000',
                color: method==='agent' ? '#ffffff' : '#8B5E3C',
              ...
              <span style={{
                display:'inline-flex', alignItems:'center', gap:3,
                fontSize:10, fontWeight:600, padding:'2px 7px', borderRadius:20,
                background: isVerified ? '#111111' : isAlert ? 'rgba(239,68,68,0.08)' : '#000000',
                color: isVerified ? '#ffffff' : isAlert ? '#C45A4A' : '#7A9E9B',
      ...
      <ChevronRight size={14} color="rgba(255,255,255,0.12)" style={{ flexShrink:0 }}/>
    </div>
  )
}
...
function LoggedInDashboard({ user, nav, onIssue }) {
  ...
  return (
    <div style={{ background:'#0d1117', minHeight:'100vh', fontFamily:"'Inter',system-ui,sans-serif", color:'#e6edf3' }}>
      <div style={{ maxWidth:1400, margin:'0 auto', padding:'24px 28px 80px' }}>
        ...
        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:28, gap:16, flexWrap:'wrap' }}>
          <div>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:4 }}>
              <h1 style={{ fontSize:18, fontWeight:600, color:'#e6edf3', letterSpacing:'-0.2px', margin:0 }}>Certificate Dashboard</h1>
            ...
            <p style={{ fontSize:12, color:'#6e7681', margin:0 }}>{user.email} · {domainGroups.length} domain{domainGroups.length!==1?'s':''} · {total} certificate{total!==1?'s':''}</p>
          </div>
          <button
            onClick={async () => {
              const { data: { session: s } } = await supabase.auth.getSession()
            ...
            style={{ display:'flex', alignItems:'center', gap:6, background:'transparent',
              border:'1px solid rgba(255,255,255,0.12)', borderRadius:6, padding:'7px 14px',
              fontSize:12, fontWeight:500, color:'#8b949e', cursor:'pointer', fontFamily:'inherit',
              transition:'all .15s' }}
            onMouseEnter={e=>{e.currentTarget.style.borderColor='rgba(255,255,255,0.25)';e.currentTarget.style.color='#e6edf3'}}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:1, marginBottom:24,
          ...
          {[
            { label:'Total',       value:total,    color:'#388bfd', note:'certificates' },
            { label:'Healthy',     value:healthy,  color:'#3fb950', note:healthy>0?'All valid':'None' },
            { label:'Expiring',    value:expiring, color:expiring>0?'#d29922':'#484f58', note:expiring>0?'Within 30 days':'None expiring' },
            { label:'Pending DCV', value:orders.filter(o=>o.status==='dv_pending').length,
              color:orders.filter(o=>o.status==='dv_pending').length>0?'#f85149':'#484f58',
              note:orders.filter(o=>o.status==='dv_pending').length>0?'Awaiting DNS':'None pending' },
            { label:'Expired',     value:expired,  color:expired>0?'#f85149':'#484f58', note:expired>0?'Renew now':'None expired' },
          ].map((s,i) => (
            <div key={s.label} style={{ background:'#161b22', padding:'20px 24px' }}>
              <div style={{ fontSize:11, fontWeight:500, color:'#6e7681', marginBottom:12, letterSpacing:'0.02em' }}>{s.label}</div>
              <div style={{ fontSize:32, fontWeight:700, color:s.value>0?s.color:'#484f58', letterSpacing:'-1px', lineHeight:1, marginBottom:8 }}>{s.value}</div>
              <div style={{ fontSize:11, color:'#484f58' }}>{s.note}</div>
            </div>
          ))}
        </div>
        ...
        {/* ── Alerts ── */}
        {expired > 0 && (
          <div style={{ background:'rgba(248,81,73,0.08)', border:'1px solid rgba(248,81,73,0.2)', borderRadius:6, padding:'10px 16px', marginBottom:16, display:'flex', alignItems:'center', gap:10 }}>
            <AlertTriangle size={14} color="#f85149" style={{ flexShrink:0 }}/>
            <span style={{ fontSize:12, color:'#f85149' }}><strong>{expired} expired certificate{expired!==1?'s':''}</strong> — renew immediately to avoid service disruption.</span>
          </div>
        )}
        {expiring > 0 && expired === 0 && (
          <div style={{ background:'rgba(210,153,34,0.08)', border:'1px solid rgba(210,153,34,0.2)', borderRadius:6, padding:'10px 16px', marginBottom:16, display:'flex', alignItems:'center', gap:10 }}>
            <AlertTriangle size={14} color="#d29922" style={{ flexShrink:0 }}/>
            <span style={{ fontSize:12, color:'#d29922' }}><strong>{expiring} certificate{expiring!==1?'s':''} expiring within 30 days</strong> — auto-renew will handle this if enabled.</span>
          </div>
        )}

        {orders.length > 0 && (
          <div style={{ marginBottom:16 }}>
            <div style={{ fontSize:11, fontWeight:600, color:'#6e7681', textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:8 }}>DNS Validation Pending ({orders.length})</div>
            {orders.map(o => <DvPendingCard key={o.id} order={o} onRefresh={load}/>)}
          </div>
        )}

        {/* ── Main cert table + detail panel ── */}
        <div style={{ display:'grid', gridTemplateColumns:(!isMobile&&selectedCert)?'minmax(0,1fr) clamp(320px,36vw,420px)':'1fr', gap:16, alignItems:'start' }}>
          <div style={{ background:'#161b22', border:'1px solid rgba(255,255,255,0.08)', borderRadius:8, overflow:'hidden' }}>

            {/* Table toolbar */}
            <div style={{ padding:'12px 16px', borderBottom:'1px solid rgba(255,255,255,0.08)', display:'flex', alignItems:'center', gap:8, flexWrap:'wrap', background:'#161b22' }}>
              <div style={{ display:'flex', gap:0 }}>
                {[
                  ...
                  <button key={f.key} onClick={() => setFilter(f.key)}
                    style={{ padding:'5px 12px', fontSize:12, fontWeight:500,
                      cursor:'pointer', fontFamily:'inherit',
                      border:'none', borderBottom: filter===f.key ? '2px solid #388bfd' : '2px solid transparent',
                      background:'transparent', transition:'all .12s',
                      color: filter===f.key ? '#e6edf3' : '#6e7681',
                      marginRight:2 }}>
                    {f.label}
                    {f.count > 0 && <span style={{ marginLeft:5, fontSize:10, fontWeight:600,
                      padding:'1px 5px', borderRadius:10, background:'rgba(255,255,255,0.07)',
                      color:'#6e7681' }}>{f.count}</span>}
                  </button>
                ))}
              </div>
              <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center' }}>
                <div style={{ position:'relative' }}>
                  <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search domains..."
                    style={{ background:'#0d1117', border:'1px solid rgba(255,255,255,0.1)', borderRadius:6,
                      color:'#e6edf3', fontSize:12, padding:'6px 10px 6px 30px', width:200, outline:'none',
                      fontFamily:'inherit', transition:'border-color .15s' }}
                    onFocus={e=>e.target.style.borderColor='#388bfd'}
                    onBlur={e=>e.target.style.borderColor='rgba(255,255,255,0.1)'}/>
                  <Globe size={12} style={{ position:'absolute', left:10, top:'50%', transform:'translateY(-50%)', color:'#484f58', pointerEvents:'none' }}/>
                </div>
                <ScanPqcButton onDone={load}/>
                <button onClick={() => onIssue ? onIssue() : nav('/buy')}
                  style={{ display:'flex', alignItems:'center', gap:5, background:'#388bfd', color:'#fff',
                  ...
                  <Plus size={13}/> New SSL
                </button>
              </div>
            </div>

            {/* Column headers */}
            <div style={{ display:'grid', gridTemplateColumns:'32px 1fr 80px 90px 90px 100px 28px',
              padding:'8px 16px', borderBottom:'1px solid rgba(255,255,255,0.06)',
              background:'#0d1117' }}>
              {['#','Domain','Days left','Grade','Auto-renew','Expires',''].map((h,i) => (
                <div key={i} style={{ fontSize:10, fontWeight:600, color:'#484f58', textTransform:'uppercase', letterSpacing:'0.05em' }}>{h}</div>
              ))}
            </div>
            ...
            {filteredGroups.length === 0 ? (
              <div style={{ padding:'48px 16px', textAlign:'center' }}>
                ...
                {total === 0 && (
                  <button onClick={() => onIssue ? onIssue() : nav('/buy')}
                    style={{ background:'#388bfd', color:'#fff', border:'none', borderRadius:6,
              ...
              const progress = cert.issued_at && cert.expires_at
                ? Math.max(0, Math.min(100, ((Date.now() - new Date(cert.issued_at)) / (new Date(cert.expires_at) - new Date(cert.issued_at))) * 100))
                : 0
              ...
              return (
                <div key={group.domain} onClick={() => setSelected(isSelected ? null : cert.id)}
                  ...
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'center' }}>
                    <span style={{ fontSize:11, color:'#484f58', fontWeight:500 }}>{gi+1}</span>
                  </div>
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:3 }}>
                      ...
                      <div style={{ minWidth:0 }}>
                        ...
                        <div style={{ display:'flex', alignItems:'center', gap:6, marginTop:2 }}>
                          ...
                          <span style={{ fontSize:11, color:'#484f58' }}>{cert.ca_name || 'RapidSSL'}</span>
                        </div>
                      </div>
                    </div>
                    <div style={{ height:2, background:'rgba(255,255,255,0.06)', borderRadius:2, marginTop:6, marginLeft:36 }}>
                      <div style={{ height:'100%', width:`${progress}%`, background:statusColor, borderRadius:2, opacity:0.7 }}/>
                    </div>
                  </div>
                  ...
                  <div style={{ display:'flex', alignItems:'center' }}>
                    <span style={{ fontSize:12, fontWeight:700, color:gradeColor }}>{cert.tls_grade||'—'}</span>
                  </div>
                  <div style={{ display:'flex', alignItems:'center' }}>
                    <span style={{ fontSize:11, color:cert.auto_reissue_enabled?'#3fb950':'#484f58' }}>
                      {cert.auto_reissue_enabled?'✓ On':'Off'}
                    </span>
                  </div>
                  <div style={{ display:'flex', alignItems:'center' }}>
                    <span style={{ fontSize:11, color:'#6e7681' }}>
                      {cert.expires_at ? new Date(cert.expires_at).toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'}) : '—'}
                    </span>
                  </div>
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'center' }}>
          ...
          {!isMobile && selectedCert && (
            <CertDetailPanel
              ...
              onRefresh={load}
              onInstallAgent={cert => setAgentCert(cert)}
              onInstallCpanel={cert => setCpanelCert(cert)}
              nav={nav}
            />
          )}
        </div>
        ...
        <div style={{ marginTop:24 }}>
          <div style={{ fontSize:11, fontWeight:600, color:'#6e7681', textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:12 }}>Quick actions</div>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(180px,1fr))', gap:8 }}>
            {[
              { icon:Shield,   label:'Issue Certificate',  desc:'RapidSSL DV · ~5 min',         action:() => onIssue ? onIssue() : nav('/buy') },
              { icon:Download, label:'Install Guide',      desc:'Nginx, Apache, cPanel',         action:() => nav('/install') },
              { icon:Activity, label:'Integrations',       desc:'Cloudflare, Vercel, agent',     action:() => nav('/integrations') },
              { icon:Zap,      label:'Knowledge Base',     desc:'Guides, FAQs, troubleshooting', action:() => nav('/knowledge-base') },
            ].map(({ icon:Icon, label, desc, action }) => (
              <button key={label} onClick={action}
                style={{ background:'#161b22', border:'1px solid rgba(255,255,255,0.08)', borderRadius:8,
                ...
                onMouseEnter={e=>{e.currentTarget.style.borderColor='rgba(255,255,255,0.15)';e.currentTarget.style.background='#1c2128'}}
                onMouseLeave={e=>{e.currentTarget.style.borderColor='rgba(255,255,255,0.08)';e.currentTarget.style.background='#161b22'}}>
                <div style={{ width:32, height:32, borderRadius:6, background:'rgba(56,139,253,0.1)',
                  display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                  <Icon size={15} color="#388bfd" strokeWidth={1.8}/>
                </div>
                <div>
                  <div style={{ fontSize:12, fontWeight:600, color:'#e6edf3', marginBottom:2 }}>{label}</div>
                  <div style={{ fontSize:11, color:'#484f58', lineHeight:1.4 }}>{desc}</div>
                </div>
              </button>
            ))}
          </div>
        ...
        {recentEvents.length > 0 && (
          <div style={{ marginTop:24 }}>
            <div style={{ fontSize:11, fontWeight:600, color:'#6e7681', textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:12 }}>Recent activity</div>
            <div style={{ background:'#161b22', border:'1px solid rgba(255,255,255,0.08)', borderRadius:8, overflow:'hidden' }}>
              {recentEvents.map((ev, i) => {
                const dotColor = ev.event_type==='issued'||ev.event_type==='renewed' ? '#3fb950'
                               : ev.event_type==='revoked' ? '#f85149' : '#484f58'
                ...
                return (
                  <div key={ev.id} style={{ display:'flex', alignItems:'center', gap:12,
                    padding:'10px 16px', borderBottom: i < recentEvents.length-1 ? '1px solid rgba(255,255,255,0.06)' : 'none' }}>
                    <div style={{ width:6, height:6, borderRadius:'50%', background:dotColor, flexShrink:0 }}/>
                    <div style={{ flex:1, minWidth:0 }}>
                      <span style={{ fontSize:12, fontWeight:500, color:'#e6edf3' }}>{ev.event_type.replace(/_/g,' ')}</span>
                      <span style={{ fontSize:12, color:'#484f58' }}> — {ev.domain}</span>
                    </div>
                    <span style={{ fontSize:11, color:'#484f58', flexShrink:0, fontFamily:"'SF Mono','Menlo',monospace" }}>{ago}</span>
                  </div>
                )
              })}
            </div>
