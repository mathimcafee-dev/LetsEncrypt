    ...
    <div className="v2-card v2-card-pad" style={{ maxWidth: 480, margin: '48px auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
        ...
        <div>
          ...
          <div style={{ fontSize:11, color: '#888888', marginTop: 1 }}>SCM REST API credentials</div>
        </div>
      </div>
...
function InventoryLayer({ creds }) {
  ...
  if (loading) return <div style={{ padding: 32, textAlign: 'center', color: '#888888' }}><Spinner/><span style={{ marginLeft: 8 }}>Loading inventory…</span></div>
          ...
          return (
            <div key={c.id || i} style={{ display: 'grid', gridTemplateColumns: '2.5fr 1fr 1fr 1fr 1fr',
              ...
              <div>
                ...
                {c.id && <div style={{ fontSize:10, color: '#888888', marginTop: 1 }}>ID: {c.id}</div>}
              </div>
              <div style={{ fontSize:11, color: '#333333' }}>{typ}</div>
...
function OrgsLayer({ creds }) {
  ...
  if (loading) return <div style={{ padding: 32, textAlign: 'center', color: '#888888' }}><Spinner/><span style={{ marginLeft: 8 }}>Loading organisations…</span></div>
  if (error)   return <div className="v2-callout error" style={{ margin: 16 }}><AlertCircle size={13}/>{error}</div>
  if (!orgs)   return null
        ...
        {orgs.length === 0 ? (
          ...
          <div key={org.id || i} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr',
            ...
            <div>
              ...
              {org.id && <div style={{ fontSize:10, color: '#888888', marginTop: 1 }}>ID: {org.id}</div>}
            </div>
            <div style={{ fontSize:11, color: '#333333' }}>{[org.city, org.country].filter(Boolean).join(', ') || '—'}</div>
            <div>
              <StatusBadge status={org.status || org.validationStatus || 'active'}/>
            </div>
            <div style={{ fontSize:11, color: '#333333' }}>{org.certTypes || org.certType || '—'}</div>
          </div>
        ))}
      </div>
    </div>
...
function AnalyticsLayer({ creds }) {
  ...
  if (loading) return <div style={{ padding: 32, textAlign: 'center', color: '#888888' }}><Spinner/><span style={{ marginLeft: 8 }}>Analysing…</span></div>
  if (error)   return <div className="v2-callout error" style={{ margin: 16 }}><AlertCircle size={13}/>{error}</div>
  if (!data)   return null
