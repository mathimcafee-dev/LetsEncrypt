...
export default function PKIConsortiumIntelligence({ nav }) {
  ...
  return (
    <div className="v2-page" style={{ fontFamily: FONT }}>
      <style>{`
        .pkic-hero{background:#0a0a0a;padding:32px 24px 28px;color:#fff}
        ...
        .pkic-sub{font-size:13px;color:rgba(255,255,255,.5);max-width:680px;line-height:1.7;margin-bottom:20px}
        ...
        .pkic-body{padding:20px 24px;max-width:900px}
            ...
            {PQC_CONFERENCES.map(c => (
              <div key={c.location} className="conf-card">
                <div style={{ textAlign:'center', minWidth:60 }}>
                  ...
                  <div style={{ fontSize:10, color:'var(--v2-text-3)', marginTop:4 }}>{c.delegates}</div>
                  <div style={{ fontSize:9, color:'var(--v2-text-3)' }}>delegates</div>
                </div>
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:14, fontWeight:600, color:'var(--v2-text)', marginBottom:6 }}>{c.location}</div>
                  <div style={{ fontSize:12, color:'var(--v2-text-2)', lineHeight:1.7 }}>{c.desc}</div>
                </div>
              </div>
            ))}
          </>
          ...
          <>
            ...
            {KEY_INITIATIVES.map(init => (
              <div key={init.title} className="init-card">
                <div style={{ fontSize:13, fontWeight:600, color:'var(--v2-text)', marginBottom:6 }}>{init.title}</div>
        ...
        {tab === 'Members' && (
          <>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:8, marginBottom:20 }}>
              {MEMBERS.map(m => (
                <div key={m} style={{ background:'var(--v2-surface)', border:'0.5px solid var(--v2-border)', borderRadius:'var(--v2-r-xl)', padding:'8px 14px', fontSize:12, fontWeight:500, color:'var(--v2-text)' }}>{m}</div>
              ))}
            </div>
