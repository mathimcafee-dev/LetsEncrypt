  ...
  return (
    <div onClick={() => setOpen(o => !o)}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}>
        <div style={{ fontSize:14, fontWeight: 600, color: open ? '#000000' : 'rgba(255,255,255,0.7)' }}>{q}</div>
        <span style={{ color: '#ffffff', fontSize:20, lineHeight: 1, flexShrink: 0 }}>{open ? '−' : '+'}</span>
      </div>
      {open && (
...
const PLANS = [
  { key: 'starter',  name: 'Starter',  price: 'Starter', per: 'entry tier',                    color: '#ffffff', border: 'rgba(16,185,129,0.2)',  highlight: false, cta: 'Get started', badge: null,           certNote: 'Enterprise certificate pricing via RapidSSL' },
  { key: 'pro',      name: 'Pro',      price: '$29',  per: '/mo · billed annually',  color: '#ffffff', border: 'rgba(99,102,241,0.3)',  highlight: true,  cta: 'Start Pro',        badge: 'Most Popular', certNote: 'All cert types · RapidSSL partner rates' },
  { key: 'reseller', name: 'Reseller', price: 'Custom', per: 'contact us',           color: '#ffffff', border: 'rgba(245,158,11,0.2)',  highlight: false, cta: 'Contact us',       badge: null,           certNote: 'Volume pricing available' },
]
...
export default function Pricing({ nav }) {
  ...
  return (
    <div style={{ background: '#000000', minHeight: '100vh', color: '#ffffff', ...S }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: 'clamp(48px,8vw,80px) clamp(16px,4vw,24px) 100px' }}>
        ...
        <div className="pricing-hero" style={{ textAlign: 'center', marginBottom: 64 }}>
          <div style={{ display: 'inline-block', fontSize: 9, fontWeight: 700, color: '#ffffff',
          ...
          <h1 style={{ fontSize: 'clamp(32px,5vw,56px)', fontWeight: 900, letterSpacing: '-2px',
            ...
            Professional CLM.<br/>
            <span style={{ color: '#ffffff' }}>Transparent cert pricing.</span>
          </h1>
            ...
            <div key={plan.name} style={{
              ...
              {plan.badge && (
                <div style={{ position: 'absolute', top: -11, left: '50%', transform: 'translateX(-50%)',
                  ...
                  fontSize:10, fontWeight: 800, color: '#ffffff', whiteSpace: 'nowrap', letterSpacing: '0.5px', ...Mo }}>
                  {plan.badge}
                </div>
              )}
              ...
              <div style={{ marginBottom: 4 }}>
                ...
                <span style={{ fontSize:11, color: 'rgba(255,255,255,0.35)', marginLeft: 6, ...Mo }}>{plan.per}</span>
              </div>
        ...
        <div style={{ textAlign: 'center', marginBottom: 56 }}>
          <button onClick={() => setShowComparison(v => !v)}
            style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.6)',
        ...
        {showComparison && (
          <div style={{ marginBottom: 64, border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, overflow: 'hidden' }}>
            {/* Header */}
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr',
              background: 'rgba(255,255,255,0.03)', padding: '14px 20px',
              borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
              <div style={{ fontSize:11, color: 'rgba(255,255,255,0.35)', ...Mo }}>Feature</div>
              {PLANS.map(p => (
            ...
            {FEATURES.map(section => (
              <div key={section.section}>
                ...
                <div style={{ padding: '10px 20px', background: 'rgba(255,255,255,0.02)',
                  ...
                  <span style={{ fontSize:10, fontWeight: 700, color: '#ffffff', textTransform: 'uppercase',
                  ...
                  <div key={row.label} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr',
                    ...
                    {['starter', 'pro', 'reseller'].map(k => (
                      <div key={k} style={{ display: 'flex', justifyContent: 'center' }}>
                        {row[k] ? <Tick color={k === 'starter' ? '#ffffff' : k === 'pro' ? '#ffffff' : '#ffffff'}/> : <Cross/>}
                      </div>
                    ))}
                  </div>
                ))}
        ...
        <div style={{ marginBottom: 64 }}>
          <div style={{ textAlign: 'center', marginBottom: 36 }}>
            <div style={{ fontSize: 9, fontWeight: 700, color: '#ffffff', letterSpacing: '2px',
          ...
          <div className="cert-table" style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, overflow: 'hidden' }}>
            <div className="cert-table-grid" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr',
              background: 'rgba(255,255,255,0.035)', borderBottom: '1px solid rgba(255,255,255,0.08)', padding: '12px 20px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  {row.badge && (
                    <span style={{ fontSize: 8, fontWeight: 800, padding: '2px 7px', borderRadius: 10,
                      background: '#0d948818', color: '#ffffff', letterSpacing: '0.5px', ...Mo }}>
                      {row.badge}
                    </span>
                  )}
                </div>
                <div style={{ fontSize:12, color: 'rgba(255,255,255,0.35)', ...Mo }}>{row.term}</div>
                <div style={{ fontSize:12, color: 'rgba(255,255,255,0.35)', ...Mo }}>{row.ca}</div>
                <div style={{ fontSize:15, fontWeight: 800, color: '#ffffff', ...Mo }}>{row.price}</div>
              </div>
            ))}
          </div>
        ...
        <div style={{ border: '1px solid rgba(0,0,0,0.04)', borderRadius: 12, padding: '32px',
          ...
          <div className="pricing-compare-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: 1 }}>
            {[
              { name: 'SSLVault', price: 'Starter', sub: 'entry tier + cert costs', color: '#ffffff',
                points: ['Complete CLM platform', '47-day readiness built-in', 'CA-agnostic intelligence', 'CertVault included', 'cPanel + agent + DNS'] },
              { name: 'Venafi TLS Protect', price: '$250k+', sub: 'per year', color: 'rgba(255,255,255,0.35)',
                points: ['Enterprise only', 'No free tier', 'No cPanel support', 'No cert issuance', 'PKI team required'] },
              { name: 'Keyfactor Command', price: '$75–200k', sub: 'per year', color: 'rgba(255,255,255,0.35)',
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                  {item.points.map(p => (
                    <div key={p} style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                      ...
                      <span style={{ fontSize:11, color: i === 0 ? 'rgba(255,255,255,0.65)' : 'rgba(255,255,255,0.25)', lineHeight: 1.4 }}>{p}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div style={{ border: '1px solid rgba(16,185,129,0.18)', borderRadius: 12, padding: 'clamp(32px,5vw,48px) clamp(20px,4vw,40px)',
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <button onClick={() => nav('/auth')}
              style={{ background: '#ffffff', border: 'none', borderRadius: 7, color: '#ffffff',
            ...
            <button onClick={() => window.location.href = 'mailto:mathimcafee@gmail.com'}
              style={{ background: 'transparent', border: '1px solid rgba(245,158,11,0.4)', borderRadius: 7,
                color: '#ffffff', fontSize:14, fontWeight: 600, padding: '13px 28px', cursor: 'pointer', ...S }}>
