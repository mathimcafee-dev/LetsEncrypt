...
export default function eIDASIntelligence({ nav }) {
  ...
  return (
    <div className="v2-page" style={{ fontFamily: FONT }}>
      <style>{`
        .eid-hero{background:transparent;padding:clamp(16px,4vw,32px) clamp(12px,3vw,24px) 28px;color:#f0ede8}
        ...
        .eid-h1{font-size:26px;font-weight:600;letter-spacing:-.4px;color:#f0ede8;line-height:1.15;margin-bottom:10px}
        .eid-h1 em{color:#e07060;font-style:normal}
        ...
        .eid-sn{font-size:22px;font-weight:600;color:#f0ede8;line-height:1}
          ...
          <>
            ...
            {TRUST_SERVICES.map(ts => (
              <div key={ts.name} className="ts-card">
                <div style={{ fontSize:13, fontWeight:600, color:'var(--v2-text)', marginBottom:6 }}>{ts.name}</div>
                <div style={{ fontSize:12, color:'var(--v2-text-2)', lineHeight:1.7 }}>{ts.desc}</div>
              </div>
            ))}
          </>
        )}
          ...
          <>
            ...
            <div className="cmp-grid">
              <div className="cmp-card">
                <div style={{ fontFamily:MONO, fontSize:10, fontWeight:700, color:'var(--v2-text-3)', marginBottom:8 }}>eIDAS (910/2014)</div>
                {['Qualified electronic signatures legally binding EU-wide','Qualified Trust Service Providers (QTSPs) regulated','EU Trusted Lists — machine-readable QTSP registries','QWAC certificates for website authentication (voluntary)','No digital identity wallet mandate','CAs could choose whether to pursue QTSP status'].map(p=>(
                  <div key={p} style={{ display:'flex', gap:8, padding:'6px 0', borderBottom:'0.5px solid var(--v2-border)', fontSize:12, color:'var(--v2-text-2)' }}>
                    <span style={{ color:'var(--v2-text-3)', flexShrink:0 }}>·</span>{p}
                  </div>
                ))}
              </div>
              <div className="cmp-card" style={{ borderColor:'var(--v2-green-border)', background:'var(--v2-green-bg)' }}>
                ...
                {['All eIDAS 1.0 provisions retained and strengthened','EU Digital Identity Wallet — free, mandatory, every member state','Browsers legally required to trust QWAC certificates','Electronic attestation of attributes (verifiable credentials)','Remote qualified signing for mobile-first workflows','PQC migration planning provisions included'].map(p=>(
                  <div key={p} style={{ display:'flex', gap:8, padding:'6px 0', borderBottom:'0.5px solid var(--v2-green-border)', fontSize:12, color:'var(--v2-text)' }}>
