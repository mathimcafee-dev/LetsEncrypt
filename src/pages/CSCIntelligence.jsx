...
export default function CSCIntelligence({ nav }) {
  ...
  return (
    <div className="v2-page" style={{ fontFamily: FONT }}>
      <style>{`
        .csc-hero{background:transparent;padding:clamp(16px,4vw,32px) clamp(12px,3vw,24px) 28px;color:#f5f0eb}
        ...
        .csc-h1{font-size:26px;font-weight:600;letter-spacing:-.4px;color:#f5f0eb;line-height:1.15;margin-bottom:10px}
        ...
        .csc-sn{font-size:22px;font-weight:600;color:#f5f0eb;line-height:1}
        ...
        .ep-row{display:flex;gap:10px;padding:10px 0;border-bottom:0.5px solid var(--v2-border);align-items:flex-start}
            ...
            <div className="v2-card v2-card-pad">
              {API_ENDPOINTS.map(ep => (
                <div key={ep.endpoint} className="ep-row">
                  <span style={{ fontFamily:MONO, fontSize:11, fontWeight:700, color:'var(--v2-text-2)', minWidth:200, flexShrink:0 }}>{ep.endpoint}</span>
                  <span style={{ fontSize:12, color:'var(--v2-text-2)', lineHeight:1.6 }}>{ep.desc}</span>
                </div>
              ))}
            </div>
          </>
          ...
          <>
            ...
            {ADOPTERS.map(a => (
              <div key={a.name} className="adopter-card">
                <div style={{ fontSize:13, fontWeight:600, color:'var(--v2-text)', marginBottom:5 }}>{a.name}</div>
                <div style={{ fontSize:12, color:'var(--v2-text-2)', lineHeight:1.7 }}>{a.desc}</div>
              </div>
            ))}
          </>
        )}
