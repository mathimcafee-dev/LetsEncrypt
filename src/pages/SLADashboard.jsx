...
function scoreCol(s) { return s >= 80 ? '#4ade80' : s >= 50 ? '#f59e0b' : '#c0392b' }
function certCol(d)  { return d < 10 ? '#c0392b' : d < 30 ? '#f59e0b' : '#4ade80' }
...
export default function SLADashboard({ nav }) {
  ...
  var S = {
    ...
    card: { background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(192,57,43,0.25)', borderRadius: 10, padding: '14px 16px', marginBottom: 12 },
    ...
    btn:  { display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 16px', background: 'rgba(192,57,43,0.15)', border: '1px solid rgba(192,57,43,0.4)', borderRadius: 8, color: '#f0ede8', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: "'Segoe UI',system-ui,sans-serif" },
  ...
  if (err) return (
    <div style={S.page}>
      <div style={{ background: 'rgba(192,57,43,0.1)', border: '1px solid rgba(192,57,43,0.3)', borderRadius: 8, padding: '12px 16px', color: '#f87171', fontSize: 13, marginBottom: 12 }}>Error: {err}</div>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: 'rgba(240,237,232,0.4)' }}>
              Plan: <span style={{ color: '#c0392b', fontWeight: 600 }}>{(sub.plan || 'active').toUpperCase()}</span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 20 }}>
          ...
          <div style={S.card}>
            ...
            <div style={{ fontSize: 28, fontWeight: 800, color: (status.expiring_10d || 0) > 0 ? '#c0392b' : '#4ade80' }}>{status.expiring_10d || 0}</div>
          </div>
        </div>
            ...
            <tbody>
              {certs.length === 0
                ...
                : certs.map(function(c) {
                  ...
                  var cc = certCol(dl)
                  return (
                    <tr key={c.id || c.domain} style={{ borderTop: '1px solid rgba(192,57,43,0.1)' }}>
          ...
          : reports.map(function(r) {
            return (
              <div key={r.id} style={Object.assign({}, S.card, { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 })}>
                ...
                {r.report_url && (
                  <a href={r.report_url} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: '#c0392b', textDecoration: 'none', fontWeight: 600, flexShrink: 0 }}>View</a>
                )}
              </div>
            )
          })
