    ...
    <div style={{minHeight:'100vh',background:bg,position:'relative'}}>
        ...
        <div style={{maxWidth:820,margin:'0 auto',padding:'0 24px'}}>
          <div style={{background:'white',borderRadius:24,padding:'48px 50px',boxShadow:'0 4px 24px rgba(15,23,42,0.06)',border:'1px solid #e2e8f0'}}>
            ...
            <Section title='2. What we collect'>
              ...
              <p style={p}><strong>Private keys (temporary, user-controlled):</strong> when you issue a certificate through SSLVault, the private key is generated server-side and stored in our database. This is a technical requirement — the SSLVault agent and cPanel agent are asynchronous and must retrieve the key later to complete installation on your server. The private key is transmitted only over HTTPS, is protected by row-level security (only your account can access it), and is never logged. Once your certificate is installed, you can permanently delete the private key from our servers using the "Delete key from servers" button in your Dashboard — after which we hold no copy. Auto-renewal generates a fresh key at renewal time. We are working toward client-side key generation (Phase 2) which will eliminate server-side key storage entirely.</p>
      ...
      <section style={{position:'relative',padding:'0 0 80px'}}>
        <div style={{maxWidth:820,margin:'0 auto',padding:'0 24px',display:'flex',gap:12,justifyContent:'center',flexWrap:'wrap'}}>
          <button onClick={()=>nav('/terms')} style={{background:'white',color:'#0f172a',border:'1.5px solid #e2e8f0',padding:'12px 22px',borderRadius:10,fontSize:13,fontWeight:700,cursor:'pointer',display:'inline-flex',alignItems:'center',gap:8}}>
            <FileText size={14}/> Terms of Service
          </button>
          <button onClick={()=>nav('/contact')} style={{background:'#0f172a',color:'white',border:'none',padding:'12px 22px',borderRadius:10,fontSize:13,fontWeight:700,cursor:'pointer',display:'inline-flex',alignItems:'center',gap:8}}>
...
const p = {fontSize:14,color:'#475569',lineHeight:1.75,marginBottom:12}
const ul = {fontSize:14,color:'#475569',lineHeight:1.85,paddingLeft:22,marginBottom:14}
const a = {color:'#2563eb',fontWeight:700,textDecoration:'none'}

function Section({title, children, last}) {
  return (
    <div style={{paddingBottom:last?0:28,marginBottom:last?0:24,borderBottom:last?'none':'1px dashed #e2e8f0'}}>
      <h3 style={{fontFamily:'Georgia, serif',fontSize:20,fontWeight:800,color:'#0f172a',letterSpacing:'-0.4px',marginBottom:14}}>{title}</h3>
