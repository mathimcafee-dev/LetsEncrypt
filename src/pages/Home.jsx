// Home.jsx — SSLVault · Owlish.bot design · all content preserved
...
function H2({ children, style={} }) {
  return <h2 style={{fontSize:'clamp(22px,3vw,36px)',fontWeight:700,color:T1,letterSpacing:'-0.8px',lineHeight:1.15,...style}}>{children}</h2>
}
...
function Card({ children, style={} }) {
  return <div style={{background:BG3,border:`1px solid ${LINE}`,borderRadius:4,padding:'20px',...style}}>{children}</div>
}
function Term({ title, lines }) {
  return (
    <div style={{background:BG2,border:`1px solid ${LINE}`,borderRadius:4,overflow:'hidden',fontFamily:MONO}}>
      <div style={{background:BG4,padding:'9px 14px',display:'flex',alignItems:'center',gap:6,borderBottom:`1px solid ${LINE}`}}>
      ...
      <div style={{padding:'16px 18px',fontSize:11.5,lineHeight:2}}>
        ...
        <div style={{marginTop:8,display:'flex',alignItems:'center',gap:5}}><span style={{color:T3}}>›</span><span style={{display:'inline-block',width:6,height:12,background:'rgba(255,255,255,0.4)',animation:'blink 1.2s step-end infinite',borderRadius:1}}/></div>
      </div>
    </div>
  )
}
function InventoryMockup() {
  ...
  return (
    <div style={{background:BG2,border:`1px solid ${LINE}`,borderRadius:4,overflow:'hidden'}}>
      <div style={{background:BG4,padding:'9px 14px',display:'flex',alignItems:'center',gap:6,borderBottom:`1px solid ${LINE}`}}>
...
function ReadinessMockup() {
  const certs=[{d:'easysecurity.in',s:92,label:'Ready',color:GRN},{d:'api.shop.com',s:58,label:'At Risk',color:AMB},{d:'staging.portal.io',s:24,label:'Will Break',color:RED}]
  return (
    <div style={{background:BG2,border:`1px solid ${LINE}`,borderRadius:4,overflow:'hidden'}}>
      <div style={{background:BG4,padding:'9px 14px',display:'flex',alignItems:'center',gap:6,borderBottom:`1px solid ${LINE}`}}>
        <div style={{display:'flex',gap:5}}>{['#ff5f57','#ffbd2e','#28c840'].map(c=><div key={c} style={{width:8,height:8,borderRadius:'50%',background:c}}/>)}</div>
      ...
      <div style={{padding:'14px'}}>
        ...
        {certs.map(c=>(
          <div key={c.d} style={{display:'flex',alignItems:'center',gap:9,padding:'7px 9px',borderRadius:3,background:BG4,marginBottom:5,border:`1px solid ${LINE}`}}>
            <div style={{position:'relative',width:32,height:32,flexShrink:0}}>
              <svg width="32" height="32" viewBox="0 0 36 36">
                <circle cx="18" cy="18" r="14" fill="none" stroke={LINE2} strokeWidth="3"/>
                <circle cx="18" cy="18" r="14" fill="none" stroke={c.color} strokeWidth="3" strokeDasharray={`${2*Math.PI*14}`} strokeDashoffset={`${2*Math.PI*14*(1-c.s/100)}`} strokeLinecap="round" transform="rotate(-90 18 18)"/>
              </svg>
            ...
            <span style={{fontSize:10,fontWeight:600,color:c.color,background:c.color+'18',padding:'2px 7px',borderRadius:3,fontFamily:MONO}}>{c.label}</span>
          </div>
        ))}
      </div>
    </div>
...
export default function Home({ nav }) {
  ...
  useEffect(() => {
    if (!certCount) return
    let i=0; const iv=setInterval(()=>{ i+=Math.ceil(certCount/60); if(i>=certCount){setCount(certCount);clearInterval(iv)}else setCount(i) },16)
    return ()=>clearInterval(iv)
  },[certCount])

  const cols = isMobile?1:isTablet?2:3
  const sec = (id,bg=BG) => ({ id, style:{ background:bg, padding:`clamp(64px,8vw,96px) clamp(20px,4vw,48px)`, borderTop:`1px solid ${LINE}` } })
  ...
  return (
    <div style={{fontFamily:F,background:BG,color:T1,overflowX:'hidden'}}>
        ...
        {!isMobile && <nav style={{display:'flex',alignItems:'center',gap:1}}>
          {[['Platform','#platform'],['Features','#features'],['Security','#security'],['Pricing','/pricing']].map(([l,h])=>(
            <button key={l} onClick={()=>h.startsWith('/')?nav(h):document.querySelector(h)?.scrollIntoView({behavior:'smooth'})}
              style={{background:'none',border:'none',cursor:'pointer',fontFamily:F,fontSize:12,fontWeight:400,color:T2,padding:'5px 12px',borderRadius:3,transition:'color .15s'}}
              onMouseEnter={e=>e.currentTarget.style.color=T1} onMouseLeave={e=>e.currentTarget.style.color=T2}>{l}</button>
          ))}
          <div style={{position:'relative'}}
            onMouseEnter={e=>{const d=e.currentTarget.querySelector('.intel-d');if(d){d.style.opacity='1';d.style.transform='translateX(-50%) translateY(0)';d.style.pointerEvents='auto'}}}
            onMouseLeave={e=>{const d=e.currentTarget.querySelector('.intel-d');if(d){d.style.opacity='0';d.style.transform='translateX(-50%) translateY(-5px)';d.style.pointerEvents='none'}}}>
            <button style={{background:'none',border:'none',cursor:'pointer',fontFamily:F,fontSize:12,fontWeight:400,color:T2,padding:'5px 12px',borderRadius:3,transition:'color .15s',display:'flex',alignItems:'center',gap:4}}
            ...
            <div className="intel-d" style={{position:'absolute',top:'calc(100% + 6px)',left:'50%',transform:'translateX(-50%) translateY(-5px)',background:BG3,border:`1px solid ${LINE2}`,borderRadius:4,padding:'5px',minWidth:200,zIndex:300,boxShadow:'0 8px 32px rgba(0,0,0,0.5)',opacity:0,pointerEvents:'none',transition:'opacity .18s,transform .18s'}}>
      ...
      {/* ── HERO ── */}
      <section style={{background:BG,padding:`clamp(72px,10vw,120px) clamp(20px,5vw,48px) clamp(64px,8vw,100px)`,borderBottom:`1px solid ${LINE}`,position:'relative'}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:isMobile?48:72,alignItems:'center'}}>
            <div>
              ...
              <h1 style={{fontSize:`clamp(36px,5.5vw,68px)`,fontWeight:700,letterSpacing:'-2px',lineHeight:1.06,color:T1,marginBottom:20}}>
              ...
              <div style={{display:'flex',gap:10,flexWrap:'wrap',marginBottom:40}}>
                <Btn label="Start managing certs" onClick={()=>nav('/auth')} white/>
                <Btn label="View pricing" onClick={()=>nav('/pricing')}/>
              </div>
              <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',borderTop:`1px solid ${LINE}`,paddingTop:24}}>
                {[{val:certCount?`${count.toLocaleString()}+`:'—',label:'Certs managed'},{val:'99.9%',label:'Renewal success'},{val:'12',label:'PKI bodies tracked'}].map((m,i)=>(
                  <div key={m.label} style={{paddingLeft:i>0?20:0,borderLeft:i>0?`1px solid ${LINE}`:'none',marginLeft:i>0?20:0}}>
                    <div style={{fontSize:20,fontWeight:700,color:T1,fontFamily:MONO,letterSpacing:'-0.5px',marginBottom:3}}>{m.val}</div>
      ...
      {/* ── TICKER ── */}
      <div style={{background:BG2,borderBottom:`1px solid ${LINE}`,overflow:'hidden',padding:'8px 0'}}>
        <div style={{display:'flex',overflow:'hidden',maskImage:'linear-gradient(to right,transparent,black 8%,black 92%,transparent)'}}>
          <div style={{display:'flex',gap:48,flexShrink:0,animation:'ticker 36s linear infinite',whiteSpace:'nowrap'}}>
            {[...TICKER,...TICKER].map((p,i)=><span key={i} style={{fontSize:10.5,fontWeight:500,color:T3,letterSpacing:'0.05em',fontFamily:MONO}}><span style={{color:'rgba(255,255,255,0.2)',marginRight:8}}>◆</span>{p}</span>)}
          </div>
        </div>
      </div>

      {/* ── PLATFORM ── */}
      <section id="platform" style={{background:BG,padding:`clamp(64px,8vw,96px) clamp(20px,4vw,48px)`,borderTop:`1px solid ${LINE}`}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          <FadeUp>
            <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:isMobile?40:80,alignItems:'flex-start',marginBottom:52}}>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:0}}>
                {[
                  {n:'/ 01',t:'Issue',d:'Submit to RapidSSL via ACME v2. DNS-01 auto-validated via your provider API.'},
                  {n:'/ 02',t:'Install',d:'Agent deploys to Nginx/Apache. cPanel UAPI install. Zero SSH after setup.'},
                  {n:'/ 03',t:'Monitor',d:'Expiry tracking, health scoring A–F, CT log abuse detection, CA/B Forum compliance.'},
                  {n:'/ 04',t:'Renew',d:'Auto-renews 30 days before expiry. New cert deployed before old one expires.'},
                ].map((s,i)=>(
                  <div key={s.n} style={{display:'flex',gap:14,padding:'16px 0',borderBottom:i<3?`1px solid ${LINE}`:'none'}}>
                    <span style={{fontSize:11,color:T3,width:32,flexShrink:0,marginTop:1,fontFamily:MONO}}>{s.n}</span>
                    <div>
                      <div style={{fontSize:13,fontWeight:600,color:T1,marginBottom:4}}>{s.t}</div>
                      <div style={{fontSize:12.5,color:T2,lineHeight:1.6}}>{s.d}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <FadeUp>
            <div style={{fontSize:10,fontWeight:500,color:T3,fontFamily:MONO,marginBottom:10,textTransform:'uppercase',letterSpacing:'0.05em'}}>Certificate inventory — live expiry, health grade, auto-renew status</div>
            <InventoryMockup/>
          </FadeUp>
        </div>
      </section>
      ...
      {/* ── FEATURES ── */}
      <section id="features" style={{background:BG2,padding:`clamp(64px,8vw,96px) clamp(20px,4vw,48px)`,borderTop:`1px solid ${LINE}`}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          <FadeUp>
          ...
          <div style={{display:'grid',gridTemplateColumns:`repeat(${cols},1fr)`,gap:1,background:LINE,border:`1px solid ${LINE}`,borderRadius:4,overflow:'hidden'}}>
            {[
              ...
              {icon:'🤖',title:'Persistent agent',specs:['systemd daemon · polls every 5 min','Nginx + Apache auto-detect','Config test before reload','Outbound HTTPS only — no firewall changes'],badge:'systemd'},
              ...
              {icon:'📋',title:'47-day readiness',specs:['Scores every cert 0–100','200d → 100d → 47d mandate timeline','Per-cert automation checklist','Fleet-wide compliance report'],badge:'CA/B 2026'},
              ...
              {icon:'🏛',title:'Industry Intelligence',specs:['6,200+ CAs from CCADB live','CAB Forum ballot tracker','12 PKI bodies deep-dive','PQC migration tracker (ML-KEM, ML-DSA)'],badge:'PKI Hub'},
              {icon:'🔬',title:'CA Trust Store',specs:['Every root & intermediate CA indexed','PKI Trust Score per certificate','Filter by trust store · algorithm · region','CSV export · PEM download'],badge:'CCADB'},
            ].map(f=>(
              <FadeUp key={f.title}>
      ...
      {/* ── CERTVAULT + READINESS ── */}
      <section style={{background:BG,padding:`clamp(64px,8vw,96px) clamp(20px,4vw,48px)`,borderTop:`1px solid ${LINE}`}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          <FadeUp>
          ...
          <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:20}}>
            <FadeUp>
              <div>
                <div style={{fontSize:10,fontWeight:500,color:T3,fontFamily:MONO,textTransform:'uppercase',letterSpacing:'0.05em',marginBottom:10}}>🔐 AES-256-GCM private key vault</div>
                <CertVaultMockup/>
              </div>
            </FadeUp>
            <FadeUp delay={80}>
              <div>
                <div style={{fontSize:10,fontWeight:500,color:T3,fontFamily:MONO,textTransform:'uppercase',letterSpacing:'0.05em',marginBottom:10}}>📋 CA/B Forum compliance scoring</div>
                <ReadinessMockup/>
              </div>
            </FadeUp>
          </div>
      ...
      {/* ── SECURITY SPECS ── */}
      <section id="security" style={{background:BG2,padding:`clamp(64px,8vw,96px) clamp(20px,4vw,48px)`,borderTop:`1px solid ${LINE}`}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          <FadeUp>
            <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:isMobile?40:80,alignItems:'flex-start',marginBottom:48}}>
              <div>
                ...
                <Body style={{marginBottom:22}}>Every layer of SSLVault is built on auditable open standards. RFC 8555 for issuance, AES-256-GCM for storage, CT logs for transparency.</Body>
                <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                  {['RFC 8555','AES-256-GCM','TLS 1.3','CT Logs','CAA Records','HSTS','SHA-256'].map(t=><Tag key={t}>{t}</Tag>)}
                </div>
              </div>
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(min(300px,100%),1fr))',gap:1,background:LINE,border:`1px solid ${LINE}`,borderRadius:4,overflow:'hidden'}}>
                {[
                  {spec:'AES-256-GCM',title:'Key encryption',desc:'DEK wrapped with KEK. Keys never in plaintext.'},
                  {spec:'RFC 8555',title:'ACME v2',desc:'DNS-01 challenge. Auto-validated via provider API.'},
                  {spec:'CT monitoring',title:'Cert transparency',desc:'crt.sh queries for every cert ever issued.'},
                  {spec:'CAA + HSTS',title:'DNS security',desc:'CAA prevents unauthorised CA issuance.'},
                  {spec:'TLS 1.2 / 1.3',title:'TLS posture',desc:'ECDHE + PFS. HSTS max-age verified.'},
                  {spec:'Append-only',title:'Audit trail',desc:'Every access logged. CSV export for SOC 2.'},
                ].map(s=>(
          ...
          <FadeUp>
            <Card>
              <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:24}}>
                <div style={{width:7,height:7,borderRadius:'50%',background:'rgba(255,255,255,0.4)',animation:'blink 2s ease infinite'}}/>
              ...
              <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'repeat(3,1fr)',gap:isMobile?20:0}}>
                {[
                  ...
                  <div key={m.date} style={{padding:isMobile?'16px 0':'0 24px',borderLeft:!isMobile&&i>0?`1px solid ${LINE}`:'none'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:isMobile?40:80,alignItems:'center'}}>
            <FadeUp>
              <div>
                ...
                <Body style={{marginBottom:24}}>When issuing or renewing, SSLVault calls your DNS provider API to add the ACME challenge record, polls for propagation, validates, then cleans up — fully automatic, zero copy-paste.</Body>
                <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(min(300px,100%),1fr))',gap:4}}>
                  {[['Cloudflare','API Token · Zone:DNS:Edit'],['Vercel','Access Token · Settings → Tokens'],['Route53','AWS IAM · Route53 write access'],['Namecheap','API Key · IP whitelist required'],['GoDaddy','API Key + Secret'],['DigitalOcean','Personal Access Token'],['Plesk','XML API'],['+ more','Contact us']].map(([name,note])=>(
                    <div key={name} style={{display:'flex',alignItems:'center',gap:9,padding:'8px 10px',background:BG3,border:`1px solid ${LINE}`,borderRadius:3}}>
                      ...
                      <div>
                        ...
                        <div style={{fontSize:10.5,color:T3,fontFamily:MONO}}>{note}</div>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <FadeUp delay={80}>
              <Term title="dns-provider · Cloudflare API" lines={[
                ...
                {prompt:'›',text:'Polling propagation...',c:T3},
        ...
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          <FadeUp>
            <div style={{textAlign:'center',marginBottom:48}}>
              ...
              <H2 style={{maxWidth:500,margin:'8px auto 12px'}}>From CSR to live HTTPS in one automated pipeline.</H2>
              <Body style={{maxWidth:440,margin:'0 auto'}}>SSLVault orchestrates every step across ACME, your DNS provider, and your web server.</Body>
            </div>
          </FadeUp>
          <FadeUp>
            ...
            <div style={{display:'grid',gridTemplateColumns:`repeat(${isMobile?1:5},1fr)`,gap:isMobile?10:0,alignItems:'center'}}>
        ...
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          <FadeUp>
            <div style={{marginBottom:48}}>
              ...
              <H2 style={{marginBottom:12,maxWidth:560}}>The deepest PKI intelligence platform on the web.</H2>
              <Body style={{maxWidth:520}}>Not just a certificate manager — a living knowledge base covering every CA, standard, governance body, and cryptographic transition shaping the industry.</Body>
            </div>
          </FadeUp>
          <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'repeat(3,1fr)',gap:1,background:LINE,border:`1px solid ${LINE}`,borderRadius:4,overflow:'hidden',marginBottom:16}}>
            {[
              ...
              {icon:'⚖️',title:'CAB Forum Intelligence',sub:'Ballots, timelines & compliance',desc:'Every CAB Forum ballot tracked with plain-English summaries. 47-day countdown, SC081v3 compliance deadlines, 5 working groups, full PKI history timeline from 2005.',badge:'Live sync',path:'/cab-forum',stats:[['47-day','2029 mandate'],['5','Working groups'],['Real-time','Ballot feed']]},
              {icon:'🌍',title:'Global PKI Hub',sub:'12 bodies · 22 standards · PQC tracker',desc:'CAB Forum, ETSI ESI, NIST, IETF, APKIC, eIDAS 2.0, PKI Consortium, CSC, FIDO, WebTrust, CCADB, ITU-T — each with deep-dive pages, standards library, and PQC migration status.',badge:'PQC Ready',path:'/pki-hub',stats:[['12','PKI bodies'],['3','NIST PQC finals'],['2026','Amsterdam conf.']]},
            ].map(item=>(
              <FadeUp key={item.title}>
                <div onClick={()=>nav(item.path)} style={{background:BG,padding:'22px',cursor:'pointer',transition:'background .15s',height:'100%',display:'flex',flexDirection:'column',gap:14}}
                  ...
                  <div>
                    <div style={{fontSize:14,fontWeight:600,color:T1,marginBottom:3}}>{item.title}</div>
                  ...
                  <div style={{display:'flex',gap:0,borderTop:`1px solid ${LINE}`,paddingTop:12,marginTop:'auto'}}>
                    {item.stats.map(([val,label],i)=>(
                      <div key={label} style={{flex:1,paddingLeft:i>0?12:0,borderLeft:i>0?`1px solid ${LINE}`:'none',marginLeft:i>0?12:0}}>
                        <div style={{fontSize:12,fontWeight:600,color:T1,fontFamily:MONO,marginBottom:2}}>{val}</div>
                        <div style={{fontSize:10,color:T3}}>{label}</div>
                      </div>
                    ))}
                  </div>
                  <div style={{fontSize:11,fontWeight:500,color:T3}}>Explore →</div>
                </div>
              </FadeUp>
            ))}
          </div>
          <FadeUp>
            <Card style={{display:'flex',gap:18,alignItems:'flex-start',flexWrap:'wrap'}}>
              ...
              <div style={{display:'flex',gap:7,flexWrap:'wrap',alignItems:'center',flexShrink:0}}>
                {[['ML-KEM','FIPS 203'],['ML-DSA','FIPS 204'],['SLH-DSA','FIPS 205']].map(([alg,fips])=>(
                  <div key={alg} style={{background:BG4,border:`1px solid ${LINE}`,borderRadius:3,padding:'6px 10px',textAlign:'center'}}>
                ...
                <button onClick={()=>nav('/pki-hub')} style={{background:T1,border:'none',borderRadius:3,padding:'8px 14px',fontSize:12,fontWeight:600,color:BG,cursor:'pointer',fontFamily:F}}>View PQC tracker →</button>
              </div>
            </Card>
          </FadeUp>
        </div>
        ...
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          <FadeUp>
            ...
            <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:isMobile?40:80,alignItems:'center',marginBottom:48}}>
              <div>
                <h2 style={{fontSize:`clamp(24px,4vw,44px)`,fontWeight:700,letterSpacing:'-1.2px',lineHeight:1.1,color:T1,marginBottom:18}}>
                  Every other CLM tells you what cert you issued.<br/><span style={{color:T2}}>SSLVault proves what's actually running.</span>
                </h2>
                <p style={{fontSize:14,color:T2,lineHeight:1.8,marginBottom:24,maxWidth:460}}>A certificate can be valid. HTTPS can be green. Your CLM can show everything healthy. And your server can be serving a mismatched key, a rogue cert, or half a load balancer pool — and nobody knows.</p>
                <p style={{fontSize:14,color:T2,lineHeight:1.8,marginBottom:28,maxWidth:460}}>CertBind closes that gap with continuous, cryptographic proof — every 5 minutes.</p>
                <div style={{display:'flex',gap:10}}>
                  <Btn label="See CertBind" onClick={()=>nav('/auth')} white/>
                  <Btn label="Read how it works" onClick={()=>nav('/knowledge-base')}/>
                </div>
              </div>
              <div style={{display:'flex',flexDirection:'column',gap:6}}>
                <div style={{background:BG3,border:`1px solid ${LINE}`,borderRadius:4,padding:'12px 16px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                  ...
                  <div key={l.n} style={{background:BG3,border:`1px solid ${LINE}`,borderRadius:4,padding:'11px 14px',display:'flex',gap:11,alignItems:'flex-start'}}>
                    ...
                    <div style={{flex:1}}>
                      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:3}}>
                        ...
                        <span style={{fontSize:9.5,fontWeight:700,color:GRN,fontFamily:MONO}}>{l.status}</span>
                      </div>
              ...
              <div style={{display:'grid',gridTemplateColumns:`repeat(${isMobile?1:3},1fr)`,gap:8}}>
                {[
                  ...
                  <div key={s.title} style={{background:s.bg,border:`1px solid ${s.bd}`,borderRadius:4,padding:'18px'}}>
                    ...
                    <div style={{fontSize:13,fontWeight:600,color:T1,marginBottom:9}}>{s.title}</div>
                    <div style={{fontSize:12,color:T2,lineHeight:1.75,marginBottom:10}}>{s.scenario}</div>
          ...
          <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:isMobile?40:80,alignItems:'flex-start'}}>
            <FadeUp>
              <div>
                ...
                <Body style={{marginBottom:20}}>SSLVault is built by a Certified PKI Specialist with deep CA industry experience. The same automation enterprise teams pay hundreds of thousands for — available without the procurement cycle.</Body>
                ...
                <div style={{display:'flex',gap:10}}>
                  <Btn label="Get started" onClick={()=>nav('/auth')} white/>
                  <Btn label="View pricing" onClick={()=>nav('/pricing')}/>
                </div>
              </div>
            </FadeUp>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:6}}>
                {[{name:'Venafi TLS Protect',price:'$250k+/yr',notes:'Enterprise only · No cert issuance · No cPanel',highlight:false},{name:'Keyfactor Command',price:'$75–200k/yr',notes:'Mid-market · Complex setup · No free tier',highlight:false},{name:'SSLVault CLM',price:'Partner rates',notes:'Full CLM · Agent + cPanel + DNS · All cert types',highlight:true}].map(c=>(
                  <div key={c.name} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 14px',background:c.highlight?'rgba(0,0,0,0.03)':BG3,border:`1px solid ${c.highlight?LINE2:LINE}`,borderRadius:4}}>
                    ...
                    <div style={{fontSize:13,fontWeight:600,color:c.highlight?T1:T3,fontFamily:MONO,whiteSpace:'nowrap'}}>{c.price}</div>
                  </div>
                ))}
                <div style={{marginTop:6,display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(min(300px,100%),1fr))',gap:4}}>
                  {[['DigiCert','Trust chain · 99.9% browser'],['RapidSSL','CA partner · wholesale pricing'],['RFC 8555','ACME v2 · no lock-in'],['AES-256','Military-grade key storage'],['GDPR','Netherlands-based PKI engineer 🇳🇱'],['No ads','No tracking · no reselling']].map(([val,sub])=>(
                    <div key={val} style={{display:'flex',gap:9,padding:'9px 10px',background:BG3,border:`1px solid ${LINE}`,borderRadius:3,alignItems:'center'}}>
                      <div style={{fontSize:11,fontWeight:600,color:T1,fontFamily:MONO,minWidth:65}}>{val}</div>
        ...
        <div style={{maxWidth:600,margin:'0 auto',textAlign:'center'}}>
          <FadeUp>
            ...
            <h2 style={{fontSize:'clamp(24px,4.5vw,42px)',fontWeight:700,letterSpacing:'-1px',lineHeight:1.12,color:T1,marginBottom:16}}>
              Ready to automate your<br/><span style={{color:T2}}>certificate lifecycle?</span>
            </h2>
            <p style={{fontSize:14,color:T2,lineHeight:1.75,maxWidth:440,margin:'0 auto 32px'}}>
            ...
            <div style={{display:'flex',gap:18,justifyContent:'center',flexWrap:'wrap'}}>
              {['RapidSSL · DigiCert trust chain','RFC 8555 · AES-256-GCM','CA/B Forum 2026 ready'].map(t=>(
                <span key={t} style={{fontSize:11,color:T3,display:'flex',alignItems:'center',gap:5}}>
                  <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.3)" strokeWidth="2.5"><path d="M20 6L9 17l-5-5"/></svg>{t}
                </span>
              ))}
            </div>
          </FadeUp>
                ...
                {col.links.map(([l,h])=>(
                  <div key={l} style={{marginBottom:9}}>
                    <button onClick={()=>typeof h==='function'?h():document.querySelector(h)?.scrollIntoView({behavior:'smooth'})}
                      style={{background:'none',border:'none',cursor:'pointer',fontSize:12,color:T3,fontFamily:F,padding:0,transition:'color .15s',textAlign:'left'}}
