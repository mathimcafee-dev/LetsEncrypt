...
function useIn(t=0.08){const ref=useRef(null);const[v,setV]=useState(false);useEffect(()=>{const io=new IntersectionObserver(([e])=>{if(e.isIntersecting){setV(true);io.disconnect()}},{threshold:t});if(ref.current)io.observe(ref.current);return()=>io.disconnect()},[]);return[ref,v]}
...
function BtnW({label,onClick}){const[h,setH]=useState(false);return <button onClick={onClick} onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)} style={{display:'inline-flex',alignItems:'center',gap:7,fontFamily:F,fontWeight:600,fontSize:13,padding:'8px 18px',borderRadius:3,border:'none',cursor:'pointer',background:h?'#222':BK,color:W,transition:'background .12s'}}>{label} <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg></button>}
function BtnG({label,onClick}){const[h,setH]=useState(false);return <button onClick={onClick} onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)} style={{display:'inline-flex',alignItems:'center',gap:7,fontFamily:F,fontWeight:400,fontSize:13,padding:'8px 18px',borderRadius:3,cursor:'pointer',background:h?BG3:'transparent',border:`1px solid ${h?LN2:LN}`,color:h?T1:T2,transition:'all .12s'}}>{label}</button>}
function Card({children,style={}}){return <div style={{background:BG3,border:`1px solid ${LN}`,borderRadius:4,padding:'18px',...style}}>{children}</div>}

function Term({title,lines}){
  return <div style={{background:DK2,border:'1px solid rgba(255,255,255,0.08)',borderRadius:4,overflow:'hidden',fontFamily:MONO}}>
    <div style={{background:'rgba(255,255,255,0.04)',padding:'8px 14px',display:'flex',alignItems:'center',gap:6,borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
      ...
      <span style={{fontSize:10,color:'rgba(255,255,255,0.28)',flex:1,textAlign:'center'}}>{title}</span>
    </div>
    <div style={{padding:'14px 16px',fontSize:11.5,lineHeight:2}}>
      {lines.map((l,i)=><div key={i} style={{color:l.c||'rgba(255,255,255,0.55)',paddingLeft:l.indent?18:0}}>
        {l.prompt&&<span style={{color:'rgba(255,255,255,0.22)',marginRight:8}}>{l.prompt}</span>}{l.text}
      </div>)}
      <div style={{marginTop:8,display:'flex',alignItems:'center',gap:5}}><span style={{color:'rgba(255,255,255,0.22)'}}>›</span><span style={{display:'inline-block',width:6,height:12,background:'rgba(255,255,255,0.4)',animation:'blink 1.2s step-end infinite',borderRadius:1}}/></div>
    </div>
  </div>
}

function InventoryMockup(){
  const rows=[{d:'easysecurity.in',days:196,grade:'A+',ca:'RapidSSL',auto:true,s:'active'},{d:'api.shop.com',days:18,grade:'B',ca:'Sectigo',auto:true,s:'warning'},{d:'staging.portal.io',days:3,grade:'C',ca:'RapidSSL',auto:false,s:'critical'},{d:'mail.company.com',days:84,grade:'A',ca:'RapidSSL',auto:true,s:'active'}]
  const sc=s=>s==='active'?GRN:s==='warning'?AMB:RED
  const gc=g=>g.startsWith('A')?GRN:g==='B'?AMB:RED
  return <div style={{background:W,border:`1px solid ${LN}`,borderRadius:4,overflow:'hidden'}}>
    <div style={{background:BG4,padding:'8px 14px',display:'flex',alignItems:'center',gap:6,borderBottom:`1px solid ${LN}`}}>
    ...
    {rows.map((r,i)=><div key={r.d} style={{display:'grid',gridTemplateColumns:'2fr 50px 52px 80px 55px 68px',padding:'9px 14px',borderBottom:i<rows.length-1?`1px solid ${LN}`:'none',background:r.s==='critical'?'rgba(220,38,38,0.03)':r.s==='warning'?'rgba(217,119,6,0.03)':'transparent',alignItems:'center'}}>
      ...
      <span style={{fontSize:10,fontWeight:600,color:r.auto?GRN:T3}}>{r.auto?'✓ ON':'— OFF'}</span>
      <Pill status={r.s}/>
    </div>)}
  </div>
}
...
function CertVaultMockup(){
  return <div style={{background:W,border:`1px solid ${LN}`,borderRadius:4,overflow:'hidden'}}>
    ...
    <div style={{padding:'14px'}}>
      <div style={{display:'flex',alignItems:'center',gap:8,padding:'7px 10px',background:BG3,border:`1px solid ${LN}`,borderRadius:3,marginBottom:10,fontSize:11,color:T2,fontFamily:MONO}}>AES-256-GCM · Envelope encryption · Immutable audit</div>
      {[{d:'easysecurity.in',alg:'RSA-2048',last:'2h ago'},{d:'api.shop.com',alg:'EC-384',last:'Never'}].map((k,i)=>(
        <div key={k.d} style={{border:`1px solid ${LN}`,borderTop:`2px solid ${T1}`,borderRadius:3,padding:'10px',marginBottom:i===0?6:0}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:7}}>
            <span style={{fontSize:12,fontWeight:500,color:T1,fontFamily:MONO}}>{k.d}</span>
            <span style={{fontSize:9,fontWeight:600,color:T3,background:BG3,padding:'2px 7px',borderRadius:3,fontFamily:MONO}}>🔒 VAULT SECURED</span>
          </div>
          ...
          <div style={{display:'flex',gap:5}}>
            {['Reveal key','Rotate','Audit log'].map(t=><button key={t} style={{fontSize:10,fontWeight:500,padding:'4px 9px',borderRadius:3,border:`1px solid ${t==='Reveal key'?LN2:LN}`,background:t==='Reveal key'?BG3:W,color:t==='Reveal key'?T1:T2,cursor:'pointer',fontFamily:F}}>{t}</button>)}
          </div>
        </div>
      ))}
    </div>
    ...
    <div style={{padding:'12px'}}>
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:5,marginBottom:10}}>
        {[{d:'Mar 2026',v:'200d',c:RED},{d:'Mar 2027',v:'100d',c:AMB},{d:'Mar 2029',v:'47d',c:GRN}].map(m=>(
          <div key={m.d} style={{padding:'8px',borderRadius:3,background:BG3,border:`1px solid ${LN}`}}>
      ...
      {certs.map(c=>(
        <div key={c.d} style={{display:'flex',alignItems:'center',gap:9,padding:'7px 9px',borderRadius:3,background:BG3,marginBottom:5,border:`1px solid ${LN}`}}>
          ...
          <span style={{fontSize:10,fontWeight:500,color:c.c,background:c.c+'12',padding:'2px 7px',borderRadius:3,fontFamily:MONO}}>{c.label}</span>
        </div>
      ))}
    </div>
  </div>
    ...
    <div style={{fontFamily:F,background:BG1,color:T1,overflowX:'hidden'}}>
      ...
      <header style={{position:'sticky',top:0,zIndex:200,background:W,borderBottom:`1px solid ${LN}`,height:52,display:'flex',alignItems:'center',padding:'0 clamp(16px,4vw,40px)',justifyContent:'space-between'}}>
        ...
        {!isMobile&&<nav style={{display:'flex',alignItems:'center',gap:1}}>
          {[['Platform','#platform'],['Features','#features'],['Security','#security'],['Pricing','/pricing']].map(([l,h])=>(
            <button key={l} onClick={()=>h.startsWith('/')?nav(h):document.querySelector(h)?.scrollIntoView({behavior:'smooth'})}
              style={{background:'none',border:'none',cursor:'pointer',fontFamily:F,fontSize:12,color:T2,padding:'5px 11px',borderRadius:3,transition:'color .12s'}}
          ...
          <div style={{position:'relative'}}
            ...
            <button style={{background:'none',border:'none',cursor:'pointer',fontFamily:F,fontSize:12,color:T2,padding:'5px 11px',borderRadius:3,transition:'color .12s',display:'flex',alignItems:'center',gap:3}}
        ...
        <div style={{display:'flex',gap:8,alignItems:'center',flexShrink:0}}>
          {!isMobile&&<button onClick={()=>nav('/auth')} style={{background:'none',border:'none',cursor:'pointer',fontFamily:F,fontSize:12,color:T2,padding:'5px 10px',borderRadius:3,transition:'color .12s'}} onMouseEnter={e=>e.currentTarget.style.color=T1} onMouseLeave={e=>e.currentTarget.style.color=T2}>Sign in</button>}
          <button onClick={()=>nav('/auth')} style={{background:BK,border:'none',cursor:'pointer',fontFamily:F,fontSize:12,fontWeight:600,color:W,padding:'6px 14px',borderRadius:3,transition:'background .12s'}} onMouseEnter={e=>e.currentTarget.style.background='#222'} onMouseLeave={e=>e.currentTarget.style.background=BK}>Get started</button>
        </div>
      </header>

      {/* HERO */}
      <section style={{background:BG1,padding:'clamp(64px,9vw,110px) clamp(20px,5vw,48px) clamp(56px,7vw,90px)',borderBottom:`1px solid ${LN}`}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:isMobile?48:72,alignItems:'center'}}>
            <div>
              <div style={{display:'inline-flex',alignItems:'center',gap:6,border:`1px solid ${LN}`,borderRadius:100,padding:'4px 12px',marginBottom:24}}>
                <span style={{width:5,height:5,borderRadius:'50%',background:T3,animation:'blink 2.4s ease infinite'}}/>
                <span style={{fontSize:11,color:T3,fontFamily:MONO}}>RFC 8555 · CA/B Forum Compliant · PQC Intelligence</span>
              </div>
              <h1 style={{fontSize:'clamp(34px,5vw,64px)',fontWeight:700,letterSpacing:'-2px',lineHeight:1.07,color:T1,marginBottom:18}}>
                Certificate<br/><span style={{color:T3}}>lifecycle</span><br/>management.
              </h1>
              <p style={{fontSize:15,color:T2,lineHeight:1.8,marginBottom:28,maxWidth:420}}>
              ...
              <div style={{display:'flex',gap:10,flexWrap:'wrap',marginBottom:36}}>
                <BtnW label="Start managing certs" onClick={()=>nav('/auth')}/>
                <BtnG label="View pricing" onClick={()=>nav('/pricing')}/>
              </div>
              <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',borderTop:`1px solid ${LN}`,paddingTop:22}}>
                {[{val:certCount?`${count.toLocaleString()}+`:'—',label:'Certs managed'},{val:'99.9%',label:'Renewal success'},{val:'12',label:'PKI bodies tracked'}].map((m,i)=>(
                  <div key={m.label} style={{paddingLeft:i>0?18:0,borderLeft:i>0?`1px solid ${LN}`:'none',marginLeft:i>0?18:0}}>
                    <div style={{fontSize:20,fontWeight:700,color:T1,fontFamily:MONO,letterSpacing:'-0.5px',marginBottom:2}}>{m.val}</div>
            ...
            <div>
              <Term title="sslvault-agent · prod-server-01" lines={[
                ...
                {text:'↳ DNS provider: Cloudflare',c:'rgba(255,255,255,0.28)',indent:true},
                {text:'↳ Adding TXT _acme-challenge...',c:'rgba(255,255,255,0.22)',indent:true},
                {prompt:'›',text:'[21:05:15] DNS propagated · DCV validated ✓',c:'rgba(74,222,128,0.9)'},
                {prompt:'›',text:'[21:05:16] Cert issued · RapidSSL TLS RSA CA 2022',c:'rgba(74,222,128,0.9)'},
                {text:'↳ CN=easysecurity.in  valid 180d  grade A+',c:'rgba(255,255,255,0.22)',indent:true},
                {prompt:'›',text:'[21:05:17] nginx -t OK · systemctl reload nginx ✓',c:'rgba(74,222,128,0.9)'},
                {prompt:'›',text:'[21:05:18] CertVault: AES-256-GCM encrypted ✓',c:'rgba(255,255,255,0.6)'},
                {prompt:'›',text:'[21:05:18] ✓ Complete · next run: 21:10:18',c:'rgba(74,222,128,0.9)'},
              ]}/>
              {!isMobile&&<div style={{marginTop:6,display:'inline-flex',alignItems:'center',gap:8,border:`1px solid ${LN}`,borderRadius:3,padding:'7px 12px',background:BG3,fontFamily:MONO}}>
                <div style={{width:5,height:5,borderRadius:'50%',background:GRN}}/>
                <span style={{fontSize:11,color:T1}}>easysecurity.in</span>
                <span style={{fontSize:10,background:BG4,color:T2,padding:'1px 6px',borderRadius:3}}>A+</span>
      ...
      {/* TICKER */}
      <div style={{background:BG2,borderBottom:`1px solid ${LN}`,overflow:'hidden',padding:'7px 0'}}>
        <div style={{display:'flex',overflow:'hidden',maskImage:'linear-gradient(to right,transparent,black 8%,black 92%,transparent)'}}>
          <div style={{display:'flex',gap:48,flexShrink:0,animation:'ticker 36s linear infinite',whiteSpace:'nowrap'}}>
            {[...TICKER,...TICKER].map((p,i)=><span key={i} style={{fontSize:10.5,fontWeight:500,color:T3,letterSpacing:'0.05em',fontFamily:MONO}}><span style={{color:LN2,marginRight:8}}>◆</span>{p}</span>)}
          </div>
        </div>
      </div>

      {/* PLATFORM */}
      <section id="platform" style={{background:BG1,padding:'clamp(56px,7vw,88px) clamp(20px,4vw,48px)',borderTop:`1px solid ${LN}`}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          <FadeUp>
            <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:isMobile?36:72,alignItems:'flex-start',marginBottom:44}}>
              <div>
                <Eyebrow>What SSLVault does</Eyebrow>
                <H2 style={{marginBottom:12}}>The complete certificate lifecycle — fully automated.</H2>
              ...
              <div>
                {[{n:'/ 01',t:'Issue',d:'Submit to RapidSSL via ACME v2. DNS-01 auto-validated via your provider API.'},{n:'/ 02',t:'Install',d:'Agent deploys to Nginx/Apache. cPanel UAPI install. Zero SSH after setup.'},{n:'/ 03',t:'Monitor',d:'Expiry tracking, health scoring A–F, CT log abuse detection, CA/B Forum compliance.'},{n:'/ 04',t:'Renew',d:'Auto-renews 30 days before expiry. New cert deployed before old one expires.'}].map((s,i)=>(
                  <div key={s.n} style={{display:'flex',gap:14,padding:'14px 0',borderBottom:i<3?`1px solid ${LN}`:'none'}}>
                    <span style={{fontSize:11,color:T3,width:32,flexShrink:0,marginTop:1,fontFamily:MONO}}>{s.n}</span>
                    <div><div style={{fontSize:13,fontWeight:600,color:T1,marginBottom:3}}>{s.t}</div><div style={{fontSize:12.5,color:T2,lineHeight:1.6}}>{s.d}</div></div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <FadeUp>
            <div style={{fontSize:10,fontWeight:500,color:T3,fontFamily:MONO,marginBottom:8,textTransform:'uppercase',letterSpacing:'0.05em'}}>Certificate inventory — live expiry, health grade, auto-renew status</div>
            <InventoryMockup/>
          </FadeUp>
        </div>
      </section>
      ...
      {/* FEATURES */}
      <section id="features" style={{background:BG2,padding:'clamp(56px,7vw,88px) clamp(20px,4vw,48px)',borderTop:`1px solid ${LN}`}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          <FadeUp>
            <div style={{marginBottom:40,maxWidth:480}}>
              <Eyebrow>All capabilities</Eyebrow>
              <H2 style={{marginBottom:10}}>Every feature a PKI team needs.</H2>
              <Body>Eight core capability areas — built, not bolted on.</Body>
            </div>
          </FadeUp>
          <div style={{display:'grid',gridTemplateColumns:`repeat(${cols},1fr)`,gap:1,background:LN,border:`1px solid ${LN}`,borderRadius:4,overflow:'hidden'}}>
            {[{icon:'⚡',title:'Certificate issuance',specs:['DV, OV, EV, Wildcard, SAN','RapidSSL · DigiCert trust chain','ACME v2 · RFC 8555','Issued in < 5 minutes'],badge:'ACME v2'},{icon:'🤖',title:'Persistent agent',specs:['systemd daemon · polls every 5 min','Nginx + Apache auto-detect','Config test before reload','Outbound HTTPS only — no firewall changes'],badge:'systemd'},{icon:'🌐',title:'DNS automation',specs:['Cloudflare · Vercel · Route53','Namecheap · GoDaddy · DigitalOcean','Auto TXT/CNAME challenge','Cleanup after DCV completes'],badge:'DNS-01'},{icon:'🏛',title:'cPanel install',specs:['UAPI-based installation','No SSH or agent required','API token auth','Auto-renew via cPanel'],badge:'UAPI'},{icon:'🔐',title:'CertVault',specs:['AES-256-GCM · envelope encryption','Password re-auth before reveal','30-day rotation archive','Immutable audit log → CSV'],badge:'AES-256'},{icon:'📋',title:'47-day readiness',specs:['Scores every cert 0–100','200d → 100d → 47d mandate timeline','Per-cert automation checklist','Fleet-wide compliance report'],badge:'CA/B 2026'},{icon:'🔍',title:'Discovery & monitoring',specs:['CT log scan via crt.sh','CT abuse monitor — unknown CAs','SSL health score A+ to F','HSTS · CAA · TLS 1.3 checks'],badge:'CT Logs'},{icon:'📈',title:'CA intelligence',specs:['DigiCert CertCentral sync','Sectigo SCM portfolio sync','Shadow IT detection','Policy engine · fleet compliance'],badge:'Multi-CA'},{icon:'🏛',title:'Industry Intelligence',specs:['6,200+ CAs from CCADB live','CAB Forum ballot tracker','12 PKI bodies deep-dive','PQC migration tracker (ML-KEM, ML-DSA)'],badge:'PKI Hub'},{icon:'🔬',title:'CA Trust Store',specs:['Every root & intermediate CA indexed','PKI Trust Score per certificate','Filter by trust store · algorithm · region','CSV export · PEM download'],badge:'CCADB'}].map(f=>(
              <FadeUp key={f.title}>
                <div style={{background:BG1,padding:'18px',height:'100%'}}>
                  <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:10}}>
                    <span style={{fontSize:20}}>{f.icon}</span>
                    <Tag>{f.badge}</Tag>
                  </div>
                  <div style={{fontSize:13,fontWeight:600,color:T1,marginBottom:9}}>{f.title}</div>
                  <div style={{display:'flex',flexDirection:'column',gap:5}}>
                    {f.specs.map(s=><div key={s} style={{display:'flex',alignItems:'flex-start',gap:7}}>
      ...
      {/* CERTVAULT + READINESS */}
      <section style={{background:BG1,padding:'clamp(56px,7vw,88px) clamp(20px,4vw,48px)',borderTop:`1px solid ${LN}`}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          <FadeUp>
            <div style={{marginBottom:40}}>
              <Eyebrow>Security controls</Eyebrow>
              <H2 style={{marginBottom:10}}>Enterprise PKI controls. Not an afterthought.</H2>
              <Body>CertVault and 47-Day Readiness are built into every account — not a paid add-on.</Body>
            </div>
          </FadeUp>
          <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:16}}>
            <FadeUp><div><div style={{fontSize:10,fontWeight:500,color:T3,fontFamily:MONO,textTransform:'uppercase',letterSpacing:'0.05em',marginBottom:8}}>🔐 AES-256-GCM private key vault</div><CertVaultMockup/></div></FadeUp>
            <FadeUp delay={80}><div><div style={{fontSize:10,fontWeight:500,color:T3,fontFamily:MONO,textTransform:'uppercase',letterSpacing:'0.05em',marginBottom:8}}>📋 CA/B Forum compliance scoring</div><ReadinessMockup/></div></FadeUp>
          </div>
        </div>
      </section>

      {/* SECURITY SPECS */}
      <section id="security" style={{background:BG2,padding:'clamp(56px,7vw,88px) clamp(20px,4vw,48px)',borderTop:`1px solid ${LN}`}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          <FadeUp>
            <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:isMobile?36:72,alignItems:'flex-start',marginBottom:40}}>
              <div>
                <Eyebrow>Security specifications</Eyebrow>
                <H2 style={{marginBottom:12}}>Open standards throughout. No black boxes.</H2>
                <Body style={{marginBottom:18}}>Every layer of SSLVault is built on auditable open standards. RFC 8555 for issuance, AES-256-GCM for storage, CT logs for transparency.</Body>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(min(300px,100%),1fr))',gap:1,background:LN,border:`1px solid ${LN}`,borderRadius:4,overflow:'hidden'}}>
                {[{spec:'AES-256-GCM',title:'Key encryption',desc:'DEK wrapped with KEK. Keys never in plaintext.'},{spec:'RFC 8555',title:'ACME v2',desc:'DNS-01 challenge. Auto-validated via provider API.'},{spec:'CT monitoring',title:'Cert transparency',desc:'crt.sh queries for every cert ever issued.'},{spec:'CAA + HSTS',title:'DNS security',desc:'CAA prevents unauthorised CA issuance.'},{spec:'TLS 1.2 / 1.3',title:'TLS posture',desc:'ECDHE + PFS. HSTS max-age verified.'},{spec:'Append-only',title:'Audit trail',desc:'Every access logged. CSV export for SOC 2.'}].map(s=>(
                  <div key={s.spec} style={{background:BG1,padding:'13px'}}>
                    <div style={{fontSize:9.5,fontWeight:600,color:T3,fontFamily:MONO,letterSpacing:'0.05em',marginBottom:4}}>{s.spec}</div>
                    <div style={{fontSize:12,fontWeight:600,color:T1,marginBottom:3}}>{s.title}</div>
          ...
          <FadeUp>
            <Card>
              <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:22}}>
                <div style={{width:6,height:6,borderRadius:'50%',background:T3,animation:'blink 2s ease infinite'}}/>
              ...
              <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'repeat(3,1fr)',gap:isMobile?18:0}}>
                {[{date:'March 15, 2026',limit:'200 days',status:'IMMINENT',color:RED,action:'Certificates issued with validity > 200 days will be rejected by all major browsers. Audit your fleet immediately.'},{date:'March 15, 2027',limit:'100 days',status:'UPCOMING',color:AMB,action:'Manual renewal every 100 days is operationally unsustainable. Automation becomes a hard requirement.'},{date:'March 15, 2029',limit:'47 days',status:'PLANNED',color:GRN,action:"Full zero-touch automation required. SSLVault's agent + DNS automation handles this end-to-end today."}].map((m,i)=>(
                  <div key={m.date} style={{padding:isMobile?'14px 0':'0 22px',borderLeft:!isMobile&&i>0?`1px solid ${LN}`:'none'}}>
                    <div style={{fontSize:9.5,fontWeight:700,color:m.color,fontFamily:MONO,letterSpacing:'0.08em',marginBottom:5}}>{m.status}</div>
                    <div style={{fontSize:10,color:T3,fontFamily:MONO,marginBottom:3}}>{m.date}</div>
                    <div style={{fontSize:28,fontWeight:700,color:m.color,fontFamily:MONO,letterSpacing:'-1px',marginBottom:8}}>{m.limit}</div>
          ...
          <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:isMobile?36:72,alignItems:'center'}}>
            <FadeUp>
              <div>
                ...
                <H2 style={{marginBottom:12}}>Automated DNS-01 challenge across every major provider.</H2>
                <Body style={{marginBottom:20}}>When issuing or renewing, SSLVault calls your DNS provider API to add the ACME challenge record, polls for propagation, validates, then cleans up — fully automatic, zero copy-paste.</Body>
                <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(min(300px,100%),1fr))',gap:4}}>
                  {[['Cloudflare','API Token · Zone:DNS:Edit'],['Vercel','Access Token · Settings → Tokens'],['Route53','AWS IAM · Route53 write access'],['Namecheap','API Key · IP whitelist required'],['GoDaddy','API Key + Secret'],['DigitalOcean','Personal Access Token'],['Plesk','XML API'],['+ more','Contact us']].map(([name,note])=>(
                    <div key={name} style={{display:'flex',alignItems:'center',gap:9,padding:'8px 10px',background:BG3,border:`1px solid ${LN}`,borderRadius:3}}>
            ...
            <FadeUp delay={80}>
              <Term title="dns-provider · Cloudflare API" lines={[
                ...
                {prompt:'›',text:'Resolving zone: myshop.com',c:'rgba(255,255,255,0.5)'},
                {prompt:'›',text:'PUT /zones/{id}/dns_records',c:'rgba(255,255,255,0.75)'},
                {text:'↳ type:TXT  name:_acme-challenge',c:'rgba(255,255,255,0.28)',indent:true},
                {text:'↳ content:xK3-mP9_aQ2rZ...  ttl:60',c:'rgba(255,255,255,0.22)',indent:true},
                {prompt:'›',text:'Record created · TTL 60s',c:'rgba(74,222,128,0.9)'},
                {prompt:'›',text:'Propagated in 4.2s ✓',c:'rgba(74,222,128,0.9)'},
                {prompt:'›',text:'ACME challenge: verified ✓',c:'rgba(74,222,128,0.9)'},
      ...
      {/* ARCHITECTURE */}
      <section style={{background:BG2,padding:'clamp(56px,7vw,88px) clamp(20px,4vw,48px)',borderTop:`1px solid ${LN}`}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          <FadeUp>
            <div style={{textAlign:'center',marginBottom:40}}>
              <Eyebrow>How it works</Eyebrow>
              <H2 style={{maxWidth:480,margin:'8px auto 10px'}}>From CSR to live HTTPS in one automated pipeline.</H2>
          ...
          <FadeUp>
            <div style={{display:'grid',gridTemplateColumns:`repeat(${isMobile?1:5},1fr)`,gap:isMobile?8:0,marginBottom:40,alignItems:'center'}}>
              {[{n:'/ 01',icon:'🖥',title:'Issue request',desc:'Select domain + cert type. SSLVault generates CSR.'},null,{n:'/ 02',icon:'🌐',title:'Auto DCV',desc:'DNS provider API adds ACME challenge. Auto-validated.'},null,{n:'/ 03',icon:'🏛',title:'CA issues cert',desc:'RapidSSL signs cert. Stored encrypted in CertVault.'}].map((s,i)=>{
                ...
                return <Card key={s.n} style={{textAlign:'center',padding:'16px'}}>
                  <div style={{fontSize:20,marginBottom:7}}>{s.icon}</div>
                  <div style={{fontSize:10,color:T3,fontFamily:MONO,marginBottom:4}}>{s.n}</div>
        ...
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          <FadeUp>
            <div style={{marginBottom:40}}>
              ...
              <H2 style={{marginBottom:10,maxWidth:520}}>The deepest PKI intelligence platform on the web.</H2>
          ...
          <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'repeat(3,1fr)',gap:1,background:LN,border:`1px solid ${LN}`,borderRadius:4,overflow:'hidden',marginBottom:12}}>
            {[{icon:'🏛',title:'CA Trust Store',sub:'6,200+ root & intermediate CAs',desc:'Every CA in Chrome, Firefox, Apple, and Microsoft trust stores — live from CCADB. Search by operator, algorithm, region. PKI Trust Score per cert. PEM download.',badge:'CCADB Live',path:'/ca-trust-explorer',stats:[['6,200+','CAs indexed'],['4','Trust stores'],['Daily','CCADB sync']]},{icon:'⚖️',title:'CAB Forum Intelligence',sub:'Ballots, timelines & compliance',desc:'Every CAB Forum ballot tracked with plain-English summaries. 47-day countdown, SC081v3 compliance deadlines, 5 working groups, full PKI history timeline from 2005.',badge:'Live sync',path:'/cab-forum',stats:[['47-day','2029 mandate'],['5','Working groups'],['Real-time','Ballot feed']]},{icon:'🌍',title:'Global PKI Hub',sub:'12 bodies · 22 standards · PQC tracker',desc:'CAB Forum, ETSI ESI, NIST, IETF, APKIC, eIDAS 2.0, PKI Consortium, CSC, FIDO, WebTrust, CCADB, ITU-T — each with deep-dive pages, standards library, and PQC migration status.',badge:'PQC Ready',path:'/pki-hub',stats:[['12','PKI bodies'],['3','NIST PQC finals'],['2026','Amsterdam conf.']]}].map(item=>(
              <FadeUp key={item.title}>
                <div onClick={()=>nav(item.path)} style={{background:BG1,padding:'20px',cursor:'pointer',transition:'background .12s',height:'100%',display:'flex',flexDirection:'column',gap:12}}
                  onMouseEnter={e=>e.currentTarget.style.background=BG3} onMouseLeave={e=>e.currentTarget.style.background=BG1}>
                  ...
                  <div>
                    ...
                    <div style={{fontSize:10.5,color:T3,fontFamily:MONO,marginBottom:7}}>{item.sub}</div>
                    <div style={{fontSize:12,color:T2,lineHeight:1.7}}>{item.desc}</div>
                  </div>
                  <div style={{display:'flex',borderTop:`1px solid ${LN}`,paddingTop:10,marginTop:'auto'}}>
                    {item.stats.map(([val,label],i)=>(
                      <div key={label} style={{flex:1,paddingLeft:i>0?10:0,borderLeft:i>0?`1px solid ${LN}`:'none',marginLeft:i>0?10:0}}>
          ...
          <FadeUp>
            <Card style={{display:'flex',gap:16,alignItems:'flex-start',flexWrap:'wrap'}}>
              ...
              <div style={{flex:1,minWidth:240}}>
                <div style={{fontSize:13,fontWeight:600,color:T1,marginBottom:3}}>Post-Quantum Cryptography migration tracker — live</div>
                <div style={{fontSize:12,color:T2,lineHeight:1.7}}>NIST finalised ML-KEM (FIPS 203), ML-DSA (FIPS 204), and SLH-DSA (FIPS 205) in August 2024. The PKI Hub tracks every body's PQC readiness — from IETF LAMPS WG certificate profiles to ETSI EN 319 updates — so you know exactly where the industry stands.</div>
              </div>
              <div style={{display:'flex',gap:6,flexWrap:'wrap',alignItems:'center',flexShrink:0}}>
                {[['ML-KEM','FIPS 203'],['ML-DSA','FIPS 204'],['SLH-DSA','FIPS 205']].map(([alg,fips])=>(
                  <div key={alg} style={{background:BG4,border:`1px solid ${LN}`,borderRadius:3,padding:'6px 10px',textAlign:'center'}}>
                ...
                <button onClick={()=>nav('/pki-hub')} style={{background:BK,border:'none',borderRadius:3,padding:'7px 14px',fontSize:12,fontWeight:600,color:W,cursor:'pointer',fontFamily:F,transition:'background .12s'}} onMouseEnter={e=>e.currentTarget.style.background='#222'} onMouseLeave={e=>e.currentTarget.style.background=BK}>View PQC tracker →</button>
              </div>
            </Card>
          </FadeUp>
        </div>
        ...
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          <FadeUp>
            <div style={{display:'flex',alignItems:'center',gap:9,marginBottom:16}}>
              <span style={{fontSize:10,fontWeight:600,color:RED,letterSpacing:'0.07em',textTransform:'uppercase',fontFamily:MONO,background:'rgba(220,38,38,0.06)',border:'1px solid rgba(220,38,38,0.15)',borderRadius:3,padding:'2px 9px'}}>Industry first</span>
            ...
            <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:isMobile?36:72,alignItems:'center',marginBottom:40}}>
              <div>
                ...
                <p style={{fontSize:14,color:T2,lineHeight:1.8,marginBottom:24,maxWidth:440}}>CertBind closes that gap with continuous, cryptographic proof — every 5 minutes.</p>
                <div style={{display:'flex',gap:10}}><BtnW label="See CertBind" onClick={()=>nav('/auth')}/><BtnG label="Read how it works" onClick={()=>nav('/knowledge-base')}/></div>
              </div>
      ...
      <section style={{background:BG1,padding:'clamp(56px,7vw,88px) clamp(20px,4vw,48px)',borderTop:`1px solid ${LN}`}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:isMobile?36:72,alignItems:'flex-start'}}>
            <FadeUp>
              <div>
                <Eyebrow>Why we built this</Eyebrow>
                <H2 style={{marginBottom:12}}>PKI expertise shouldn't require a $250k contract.</H2>
                ...
                <Body style={{marginBottom:24}}>As CA/B Forum mandates tighten (200d → 100d → 47d between 2026–2029), full automation becomes non-negotiable. SSLVault is built for what's coming.</Body>
                <div style={{display:'flex',gap:10}}><BtnW label="Get started" onClick={()=>nav('/auth')}/><BtnG label="View pricing" onClick={()=>nav('/pricing')}/></div>
              </div>
            </FadeUp>
            <FadeUp delay={80}>
              <div style={{display:'flex',flexDirection:'column',gap:5}}>
                {[{name:'Venafi TLS Protect',price:'$250k+/yr',notes:'Enterprise only · No cert issuance · No cPanel',hi:false},{name:'Keyfactor Command',price:'$75–200k/yr',notes:'Mid-market · Complex setup · No free tier',hi:false},{name:'SSLVault CLM',price:'Partner rates',notes:'Full CLM · Agent + cPanel + DNS · All cert types',hi:true}].map(c=>(
                  <div key={c.name} style={{display:'flex',alignItems:'center',gap:12,padding:'11px 13px',background:c.hi?BG3:W,border:`1px solid ${c.hi?LN2:LN}`,borderRadius:4}}>
                ...
                <div style={{marginTop:5,display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(min(300px,100%),1fr))',gap:4}}>
                  {[['DigiCert','Trust chain · 99.9% browser'],['RapidSSL','CA partner · wholesale pricing'],['RFC 8555','ACME v2 · no lock-in'],['AES-256','Military-grade key storage'],['GDPR','Netherlands-based PKI engineer 🇳🇱'],['No ads','No tracking · no reselling']].map(([val,sub])=>(
                    <div key={val} style={{display:'flex',gap:9,padding:'8px 10px',background:BG3,border:`1px solid ${LN}`,borderRadius:3,alignItems:'center'}}>
                      <div style={{fontSize:11,fontWeight:600,color:T1,fontFamily:MONO,minWidth:62}}>{val}</div>
      ...
      <section style={{background:BG2,padding:'clamp(56px,7vw,88px) clamp(20px,4vw,48px)',borderTop:`1px solid ${LN}`}}>
        <div style={{maxWidth:580,margin:'0 auto',textAlign:'center'}}>
          <FadeUp>
            <div style={{display:'inline-flex',alignItems:'center',gap:6,border:`1px solid ${LN}`,borderRadius:100,padding:'4px 12px',marginBottom:20}}>
              <span style={{width:5,height:5,borderRadius:'50%',background:T3,animation:'blink 2.4s ease infinite'}}/>
            ...
            <p style={{fontSize:14,color:T2,lineHeight:1.75,maxWidth:420,margin:'0 auto 26px'}}>
      ...
      <footer style={{background:DK,borderTop:'1px solid rgba(255,255,255,0.07)',padding:'clamp(36px,5vw,52px) clamp(20px,4vw,48px) clamp(22px,3vw,32px)'}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:32,paddingBottom:24,borderBottom:'1px solid rgba(255,255,255,0.07)'}}>
            ...
            <span style={{fontSize:11,color:'rgba(255,255,255,0.28)',fontFamily:MONO,marginLeft:4}}>PKI-first CLM · Built by a real PKI engineer</span>
          </div>
          <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr 1fr':'repeat(auto-fill,minmax(140px,1fr))',gap:24,marginBottom:32}}>
            {[{title:'Product',links:[['Pricing',()=>nav('/pricing')],['Get started',()=>nav('/auth')],['Dashboard',()=>nav('/dashboard')]]},{title:'Support Resources',links:[['Install Guide',()=>nav('/install')],['Knowledge Base',()=>nav('/knowledge-base')],['CA Intelligence',()=>nav('/ca-intelligence')],['CAA Checker',()=>nav('/caa-check')]]},{title:'Industry Intelligence',links:[['CA Trust Store',()=>nav('/ca-trust-explorer')],['CAB Forum',()=>nav('/cab-forum')],['PKI Hub',()=>nav('/pki-hub')],['Trust Passport',()=>nav('/trust-passport')]]},{title:'Security',links:[['CertVault','#security'],['47-Day Readiness','#security'],['CT Monitoring','#security'],['Health Scoring','#security']]},{title:'Protocol',links:[['RFC 8555 ACME','#security'],['DNS-01 Challenge','#security'],['AES-256-GCM','#security']]},{title:'Company',links:[['About',()=>nav('/about')],['Developer',()=>nav('/developer')]]}].map(col=>(
              <div key={col.title}>
                <div style={{fontSize:10,fontWeight:600,color:'rgba(255,255,255,0.3)',textTransform:'uppercase',letterSpacing:'0.07em',fontFamily:MONO,marginBottom:12}}>{col.title}</div>
                {col.links.map(([l,h])=>(
                  <div key={l} style={{marginBottom:8}}>
                    <button onClick={()=>typeof h==='function'?h():document.querySelector(h)?.scrollIntoView({behavior:'smooth'})}
                      style={{background:'none',border:'none',cursor:'pointer',fontSize:12,color:'rgba(255,255,255,0.32)',fontFamily:F,padding:0,transition:'color .12s',textAlign:'left'}}
                      onMouseEnter={e=>e.currentTarget.style.color='rgba(255,255,255,0.75)'} onMouseLeave={e=>e.currentTarget.style.color='rgba(255,255,255,0.32)'}>{l}</button>
                  </div>
                ))}
              </div>
            ))}
          ...
          <div style={{borderTop:'1px solid rgba(255,255,255,0.07)',paddingTop:18,display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10}}>
            <span style={{fontSize:11,color:'rgba(255,255,255,0.22)',fontFamily:MONO}}>PKI-first CLM · Made with ♥ towards PKI · Built by a real PKI engineer</span>
            <span style={{fontSize:11,color:'rgba(255,255,255,0.22)'}}>© 2026 SSLVault. All rights reserved.</span>
          </div>
        </div>
      </footer>
    </div>
