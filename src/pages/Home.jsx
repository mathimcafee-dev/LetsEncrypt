...
function Btn({ label, onClick, primary, dark }) {
  ...
  if (primary) return (
    <button onClick={onClick} onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
      style={{ display:'inline-flex', alignItems:'center', gap:7, fontFamily:F, fontWeight:500, fontSize:14, padding:'9px 22px', borderRadius:100, border:'none', cursor:'pointer', background:h?D.tealDk:D.teal, color:'white', transition:'all .15s', letterSpacing:'-0.01em' }}>
...
function Terminal({ title, lines }) {
  return (
    <div style={{ background:D.bg2, border:`1px solid ${D.bd}`, borderRadius:12, overflow:'hidden', fontFamily:MONO, boxShadow:'0 20px 60px rgba(0,0,0,0.4)' }}>
...
export default function Home({ nav }) {
  ...
  return (
    <div style={{ fontFamily:F, background:W.bg, color:W.heading, overflowX:'hidden' }}>
      ...
      {/* ── NAV ── */}
      <header style={{ position:'sticky', top:0, zIndex:200, background:'rgba(8,12,20,0.8)', backdropFilter:'blur(28px)', WebkitBackdropFilter:'blur(28px)', borderBottom:'1px solid rgba(255,255,255,0.06)', height:60, display:'flex', alignItems:'center', padding:`0 clamp(16px,4vw,40px)`, justifyContent:'space-between' }}>
        <div style={{ display:'flex', alignItems:'center', gap:9, cursor:'pointer', flexShrink:0 }} onClick={() => nav('/')}>
          <div style={{ width:30, height:30, background:D.teal, borderRadius:8, display:'flex', alignItems:'center', justifyContent:'center' }}>
      ...
      {/* ── HERO ── */}
      <section style={{ background:D.bg, padding:`clamp(80px,10vw,130px) clamp(20px,5vw,48px) clamp(70px,9vw,110px)`, position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ position:'absolute', top:'-10%', left:'50%', transform:'translateX(-50%)', width:900, height:500, background:`radial-gradient(ellipse 60% 55% at 50% 0%, rgba(14,165,233,0.12) 0%, transparent 100%)`, pointerEvents:'none' }}/>
            ...
            <div style={{ position:'relative' }}>
              ...
              <div style={{ position:'relative' }}>
                <Terminal title="sslvault-agent · prod-server-01" lines={[
                  ...
                  { prompt:'›', text:'[21:05:13] Job received: renew · easysecurity.in', c:'rgba(14,165,233,0.9)' },
      ...
      {/* ── TICKER ── */}
      <div style={{ background:W.bg3, borderTop:`1px solid ${W.border}`, borderBottom:`1px solid ${W.border}`, overflow:'hidden', padding:'10px 0' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:isMobile?'1fr':'1fr 1fr', gap:isMobile?40:80, alignItems:'flex-start', marginBottom:52 }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
                {[
                  ...
                  { spec:'TLS 1.2 / 1.3',    title:'TLS posture',       color:'#06b6d4',desc:'ECDHE + PFS. HSTS max-age verified.' },
      ...
      {/* ── CA TRUST EXPLORER ── */}
      <CATrustExplorerSection isMobile={isMobile} />
          ...
          <div style={{ display:'grid', gridTemplateColumns:isMobile?'1fr':'1fr 1fr', gap:isMobile?40:80, alignItems:'center' }}>
            ...
            <FadeUp delay={80}>
              <Terminal title="dns-provider · Cloudflare API" lines={[
                ...
                { prompt:'›', text:'PUT /zones/{id}/dns_records', c:'rgba(14,165,233,0.9)' },
                ...
                { prompt:'›', text:'Challenge record removed ✓', c:'rgba(14,165,233,0.9)' },
              ]}/>
            </FadeUp>
          </div>
        </div>
      ...
      {/* ── FOOTER ── */}
      <footer style={{ background:D.bg, borderTop:'1px solid rgba(255,255,255,0.06)', padding:`clamp(48px,6vw,64px) clamp(20px,4vw,48px) clamp(28px,3vw,40px)` }}>
        <div style={{ maxWidth:1100, margin:'0 auto' }}>
...
// ── CA Trust Explorer section (landing page embed) ────────────────────
function CATrustExplorerSection({ isMobile }) {
  ...
  const COLS = 'id,ca_owner,common_name,cert_type,status,key_algorithm,valid_from,valid_to,sha256_fingerprint,country,mozilla_trusted,microsoft_trusted,apple_trusted,chrome_trusted,ev_capable'

  useEffect(() => {
    supabase.from('ca_certificates').select(COLS).order('ca_owner', { ascending: true }).limit(5000)
      .then(({ data }) => {
        setCerts(data || [])
        setFiltered(data || [])
  ...
  const scoreColor = (n) => n >= 80 ? W.teal : n >= 50 ? W.amber : W.red
  const computeScore = (c) => {
    if (!c) return null
    let s = 100
    if (c.status === 'Revoked' || c.status === 'Distrusted') s -= 60
    else if (c.status === 'Expiring' || c.status === 'Expired') s -= 25
    const algo = (c.key_algorithm||'').toLowerCase()
    if (algo.includes('sha1') || algo.includes('md5')) s -= 30
    const validTo = c.valid_to ? new Date(c.valid_to) : null
    if (validTo) {
      const days = (validTo - new Date()) / 86400000
      if (days < 0) s -= 20
      else if (days < 365) s -= 8
    }
    if (!c.mozilla_trusted && !c.chrome_trusted && !c.apple_trusted && !c.microsoft_trusted) s -= 10
    return Math.max(0, Math.min(100, s))
  }

  const statusDot = (s) => s === 'Active' ? W.green : s === 'Revoked' || s === 'Distrusted' ? W.red : W.amber

  const doSearch = (q, type) => {
    const lq = q.toLowerCase()
    let res = certs
    if (type === 'root') res = res.filter(c => c.cert_type === 'Root CA')
    else if (type === 'inter') res = res.filter(c => c.cert_type !== 'Root CA')
    else if (type === 'ev') res = res.filter(c => c.ev_capable)
    if (lq) res = res.filter(c => (c.common_name||'').toLowerCase().includes(lq) || (c.ca_owner||'').toLowerCase().includes(lq))
    setFiltered(res)
  ...
  const stats = {
    total: certs.length,
    roots: certs.filter(c => c.cert_type === 'Root CA').length,
    inter: certs.filter(c => c.cert_type !== 'Root CA').length,
    ev:    certs.filter(c => c.ev_capable).length,
    dist:  certs.filter(c => c.status === 'Distrusted').length,
  }

  const score = selected ? computeScore(selected) : null
  const storeItems = [
    { name: 'Chrome',    trusted: selected?.chrome_trusted },
    { name: 'Mozilla',   trusted: selected?.mozilla_trusted },
    { name: 'Apple',     trusted: selected?.apple_trusted },
    { name: 'Microsoft', trusted: selected?.microsoft_trusted },
  ]
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        <FadeUp>
          <div style={{ textAlign: 'center', marginBottom: 48 }}>
            <SectionLabel color={W.green}>Live PKI intelligence</SectionLabel>
            <H2 style={{ marginBottom: 14 }}>Global CA Trust Store — every publicly-trusted certificate.</H2>
            <Body style={{ maxWidth: 580, margin: '0 auto 28px' }}>
              Every Root and Intermediate CA approved by the CAB Forum, sourced live from CCADB. Search, inspect trust chains, and verify PKI health — right here.
            </Body>
            {/* Live stat pills */}
            <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
              {[
                { label: `${stats.total.toLocaleString()} total`, color: W.teal, bg: W.tealBg, border: W.tealBd },
                { label: `${stats.roots} root CAs`, color: W.green, bg: W.greenBg, border: '#bbf7d0' },
                { label: `${stats.inter.toLocaleString()} intermediates`, color: W.body, bg: W.bg3, border: W.border },
                { label: `${stats.ev} EV capable`, color: W.purple, bg: W.purpleBg, border: '#ddd6fe' },
                { label: `${stats.dist} distrusted`, color: W.red, bg: W.redBg, border: '#fecaca' },
              ].map(({ label, color, bg, border }) => (
                <span key={label} style={{ fontSize: 11, fontWeight: 600, color, background: bg, border: `1px solid ${border}`, borderRadius: 20, padding: '4px 12px', fontFamily: MONO }}>
                  {label}
                </span>
              ))}
            </div>
        ...
        <FadeUp delay={80}>
          ...
          <div style={{ display: 'flex', gap: 4, marginBottom: 14, flexWrap: 'wrap' }}>
            {[
              ...
              <button key={key}
                onClick={() => { setActiveType(key); doSearch(query, key) }}
                style={{
                  fontSize: 12, fontWeight: 600, padding: '5px 14px', borderRadius: 6, cursor: 'pointer',
                  fontFamily: F, border: `1px solid ${activeType === key ? W.teal : W.border}`,
                  background: activeType === key ? W.tealBg : W.bg,
                  color: activeType === key ? W.teal : W.body,
                  transition: 'all .12s'
                }}>
                {label}
              </button>
            ...
            {/* Search */}
            <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8, background: W.bg, border: `1px solid ${W.border2}`, borderRadius: 8, padding: '6px 12px', minWidth: 220 }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={W.muted} strokeWidth="2.5" strokeLinecap="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
              <input
                type="text" placeholder="Search CA name or owner…" value={query}
                onChange={e => { setQuery(e.target.value); doSearch(e.target.value, activeType) }}
                style={{ border: 'none', outline: 'none', background: 'transparent', fontSize: 12, color: W.heading, fontFamily: F, flex: 1, minWidth: 0 }}
              />
            </div>
          </div>

          {/* Main split panel */}
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '340px 1fr', gap: 16, background: W.bg, border: `1px solid ${W.border}`, borderRadius: 14, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>

            {/* Left: cert list */}
            <div style={{ borderRight: isMobile ? 'none' : `1px solid ${W.border}`, height: 480, overflowY: 'auto' }}>
              {loading ? (
                <div style={{ padding: 20 }}>
                  {[...Array(8)].map((_, i) => (
                    <div key={i} style={{ height: 14, background: W.bg3, borderRadius: 6, marginBottom: 12, width: i % 2 === 0 ? '75%' : '55%', animation: 'shimmer 1.4s infinite' }} />
                  ))}
                </div>
              ) : filtered.length === 0 ? (
                <div style={{ padding: 40, textAlign: 'center', color: W.muted, fontSize: 13 }}>No certificates found</div>
              ) : filtered.map((c) => {
                ...
                return (
                  <div key={c.id} onClick={() => setSelected(c)}
                    style={{
                      display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px 10px 16px',
                      borderBottom: `1px solid ${W.border}`, cursor: 'pointer',
                      background: isSelected ? W.tealBg : 'transparent',
                      borderLeft: `2px solid ${isSelected ? W.teal : 'transparent'}`,
                      transition: 'background .1s'
                    }}>
                    {/* Avatar */}
                    <div style={{
                      width: 28, height: 28, borderRadius: 6, flexShrink: 0, background: W.teal,
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      fontSize: 10, fontWeight: 700, color: '#fff'
                    }}>
                      {(c.ca_owner||c.common_name||'?').slice(0,2).toUpperCase()}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 11.5, fontWeight: 600, color: W.heading, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginBottom: 2 }}>
                        {c.common_name || c.ca_owner}
                      </div>
                      <div style={{ fontSize: 10.5, color: W.muted, display: 'flex', gap: 6, alignItems: 'center' }}>
                        <span style={{ width: 5, height: 5, borderRadius: '50%', background: statusDot(c.status), flexShrink: 0, display: 'inline-block' }} />
                        <span>{c.ca_owner?.slice(0,22)}{(c.ca_owner||'').length > 22 ? '…' : ''}</span>
                        <span style={{ background: c.cert_type === 'Root CA' ? W.tealBg : W.bg3, color: c.cert_type === 'Root CA' ? W.teal : W.body, border: `1px solid ${c.cert_type === 'Root CA' ? W.tealBd : W.border}`, borderRadius: 4, padding: '1px 5px', fontSize: 9, fontWeight: 600 }}>
                          {c.cert_type === 'Root CA' ? 'Root' : 'Int.'}
                        </span>
                      </div>
                    </div>
                    <div style={{ fontSize: 11, fontWeight: 700, color: scoreColor(sc), flexShrink: 0, fontFamily: MONO }}>{sc}</div>
                  </div>
                )
              })}
            </div>
            ...
            {/* Right: detail panel */}
            {selected ? (
              <div style={{ padding: 24, height: 480, overflowY: 'auto' }}>
                {/* Header */}
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 18, gap: 12, flexWrap: 'wrap' }}>
                  <div>
                    <div style={{ fontSize: 16, fontWeight: 700, color: W.heading, letterSpacing: '-0.2px', marginBottom: 6 }}>{selected.common_name || selected.ca_owner}</div>
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                      <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 4, background: selected.cert_type === 'Root CA' ? W.tealBg : W.bg3, color: selected.cert_type === 'Root CA' ? W.teal : W.body, border: `1px solid ${selected.cert_type === 'Root CA' ? W.tealBd : W.border}` }}>
                        {selected.cert_type}
                      </span>
                      <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 4,
                        background: selected.status === 'Active' ? W.greenBg : selected.status === 'Revoked' || selected.status === 'Distrusted' ? W.redBg : W.amberBg,
                        color: selected.status === 'Active' ? W.green : selected.status === 'Revoked' || selected.status === 'Distrusted' ? W.red : W.amber,
                        border: `1px solid ${selected.status === 'Active' ? '#bbf7d0' : selected.status === 'Revoked' || selected.status === 'Distrusted' ? '#fecaca' : '#fde68a'}`
                      }}>{selected.status}</span>
                      {selected.ev_capable && <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 4, background: W.purpleBg, color: W.purple, border: '1px solid #ddd6fe' }}>EV capable</span>}
                    </div>
                  </div>
                  {/* PKI Score */}
                  <div style={{ background: score >= 80 ? W.tealBg : score >= 50 ? W.amberBg : W.redBg, border: `1px solid ${score >= 80 ? W.tealBd : score >= 50 ? '#fde68a' : '#fecaca'}`, borderRadius: 10, padding: '10px 16px', textAlign: 'center', flexShrink: 0 }}>
                    <div style={{ fontSize: 26, fontWeight: 800, color: scoreColor(score), fontFamily: MONO, lineHeight: 1, letterSpacing: '-1px' }}>{score}</div>
                    <div style={{ fontSize: 10, color: W.muted, marginTop: 2, fontWeight: 600 }}>PKI Score</div>
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 24px', marginBottom: 18 }}>
                  {[
                    ...
                    { k: 'Key algo',   v: selected.key_algorithm || '—' },
                    { k: 'Valid from', v: fmtDate(selected.valid_from) },
                    { k: 'Valid to',   v: fmtDate(selected.valid_to) },
                  ].map(({ k, v }) => (
                    <div key={k} style={{ borderBottom: `1px solid ${W.border}`, paddingBottom: 7 }}>
                      <div style={{ fontSize: 10, color: W.muted, marginBottom: 2, fontWeight: 500 }}>{k}</div>
                      <div style={{ fontSize: 12, color: W.heading, fontWeight: 600 }}>{v || '—'}</div>
                    </div>
                  ))}
                </div>
                ...
                {selected.sha256_fingerprint && (
                  <div style={{ marginBottom: 18 }}>
                    <div style={{ fontSize: 10, color: W.muted, fontWeight: 600, marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.06em' }}>SHA-256 Fingerprint</div>
                    <div style={{ fontSize: 9.5, fontFamily: MONO, color: W.body, background: W.bg3, border: `1px solid ${W.border}`, borderRadius: 6, padding: '8px 10px', wordBreak: 'break-all', lineHeight: 1.7 }}>
                      {selected.sha256_fingerprint}
                    </div>
                  </div>
                )}
                ...
                {/* Root store trust */}
                <div style={{ marginBottom: 18 }}>
                  <div style={{ fontSize: 10, color: W.muted, fontWeight: 600, marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Root store trust</div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
                    {storeItems.map(({ name, trusted }) => (
                      <div key={name} style={{ display: 'flex', alignItems: 'center', gap: 8, background: trusted ? W.greenBg : W.bg3, border: `1px solid ${trusted ? '#bbf7d0' : W.border}`, borderRadius: 7, padding: '8px 10px' }}>
                        <div style={{ width: 6, height: 6, borderRadius: '50%', background: trusted ? W.green : W.muted, flexShrink: 0 }} />
                        <div>
                          <div style={{ fontSize: 11, fontWeight: 600, color: W.heading }}>{name}</div>
                          <div style={{ fontSize: 10, color: trusted ? W.green : W.muted }}>{trusted ? 'Trusted' : 'Not included'}</div>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {/* CTA */}
                <button
                  onClick={() => { if (window.nav) window.nav('/ca-trust-explorer'); else window.location.href = '/ca-trust-explorer' }}
                  style={{ width: '100%', background: W.heading, color: '#fff', border: 'none', borderRadius: 8, padding: '11px 20px', fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontFamily: F }}>
                  ...
                  Explore full CA database →
                </button>
              </div>
            ) : (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 480, color: W.muted, fontSize: 13 }}>
