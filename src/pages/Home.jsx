...
// Deep Navy redesign — bold section breaks, varied layouts, strong visual hierarchy
...
// Deep Navy v5 — richer contrast, more distinct surfaces
const W = {
  bg:        '#0b1120',   // deep navy page base
  bg2:       '#0f172a',   // section alternate
  bg3:       '#1e293b',   // card / elevated surface
  bg4:       '#273549',   // hover / subtle lift
  border:    '#1e293b',
  border2:   '#334155',
  heading:   '#f1f5f9',
  body:      '#94a3b8',
...
const D = {
  ...
  mid:    'rgba(241,245,249,0.55)',
  lt:     'rgba(241,245,249,0.30)',
  bd:     'rgba(241,245,249,0.08)',
  teal:   '#10b981',
  tealDk: '#059669',
  green:  '#6ee7b7',
}
...
function SectionLabel({ children, color = W.teal }) {
  return (
    <div style={{ display:'inline-flex', alignItems:'center', gap:8, fontSize:11, fontWeight:700, color, letterSpacing:'0.1em', textTransform:'uppercase', fontFamily:MONO, marginBottom:16 }}>
...
function H2({ children, style = {} }) {
  return (
    <h2 style={{ fontSize:'clamp(24px,3.5vw,40px)', fontWeight:800, color:W.heading, letterSpacing:'-1px', lineHeight:1.15, ...style }}>
      {children}
    </h2>
  )
}
...
function Body({ children, style = {} }) {
  return (
    <p style={{ fontSize:15, color:W.body, lineHeight:1.85, ...style }}>
      {children}
    </p>
  )
}
...
function Btn({ label, onClick, primary, dark }) {
  ...
  if (primary) return (
    <button onClick={onClick} onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
      style={{ display:'inline-flex', alignItems:'center', gap:7, fontFamily:F, fontWeight:700, fontSize:14, padding:'10px 24px', borderRadius:8, border:'none', cursor:'pointer', background:h?'#C0624F':'#E8897A', color:'white', transition:'all .15s', letterSpacing:'-0.01em' }}>
  ...
  if (dark) return (
    <button onClick={onClick} onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
      style={{ display:'inline-flex', alignItems:'center', gap:7, fontFamily:F, fontWeight:500, fontSize:14, padding:'10px 24px', borderRadius:8, cursor:'pointer', background:h?'rgba(255,255,255,0.08)':'transparent', border:'1px solid rgba(255,255,255,0.15)', color:h?'white':'rgba(255,255,255,0.6)', transition:'all .15s', letterSpacing:'-0.01em' }}>
      {label}
    </button>
  )
  return (
    <button onClick={onClick} onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
      style={{ display:'inline-flex', alignItems:'center', gap:7, fontFamily:F, fontWeight:500, fontSize:14, padding:'10px 24px', borderRadius:8, cursor:'pointer', background:h?W.bg4:W.bg3, border:`1px solid ${h?W.border2:W.border}`, color:W.heading, transition:'all .15s', letterSpacing:'-0.01em' }}>
      {label}
    </button>
  )
}
...
function Card({ children, style = {}, accent }) {
  return (
    <div style={{ background:W.bg3, border:`1px solid ${W.border2}`, borderRadius:12, padding:'24px', ...(accent ? { borderTop:`2px solid ${accent}` } : {}), ...style }}>
      {children}
    </div>
  )
}
...
function Terminal({ title, lines }) {
  return (
    <div style={{ background:'#060d1a', border:`1px solid ${W.border2}`, borderRadius:12, overflow:'hidden', fontFamily:MONO }}>
      <div style={{ background:'#0b1422', padding:'10px 16px', display:'flex', alignItems:'center', gap:8, borderBottom:`1px solid ${W.border}` }}>
        <div style={{ display:'flex', gap:5 }}>
          {['#ff5f57','#ffbd2e','#28c840'].map(c => <div key={c} style={{ width:10, height:10, borderRadius:'50%', background:c, opacity:.85 }}/>)}
        </div>
        <span style={{ fontSize:11, color:W.muted, flex:1, textAlign:'center', letterSpacing:'0.02em' }}>{title}</span>
      </div>
      <div style={{ padding:'20px 24px', fontSize:12, lineHeight:2.1 }}>
        {lines.map((l, i) => (
          <div key={i} style={{ color: l.c || D.mid, paddingLeft: l.indent ? 20 : 0 }}>
            {l.prompt && <span style={{ color:W.muted, marginRight:10 }}>{l.prompt}</span>}
            {l.text}
          </div>
        ))}
        <div style={{ marginTop:12, display:'flex', alignItems:'center', gap:6 }}>
          <span style={{ color:W.muted }}>›</span>
...
function InventoryMockup() {
  ...
  return (
    <div style={{ background:'#060d1a', border:`1px solid ${W.border2}`, borderRadius:12, overflow:'hidden' }}>
      <div style={{ background:'#0b1422', padding:'10px 16px', display:'flex', alignItems:'center', gap:8, borderBottom:`1px solid ${W.border}` }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'2fr 52px 52px 80px 60px 70px', padding:'8px 16px', borderBottom:`1px solid ${W.border}` }}>
      ...
      {rows.map((r,i) => (
        <div key={r.d} style={{ display:'grid', gridTemplateColumns:'2fr 52px 52px 80px 60px 70px', padding:'10px 16px', borderBottom:i<rows.length-1?`1px solid ${W.border}`:'none', background:r.s==='critical'?'rgba(239,68,68,0.07)':r.s==='warning'?'rgba(249,115,22,0.07)':'transparent', alignItems:'center' }}>
          <span style={{ fontSize:12, fontWeight:500, color:W.heading, fontFamily:MONO }}>{r.d}</span>
          ...
          <span style={{ fontSize:9.5, fontWeight:700, color:sc(r.s), background:sc(r.s)+'18', padding:'2px 8px', borderRadius:4, fontFamily:MONO }}>{r.s}</span>
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div style={{ background:'#060d1a', border:`1px solid ${W.border2}`, borderRadius:12, overflow:'hidden' }}>
      ...
      <div style={{ padding:'16px' }}>
        <div style={{ display:'flex', alignItems:'center', gap:8, padding:'9px 12px', background:W.purpleBg, border:`1px solid rgba(232,137,122,0.25)`, borderRadius:8, marginBottom:14 }}>
        ...
        {[{d:'easysecurity.in',alg:'RSA-2048',last:'2h ago'},{d:'api.shop.com',alg:'EC-384',last:'Never'}].map((k,i) => (
          <div key={k.d} style={{ border:`1px solid ${W.border2}`, borderTop:`2px solid ${W.purple}`, borderRadius:8, padding:'12px', marginBottom:i===0?8:0 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(140px,1fr))', gap:6, marginBottom:10 }}>
              {[['Algorithm',k.alg],['Rotations','2'],['Last access',k.last]].map(([l,v]) => (
                <div key={l} style={{ background:W.bg2, borderRadius:5, padding:'6px 8px' }}>
                  <div style={{ fontSize:9, color:W.muted, textTransform:'uppercase', letterSpacing:'0.3px', marginBottom:2 }}>{l}</div>
                  <div style={{ fontSize:11, fontWeight:600, color:W.heading, fontFamily:MONO }}>{v}</div>
                </div>
              ))}
            </div>
            <div style={{ display:'flex', gap:6 }}>
              {['Reveal key','Rotate','Audit log'].map(t => (
                <button key={t} style={{ fontSize:10, fontWeight:600, padding:'5px 10px', borderRadius:5, border:`1px solid ${t==='Reveal key'?'rgba(232,137,122,0.3)':W.border2}`, background:t==='Reveal key'?W.purpleBg:W.bg2, color:t==='Reveal key'?W.purple:W.body, cursor:'pointer', fontFamily:'inherit' }}>{t}</button>
              ))}
            </div>
          </div>
        ))}
      ...
      <div style={{ padding:'16px' }}>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8, marginBottom:14 }}>
          {[{d:'Mar 2026',v:'200d',s:W.red,bg:W.redBg},{d:'Mar 2027',v:'100d',s:W.amber,bg:W.amberBg},{d:'Mar 2029',v:'47d',s:W.teal,bg:W.tealBg}].map(m => (
            <div key={m.d} style={{ padding:'10px', borderRadius:8, background:m.bg, border:`1px solid ${m.s}22` }}>
              <div style={{ fontSize:10, fontWeight:700, color:m.s, marginBottom:2 }}>{m.d}</div>
              <div style={{ fontSize:18, fontWeight:800, color:m.s, fontFamily:MONO }}>{m.v}</div>
          ...
          <div key={c.d} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 10px', borderRadius:8, background:W.bg2, marginBottom:6, border:`1px solid ${W.border}` }}>
            <div style={{ position:'relative', width:36, height:36, flexShrink:0 }}>
              <svg width="36" height="36" viewBox="0 0 36 36">
                <circle cx="18" cy="18" r="14" fill="none" stroke={W.border2} strokeWidth="3.5"/>
            ...
            <span style={{ fontSize:9.5, fontWeight:700, color:c.color, background:c.color+'18', padding:'2px 8px', borderRadius:4, fontFamily:MONO }}>{c.label}</span>
          </div>
        ))}
      </div>
    </div>
...
export default function Home({ nav }) {
  ...
  return (
    <div style={{ fontFamily:F, background:W.bg, color:W.heading, overflowX:'hidden' }}>
      <link href="https://fonts.googleapis.com/css2?family=DM+Sans:ital,opsz,wght@0,9..40,300..900;1,9..40,300..900&family=JetBrains+Mono:wght@400;500;600&display=swap" rel="stylesheet"/>
      <style>{`
      ...
      {/* ── NAV ── */}
      <header style={{ position:'sticky', top:0, zIndex:200, background:'rgba(11,17,32,0.92)', backdropFilter:'blur(20px)', WebkitBackdropFilter:'blur(20px)', borderBottom:'1px solid rgba(255,255,255,0.06)', height:60, display:'flex', alignItems:'center', padding:`0 clamp(16px,4vw,40px)`, justifyContent:'space-between' }}>
        <div style={{ display:'flex', alignItems:'center', gap:9, cursor:'pointer', flexShrink:0 }} onClick={() => nav('/')}>
          ...
          <span style={{ fontSize:15, fontWeight:700, color:'#f1f5f9', letterSpacing:'-0.4px' }}>SSLVault</span>
        </div>

        {!isMobile && <nav style={{ position:'absolute', left:'50%', transform:'translateX(-50%)', display:'flex', alignItems:'center', background:'rgba(30,41,59,0.8)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:10, padding:'4px 6px', gap:2 }}>
          {[['Platform','#platform'],['Features','#features'],['Security','#security'],['Pricing','/pricing']].map(([l,h]) => (
            <button key={l} onClick={() => h.startsWith('/') ? nav(h) : document.querySelector(h)?.scrollIntoView({ behavior:'smooth' })}
              style={{ background:'none', border:'none', cursor:'pointer', fontFamily:F, fontSize:13, fontWeight:500, color:'rgba(148,163,184,1)', padding:'6px 14px', borderRadius:7, transition:'all .15s' }}
              onMouseEnter={e => { e.currentTarget.style.color='#f1f5f9'; e.currentTarget.style.background='rgba(255,255,255,0.07)' }}
              onMouseLeave={e => { e.currentTarget.style.color='rgba(148,163,184,1)'; e.currentTarget.style.background='none' }}>
              {l}
            </button>
          ))}
          <div style={{ position:'relative' }}
            ...
            <button style={{ background:'none', border:'none', cursor:'pointer', fontFamily:F, fontSize:13, fontWeight:500, color:'rgba(148,163,184,1)', padding:'6px 14px', borderRadius:7, transition:'all .15s', display:'flex', alignItems:'center', gap:4 }}
              onMouseEnter={e => { e.currentTarget.style.color='#f1f5f9'; e.currentTarget.style.background='rgba(255,255,255,0.07)' }}
              onMouseLeave={e => { e.currentTarget.style.color='rgba(148,163,184,1)'; e.currentTarget.style.background='none' }}>
              Industry Intelligence
              <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="6 9 12 15 18 9"/></svg>
            </button>
            <div className="intel-drop" style={{ position:'absolute', top:'calc(100% + 8px)', left:'50%', transform:'translateX(-50%) translateY(-6px)', background:'#0f172a', border:'1px solid rgba(255,255,255,0.1)', borderRadius:12, padding:'6px', minWidth:210, zIndex:300, boxShadow:'0 16px 40px rgba(0,0,0,0.5)', opacity:0, pointerEvents:'none', transition:'opacity .2s cubic-bezier(.16,1,.3,1), transform .2s cubic-bezier(.16,1,.3,1)' }}>
              {[
                ...
                <button key={item.path} onClick={() => nav(item.path)}
                  ...
                  <div style={{ fontSize:13, fontWeight:600, color:'rgba(241,245,249,0.88)', marginBottom:2 }}>{item.label}</div>
        ...
        <div style={{ display:'flex', gap:4, alignItems:'center', flexShrink:0 }}>
          {!isMobile && <button onClick={() => nav('/auth')}
            style={{ background:'none', border:'none', cursor:'pointer', fontFamily:F, fontSize:13, color:'rgba(148,163,184,0.8)', padding:'7px 16px', borderRadius:8, transition:'color .15s' }}
            onMouseEnter={e => e.currentTarget.style.color='#f1f5f9'}
            onMouseLeave={e => e.currentTarget.style.color='rgba(148,163,184,0.8)'}>
            Sign in
          </button>}
          <button onClick={() => nav('/auth')}
            style={{ background:'#E8897A', border:'none', cursor:'pointer', fontFamily:F, fontSize:13, fontWeight:700, color:'white', padding:'8px 20px', borderRadius:8, transition:'all .15s' }}
            onMouseEnter={e => { e.currentTarget.style.background='#10b981' }}
            onMouseLeave={e => { e.currentTarget.style.background='#E8897A' }}>
            Get started
          </button>
        </div>
      </header>
      ...
      {/* ── HERO ── */}
      <section style={{ background:'#060d1a', padding:`clamp(80px,10vw,130px) clamp(20px,5vw,48px) clamp(70px,9vw,110px)`, position:'relative', overflow:'hidden' }}>
        {/* Grid background */}
        ...
        {/* Top glow */}
        <div style={{ position:'absolute', top:0, left:'50%', transform:'translateX(-50%)', width:700, height:300, background:'radial-gradient(ellipse 70% 60% at 50% 0%, rgba(16,185,129,0.12) 0%, transparent 100%)', pointerEvents:'none' }}/>
        {/* Bottom fade */}
        <div style={{ position:'absolute', bottom:0, left:0, right:0, height:120, background:'linear-gradient(transparent, #0b1120)', pointerEvents:'none' }}/>

        <div style={{ maxWidth:1100, margin:'0 auto', position:'relative' }}>
          <div style={{ display:'grid', gridTemplateColumns:isMobile?'1fr':'1fr 1fr', gap:isMobile?48:72, alignItems:'center' }}>

            <div>
              <div style={{ display:'inline-flex', alignItems:'center', gap:8, border:'1px solid rgba(16,185,129,0.25)', borderRadius:8, padding:'6px 14px', marginBottom:32, background:'rgba(16,185,129,0.06)' }}>
                <span style={{ width:6, height:6, borderRadius:'50%', background:'#10b981', animation:'blink 2.4s ease infinite' }}/>
                <span style={{ fontSize:11.5, fontWeight:600, color:'rgba(16,185,129,0.9)', fontFamily:MONO }}>RFC 8555 · CA/B Forum Compliant · PQC Intelligence</span>
              </div>

              <h1 style={{ fontSize:`clamp(38px,5.5vw,72px)`, fontWeight:800, letterSpacing:'-2px', lineHeight:1.06, color:'#f1f5f9', marginBottom:22 }}>
                Certificate<br/>
                <span style={{ color:'#E8897A' }}>lifecycle</span><br/>
                management.
              </h1>

              <p style={{ fontSize:16.5, color:'rgba(148,163,184,0.85)', lineHeight:1.85, marginBottom:36, maxWidth:420 }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:0, borderTop:'1px solid rgba(255,255,255,0.06)', paddingTop:28 }}>
                {[
                  { val: certCount ? `${count.toLocaleString()}+` : '—', label:'Certs managed' },
                  { val:'99.9%', label:'Renewal success' },
                  { val:'12', label:'PKI bodies tracked' },
                ].map((m, i) => (
                  <div key={m.label} style={{ paddingLeft:i>0?24:0, borderLeft:i>0?'1px solid rgba(255,255,255,0.06)':'none', marginLeft:i>0?24:0 }}>
                    <div style={{ fontSize:22, fontWeight:800, color:'#E8897A', fontFamily:MONO, letterSpacing:'-0.5px', marginBottom:4 }}>{m.val}</div>
                    <div style={{ fontSize:11.5, color:W.muted }}>{m.label}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ position:'relative' }}>
              ...
              {!isMobile && (
                <div style={{ position:'absolute', bottom:-18, right:-14, background:'#0b1422', border:'1px solid rgba(16,185,129,0.3)', borderRadius:10, padding:'10px 14px' }}>
                  <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                    <div style={{ position:'relative' }}>
                      <div style={{ width:7, height:7, borderRadius:'50%', background:D.green }}/>
                    </div>
                    <span style={{ fontSize:11, fontWeight:600, color:'rgba(241,245,249,0.8)', fontFamily:MONO }}>easysecurity.in</span>
                    <span style={{ fontSize:10, fontWeight:700, color:D.green, background:'rgba(16,185,129,0.1)', border:'1px solid rgba(16,185,129,0.2)', borderRadius:4, padding:'1px 7px', fontFamily:MONO }}>A+</span>
                  </div>
                  <div style={{ fontSize:10, color:W.muted, marginTop:3, fontFamily:MONO }}>cert valid · 196d · auto-renew ✓</div>
                </div>
              )}
            </div>
          </div>
      ...
      {/* ── TICKER ── */}
      <div style={{ background:'#10b981', overflow:'hidden', padding:'9px 0' }}>
        <div style={{ display:'flex', overflow:'hidden', maskImage:'linear-gradient(to right, transparent, black 8%, black 92%, transparent)' }}>
          <div style={{ display:'flex', gap:48, flexShrink:0, animation:'ticker 36s linear infinite', whiteSpace:'nowrap' }}>
            {[...TICKER,...TICKER].map((p,i) => (
              <span key={i} style={{ fontSize:11, fontWeight:700, color:'rgba(0,0,0,0.65)', letterSpacing:'0.05em', fontFamily:MONO }}>
                <span style={{ color:'rgba(0,0,0,0.35)', marginRight:8 }}>◆</span>{p}
              </span>
            ))}
          </div>
        </div>
                ...
                {[
                  ...
                  <div key={s.n} style={{ display:'flex', gap:0, alignItems:'stretch' }}>
                    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', marginRight:16 }}>
                      <div style={{ width:32, height:32, borderRadius:8, background:s.color+'18', border:`1px solid ${s.color}30`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:10, fontWeight:800, color:s.color, fontFamily:MONO, flexShrink:0 }}>{s.n}</div>
                      <div style={{ width:1, flex:1, background:`linear-gradient(${s.color}30, transparent)`, minHeight:12 }}/>
                    </div>
                    <div style={{ paddingBottom:20 }}>
                      <div style={{ fontSize:14, fontWeight:700, color:W.heading, marginBottom:4 }}>{s.t}</div>
                      <div style={{ fontSize:12.5, color:W.body, lineHeight:1.6 }}>{s.d}</div>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <section id="features" style={{ background:W.bg2, padding:`clamp(72px,9vw,100px) clamp(20px,4vw,48px)`, borderTop:`1px solid ${W.border}` }}>
        <div style={{ maxWidth:1100, margin:'0 auto' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:`repeat(${cols},1fr)`, gap:1, background:W.border, border:`1px solid ${W.border}`, borderRadius:14, overflow:'hidden' }}>
            {[
              { icon:'⚡', color:'#E8897A', title:'Certificate issuance', specs:['DV, OV, EV, Wildcard, SAN','RapidSSL · DigiCert trust chain','ACME v2 · RFC 8555','Issued in < 5 minutes'], badge:'ACME v2' },
              { icon:'🤖', color:W.teal, title:'Persistent agent', specs:['systemd daemon · polls every 5 min','Nginx + Apache auto-detect','Config test before reload','Outbound HTTPS only — no firewall changes'], badge:'systemd' },
              { icon:'🌐', color:W.teal, title:'DNS automation', specs:['Cloudflare · Vercel · Route53','Namecheap · GoDaddy · DigitalOcean','Auto TXT/CNAME challenge','Cleanup after DCV completes'], badge:'DNS-01' },
              { icon:'🏛', color:'#10b981', title:'cPanel install', specs:['UAPI-based installation','No SSH or agent required','API token auth','Auto-renew via cPanel'], badge:'UAPI' },
              { icon:'🔐', color:'#E8897A', title:'CertVault', specs:['AES-256-GCM · envelope encryption','Password re-auth before reveal','30-day rotation archive','Immutable audit log → CSV'], badge:'AES-256' },
              { icon:'📋', color:'#E8897A', title:'47-day readiness', specs:['Scores every cert 0–100','200d → 100d → 47d mandate timeline','Per-cert automation checklist','Fleet-wide compliance report'], badge:'CA/B 2026' },
              { icon:'🔍', color:W.teal, title:'Discovery & monitoring', specs:['CT log scan via crt.sh','CT abuse monitor — unknown CAs','SSL health score A+ to F','HSTS · CAA · TLS 1.3 checks'], badge:'CT Logs' },
              { icon:'📈', color:W.tealDk, title:'CA intelligence', specs:['DigiCert CertCentral sync','Sectigo SCM portfolio sync','Shadow IT detection','Policy engine · fleet compliance'], badge:'Multi-CA' },
              { icon:'🏛', color:W.teal, title:'Industry Intelligence', specs:['6,200+ CAs from CCADB live','CAB Forum ballot tracker','12 PKI bodies deep-dive','PQC migration tracker (ML-KEM, ML-DSA)'], badge:'PKI Hub' },
              { icon:'🔬', color:'#E8897A', title:'CA Trust Store', specs:['Every root & intermediate CA indexed','PKI Trust Score per certificate','Filter by trust store · algorithm · region','CSV export · PEM download'], badge:'CCADB' },
            ].map(f => (
              <FadeUp key={f.title}>
                <div style={{ background:W.bg, padding:'24px', height:'100%' }}>
                  <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:14 }}>
                    <div style={{ width:36, height:36, borderRadius:8, background:f.color+'14', border:`1px solid ${f.color}22`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:18 }}>{f.icon}</div>
        ...
        <div style={{ maxWidth:1100, margin:'0 auto' }}>
          <FadeUp>
            <div style={{ display:'grid', gridTemplateColumns:isMobile?'1fr':'1fr 1fr', gap:isMobile?40:80, alignItems:'flex-start', marginBottom:52 }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(min(300px,100%),1fr))', gap:1, background:W.border2, border:`1px solid ${W.border2}`, borderRadius:12, overflow:'hidden' }}>
                {[
                  ...
                  <div key={s.spec} style={{ background:W.bg, padding:'16px' }}>
                    <div style={{ fontSize:9.5, fontWeight:700, color:s.color, fontFamily:MONO, letterSpacing:'0.05em', marginBottom:5 }}>{s.spec}</div>
                    <div style={{ fontSize:13, fontWeight:600, color:W.heading, marginBottom:4 }}>{s.title}</div>
          ...
          <FadeUp>
            <div style={{ background:'#060d1a', border:`1px solid ${W.border2}`, borderRadius:12, padding:'28px' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:isMobile?'1fr':'repeat(auto-fill,minmax(180px,1fr))', gap:isMobile?20:0 }}>
                {[
                  { date:'March 15, 2026', limit:'200 days', status:'IMMINENT', color:'#E8897A', action:'Certificates issued with validity > 200 days will be rejected by all major browsers. Audit your fleet immediately.' },
                  { date:'March 15, 2027', limit:'100 days', status:'UPCOMING', color:W.amber, action:'Manual renewal every 100 days is operationally unsustainable. Automation becomes a hard requirement.' },
                  { date:'March 15, 2029', limit:'47 days',  status:'PLANNED',  color:W.teal, action:"Full zero-touch automation required. SSLVault's agent + DNS automation handles this end-to-end today." },
                ].map((m, i) => (
                  <div key={m.date} style={{ padding:isMobile?'16px 0':'0 28px', borderLeft:!isMobile&&i>0?`1px solid ${W.border}`:'none' }}>
                    ...
                    <div style={{ fontSize:32, fontWeight:800, color:m.color, fontFamily:MONO, letterSpacing:'-1.5px', marginBottom:10 }}>{m.limit}</div>
              ...
              <div>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(min(300px,100%),1fr))', gap:6 }}>
                  {[
                    ...
                    <div key={name} style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 12px', background:W.bg3, border:`1px solid ${W.border}`, borderRadius:8 }}>
          ...
          <FadeUp>
            <div style={{ display:'grid', gridTemplateColumns:`repeat(${isMobile?1:5},1fr)`, gap:isMobile?12:0, marginBottom:52, alignItems:'center' }}>
              {[
                ...
                return (
                  <div key={s.n} style={{ background:W.bg, border:`1px solid ${W.border2}`, borderRadius:12, padding:'20px', textAlign:'center' }}>
                    <div style={{ fontSize:24, marginBottom:10 }}>{s.icon}</div>
                    <div style={{ fontSize:10, fontWeight:700, color:W.teal, fontFamily:MONO, marginBottom:6 }}>{s.n}</div>
                    <div style={{ fontSize:13, fontWeight:700, color:W.heading, marginBottom:6 }}>{s.title}</div>
                    <div style={{ fontSize:11.5, color:W.body, lineHeight:1.6 }}>{s.desc}</div>
                  </div>
                )
              })}
      ...
      {/* ── INDUSTRY INTELLIGENCE ── */}
      <section style={{ background:'#060d1a', padding:`clamp(72px,9vw,100px) clamp(20px,4vw,48px)`, borderTop:`3px solid ${W.teal}` }}>
        <div style={{ maxWidth:1100, margin:'0 auto' }}>
          <FadeUp>
            <div style={{ textAlign:'center', marginBottom:52 }}>
              <SectionLabel>Industry Intelligence</SectionLabel>
              <h2 style={{ fontSize:`clamp(26px,3.5vw,40px)`, fontWeight:800, letterSpacing:'-1px', lineHeight:1.12, color:'rgba(241,245,249,0.95)', marginBottom:14, maxWidth:620, margin:'8px auto 14px' }}>
                The deepest PKI intelligence platform on the web.
              </h2>
              <p style={{ fontSize:15, color:'rgba(148,163,184,0.7)', lineHeight:1.7, maxWidth:520, margin:'0 auto' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:isMobile?'1fr':'repeat(auto-fill,minmax(180px,1fr))', gap:16, marginBottom:20 }}>
            {[
              { icon:'🏛', color:'#10b981', title:'CA Trust Store', sub:'6,200+ root & intermediate CAs', desc:'Every CA in Chrome, Firefox, Apple, and Microsoft trust stores — live from CCADB. Search by operator, algorithm, region. PKI Trust Score per cert. PEM download.', badge:'CCADB Live', path:'/ca-trust-explorer', stats:[['6,200+','CAs indexed'],['4','Trust stores'],['Daily','CCADB sync']] },
              { icon:'⚖️', color:'#E8897A', title:'CAB Forum Intelligence', sub:'Ballots, timelines & compliance', desc:'Every CAB Forum ballot tracked with plain-English summaries. 47-day countdown, SC081v3 compliance deadlines, 5 working groups, full PKI history timeline from 2005.', badge:'Live sync', path:'/cab-forum', stats:[['47-day','2029 mandate'],['5','Working groups'],['Real-time','Ballot feed']] },
              { icon:'🌍', color:'#10b981', title:'Global PKI Hub', sub:'12 bodies · 22 standards · PQC tracker', desc:'CAB Forum, ETSI ESI, NIST, IETF, APKIC, eIDAS 2.0, PKI Consortium, CSC, FIDO, WebTrust, CCADB, ITU-T — each with deep-dive pages, standards library, and PQC migration status.', badge:'PQC Ready', path:'/pki-hub', stats:[['12','PKI bodies'],['3','NIST PQC finals'],['2026','Amsterdam conf.']] },
            ].map(item => (
              <FadeUp key={item.title}>
                <div onClick={() => nav(item.path)} style={{ background:'rgba(255,255,255,0.03)', border:`1px solid rgba(255,255,255,0.08)`, borderRadius:12, padding:'24px', cursor:'pointer', transition:'border-color .15s, background .15s', height:'100%', display:'flex', flexDirection:'column', gap:16 }}
                  ...
                  <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between' }}>
                    ...
                    <span style={{ fontSize:9, fontWeight:700, color:item.color, background:item.color+'18', border:`1px solid ${item.color}30`, borderRadius:6, padding:'3px 9px', fontFamily:MONO, letterSpacing:'0.04em' }}>{item.badge}</span>
                  </div>
                  <div>
                    <div style={{ fontSize:15, fontWeight:700, color:'rgba(241,245,249,0.88)', marginBottom:4 }}>{item.title}</div>
                    <div style={{ fontSize:11, color:item.color, fontWeight:600, fontFamily:MONO, marginBottom:10 }}>{item.sub}</div>
                    <div style={{ fontSize:12.5, color:'rgba(148,163,184,0.7)', lineHeight:1.7 }}>{item.desc}</div>
                  </div>
                  <div style={{ display:'flex', gap:0, borderTop:'1px solid rgba(255,255,255,0.06)', paddingTop:14, marginTop:'auto' }}>
                    {item.stats.map(([val, label], i) => (
                      <div key={label} style={{ flex:1, paddingLeft:i>0?12:0, borderLeft:i>0?'1px solid rgba(255,255,255,0.06)':'none', marginLeft:i>0?12:0 }}>
                        <div style={{ fontSize:13, fontWeight:700, color:'rgba(241,245,249,0.75)', fontFamily:MONO, marginBottom:2 }}>{val}</div>
          ...
          <FadeUp>
            <div style={{ background:'rgba(16,185,129,0.05)', border:'1px solid rgba(16,185,129,0.15)', borderRadius:14, padding:'20px 24px', display:'flex', gap:20, alignItems:'flex-start', flexWrap:'wrap' }}>
              <div style={{ fontSize:28, flexShrink:0 }}>⚛️</div>
              <div style={{ flex:1, minWidth:260 }}>
                <div style={{ fontSize:13, fontWeight:700, color:'rgba(241,245,249,0.85)', marginBottom:5 }}>Post-Quantum Cryptography migration tracker — live</div>
                <div style={{ fontSize:12, color:'rgba(148,163,184,0.6)', lineHeight:1.7 }}>NIST finalised ML-KEM (FIPS 203), ML-DSA (FIPS 204), and SLH-DSA (FIPS 205) in August 2024. The PKI Hub tracks every body's PQC readiness — from IETF LAMPS WG certificate profiles to ETSI EN 319 updates — so you know exactly where the industry stands.</div>
              </div>
              <div style={{ display:'flex', gap:8, flexWrap:'wrap', alignItems:'center', flexShrink:0 }}>
                {[['ML-KEM','FIPS 203'],['ML-DSA','FIPS 204'],['SLH-DSA','FIPS 205']].map(([alg, fips]) => (
                  <div key={alg} style={{ background:'rgba(16,185,129,0.1)', border:'1px solid rgba(16,185,129,0.2)', borderRadius:8, padding:'7px 12px', textAlign:'center' }}>
                    ...
                    <div style={{ fontSize:10, color:'rgba(148,163,184,0.5)', marginTop:2 }}>{fips}</div>
                  </div>
                ))}
                <button onClick={() => nav('/pki-hub')} style={{ background:'#10b981', border:'none', borderRadius:8, padding:'9px 16px', fontSize:12, fontWeight:700, color:'#000', cursor:'pointer', fontFamily:F }}>
                  View PQC tracker →
                </button>
              </div>
            </div>
      ...
      {/* ── CERTBIND ── */}
      <section style={{ background:W.bg, padding:`clamp(72px,9vw,100px) clamp(20px,4vw,48px)`, borderTop:`3px solid ${W.purple}`, position:'relative', overflow:'hidden' }}>
        <div style={{ position:'absolute', top:'50%', left:'50%', transform:'translate(-50%,-50%)', width:800, height:400, background:'radial-gradient(ellipse 60% 50% at 50% 50%, rgba(232,137,122,0.04) 0%, transparent 100%)', pointerEvents:'none' }}/>
        <div style={{ maxWidth:1100, margin:'0 auto', position:'relative' }}>
          <FadeUp>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:20 }}>
              ...
              <span style={{ fontSize:11, color:W.muted, letterSpacing:'0.06em', textTransform:'uppercase', fontFamily:MONO }}>No other CLM vendor has built this</span>
            </div>

            <div style={{ display:'grid', gridTemplateColumns:isMobile?'1fr':'1fr 1fr', gap:isMobile?40:80, alignItems:'center', marginBottom:56 }}>
              <div>
                <h2 style={{ fontSize:`clamp(28px,4.5vw,52px)`, fontWeight:800, letterSpacing:'-1.5px', lineHeight:1.08, color:'rgba(241,245,249,0.95)', marginBottom:20 }}>
                  Every other CLM tells you what cert you issued.<br/>
                  <span style={{ color:'#E8897A' }}>SSLVault proves what's actually running.</span>
                </h2>
                <p style={{ fontSize:16, color:W.body, lineHeight:1.85, marginBottom:28, maxWidth:480 }}>
                ...
                <p style={{ fontSize:16, color:W.body, lineHeight:1.85, marginBottom:32, maxWidth:480 }}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                <div style={{ background:W.bg3, border:`1px solid ${W.border2}`, borderRadius:12, padding:'14px 18px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                  <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                    <span style={{ fontSize:16 }}>🔗</span>
                    <span style={{ fontSize:13, fontWeight:700, color:W.heading, fontFamily:MONO }}>CertBind</span>
                    <span style={{ fontSize:10, fontWeight:700, color:'#10b981', background:'rgba(16,185,129,0.1)', border:'1px solid rgba(16,185,129,0.2)', borderRadius:4, padding:'2px 8px', fontFamily:MONO }}>ACTIVE</span>
                  </div>
                  <span style={{ fontSize:11, color:W.muted, fontFamily:MONO }}>4/4 domains bound</span>
                </div>
                ...
                {[
                  ...
                  <div key={l.n} style={{ background:W.bg3, border:`1px solid ${W.border2}`, borderRadius:10, padding:'12px 16px', display:'flex', gap:12, alignItems:'flex-start' }}>
                    ...
                    <div style={{ flex:1 }}>
                      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:4 }}>
                        <span style={{ fontSize:12, fontWeight:600, color:W.heading }}>{l.label}</span>
                        <span style={{ fontSize:10, fontWeight:700, color:l.color, fontFamily:MONO }}>{l.status}</span>
                      </div>
                      <div style={{ fontSize:11, color:W.muted, fontFamily:MONO }}>{l.desc}</div>
                    </div>
                  </div>
                ))}

                <div style={{ background:'rgba(220,38,38,0.06)', border:'1px solid rgba(220,38,38,0.18)', borderRadius:10, padding:'12px 16px', display:'flex', gap:10, alignItems:'flex-start' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:11, color:W.muted, fontFamily:MONO, lineHeight:1.7 }}>key_mismatch · cert_mismatch · chain_anomaly · partial_deploy</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <FadeUp delay={80}>
            <div style={{ borderTop:`1px solid ${W.border}`, paddingTop:40 }}>
              <div style={{ fontSize:11, fontWeight:700, color:W.muted, letterSpacing:'0.08em', textTransform:'uppercase', fontFamily:MONO, marginBottom:24, textAlign:'center' }}>Real-world failures CertBind prevents</div>
              <div style={{ display:'grid', gridTemplateColumns:`repeat(${isMobile?1:3},1fr)`, gap:12 }}>
                {[
                  ...
                  <div key={s.title} style={{ background:s.bg, border:`1px solid ${s.bd}`, borderRadius:12, padding:'20px' }}>
                    ...
                    <div style={{ fontSize:14, fontWeight:700, color:W.heading, marginBottom:10 }}>{s.title}</div>
                    <div style={{ fontSize:12.5, color:W.body, lineHeight:1.75, marginBottom:12 }}>{s.scenario}</div>
            ...
            <FadeUp delay={80}>
              <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                {[
                  ...
                  <div key={c.name} style={{ display:'flex', alignItems:'center', gap:14, padding:'14px 16px', background:c.highlight?'rgba(232,137,122,0.06)':W.bg3, border:`1px solid ${c.highlight?'rgba(232,137,122,0.2)':W.border2}`, borderRadius:10 }}>
                    <div style={{ flex:1 }}>
                ...
                <div style={{ marginTop:8, display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(min(300px,100%),1fr))', gap:6 }}>
                  {[
                    ...
                    <div key={val} style={{ display:'flex', gap:10, padding:'10px 12px', background:W.bg3, border:`1px solid ${W.border}`, borderRadius:8, alignItems:'center' }}>
      ...
      {/* ── CTA ── */}
      <section style={{ background:'#060d1a', padding:`clamp(72px,9vw,100px) clamp(20px,4vw,48px)`, borderTop:`1px solid ${W.border}` }}>
        <div style={{ maxWidth:680, margin:'0 auto', textAlign:'center' }}>
          <FadeUp>
            <div style={{ display:'inline-flex', alignItems:'center', gap:7, border:'1px solid rgba(232,137,122,0.25)', borderRadius:8, padding:'6px 14px', marginBottom:28, background:'rgba(232,137,122,0.06)' }}>
      ...
      {/* ── FOOTER ── */}
      <footer style={{ background:'#060d1a', borderTop:`1px solid ${W.border}`, padding:`clamp(48px,6vw,64px) clamp(20px,4vw,48px) clamp(28px,3vw,40px)` }}>
        <div style={{ maxWidth:1100, margin:'0 auto' }}>
          <div style={{ display:'flex', alignItems:'center', gap:9, marginBottom:40, paddingBottom:32, borderBottom:`1px solid ${W.border}` }}>
            ...
            <span style={{ fontSize:15, fontWeight:700, color:W.heading }}>SSLVault</span>
            <span style={{ fontSize:11, color:W.muted, fontFamily:MONO, marginLeft:4 }}>PKI-first CLM · Built by a real PKI engineer</span>
          </div>
          <div style={{ display:'grid', gridTemplateColumns:isMobile?'1fr 1fr':'repeat(auto-fill,minmax(150px,1fr))', gap:32, marginBottom:40 }}>
            {[
              ...
              <div key={col.title}>
                <div style={{ fontSize:11, fontWeight:700, color:W.teal, textTransform:'uppercase', letterSpacing:'0.08em', fontFamily:MONO, marginBottom:16 }}>{col.title}</div>
                {col.links.map(([l, h]) => (
                  <div key={l} style={{ marginBottom:10 }}>
                    <button onClick={() => typeof h === 'function' ? h() : document.querySelector(h)?.scrollIntoView({ behavior:'smooth' })}
                      style={{ background:'none', border:'none', cursor:'pointer', fontSize:13, color:W.muted, fontFamily:F, padding:0, transition:'color .15s', textAlign:'left' }}
                      onMouseEnter={e => e.currentTarget.style.color=W.heading}
          ...
          <div style={{ borderTop:`1px solid ${W.border}`, paddingTop:24, display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:12 }}>
            <span style={{ fontSize:11, color:W.muted, fontFamily:MONO }}>PKI-first CLM · Made with ♥ towards PKI · Built by a real PKI engineer</span>
            <span style={{ fontSize:12, color:W.muted }}>© 2026 SSLVault. All rights reserved.</span>
          </div>
        </div>
      </footer>
    </div>
