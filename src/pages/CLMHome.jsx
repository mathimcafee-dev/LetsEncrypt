...
import {
  ...
  Clock, X, Settings, FileText, Search,
  Layout, Users, CreditCard,
...
export default function CLMHome({ user, nav }) {
  ...
  const ACTCO = [
    { icon:Shield, color:'#00b48a', bg:'#ecfdf5', label:'Issue Cert', desc:'RapidSSL DV', fn:() => nav('/buy') },
    { icon:FileText, color:'#3b82f6', bg:'#eff6ff', label:'Import Cert', desc:'Bring your own', fn:() => nav('/import') },
      ...
      <div className="cc-shell">
        <nav className={`cc-sidebar${sidebarOpen ? ' open' : ''}`}>
          ...
          <div style={{ marginTop:'auto', padding:'14px 18px', borderTop:'1px solid rgba(255,255,255,0.08)' }}>
            <div style={{ fontSize:11, color:'rgba(255,255,255,0.35)' }}>Signed in as</div>
        ...
        <div className="cc-main">
          ...
          <div className="cc-canvas">
            <div style={{ marginBottom:20, display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:8 }}>
              ...
              {expired > 0 && <div style={{ display:'flex', alignItems:'center', gap:6, background:'#fef2f2', border:'1px solid #fecaca', borderRadius:8, padding:'7px 12px', cursor:'pointer' }} onClick={() => nav('/dashboard')}><AlertTriangle size={13} color="#dc2626"/><span style={{ fontSize:12, fontWeight:600, color:'#b91c1c' }}>{expired} expired</span><ChevronRight size={12} color="#dc2626"/></div>}
            </div>
            ...
            <div className="cc-widget" style={{ marginBottom:20 }}>
              ...
              <div className="cc-widget-body">
                <div className="cc-actions-grid">
                  {ACTCO.map(({ icon:Icon, color, bg, label, desc, fn }) => (
