...
import {
  ...
  Layout, Download, Settings, ChevronRight, Search,
  RefreshCw, Eye, Key
} from 'lucide-react'
...
export default function CLMHome({ user, nav }) {
  ...
  const renderContent = () => {
    if (section === 'dns') return <DnsProviders nav={nav} embedded={true}/>
    if (section === 'install') return <Install nav={nav} embedded={true}/>
    if (section === 'certs') return <Dashboard nav={nav} embedded={true}/>
    if (section === 'servers') return <Dashboard nav={nav} embedded={true}/>
