...
export default function CLMHome({ user, nav }) {
  ...
  const NAV = [
    { id:'dashboard', label:'Dashboard', icon:Layout, route:'/' },
    { id:'certs', label:'Certificates', icon:Shield, route:'/dashboard' },
    { id:'dns', label:'DNS Providers', icon:Globe, route:'/dns-providers' },
    { id:'servers', label:'Servers', icon:Server, route:'/dashboard' },
    { id:'install', label:'Install Guide', icon:Download, route:'/install' },
  ...
  const ACTIONS = [
    ...
    { icon:Globe, color:'x0ea5e9', bg:'#f0f9ff', label:'DNS Providers', desc:'uto-validation', fn:() => nav('/dns-providers') },
    { icon:Server, color:'#d97706', bg:'#fffbeb', label:'Servers', desc:'VPS and cPanel', fn:() => nav('/dashboard') },
    { icon:Zap, color:'#10b981', bg:'#ecfdf5', label:'Install Cert', desc:'VPS or shared', fn:() => nav('/install') },
  ]
  ...
  return (
    <div style={{ display:'flex', minHeight:'calc(100vh - 56px)', background:'#f4f6f9', fontFamily:"'Segoe UI',system-ui,sans-serif" }}>
      <nav style={{ width:220, background:'#1c2d3e', display:'flex', flexDirection:'column', flexShrink:0, position:'sticky', top:56, height:'calc(100vh - 56px)', overflowY:'auto' }}>
        ...
        <button style={{ margin:'14px 14px 8px', background:'#00b48a', color:'white', border:'none', borderRadius:7, padding:'10px 14px', fontSize:12, fontWeight:700, cursor:'pointer', width:'calc(100% - 28px)', display:'flex', alignItems:'center', justifyContent:'center', gap:6, fontFamily:'inherit' }} onClick={() => nav('/buy')}>
          <Plus size={13}/> Request a Certificate
        </button>
        <div style={{ padding:'10px 0 4px' }}>
          {NAV.map(({ id, label, icon:Icon, route }) => (
            <button key={id} style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 18px', cursor:'pointer', fontSize:12, fontWeight:500, color: id==='dashboard' ? 'white' : 'rgba(255,255,255,0.6)', background: id==='dashboard' ? 'rgba(0,180,138,0.15)' : 'none', borderLeft: id==='dashboard' ? '3px solid #00b48a' : '3px solid transparent', border:'none', width:'100%', textAlign:'left', fontFamily:'inherit' }} onClick={() => nav(route)}>
      ...
      <div style={{ flex:1, minWidth:0, display:'flex', flexDirection:'column' }}>
        <div style={{ background:'white', borderBottom:'1px solid #e8edf2', padding:'0 28px', height:52, display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0, position:'sticky', top:56, zIndex:30 }}>
          <div style={{ fontSize:20, fontWeight:700, color:'#1a2332', letterSpacing:'-0.3px' }}>Overview</div>
          <div style={{ display:'flex', gap:12 }}>
            <button style={{ display:'inline-flex', alignItems:'center', gap:6, background:'white', color:'#1a2332', border:'1px solid #d1d9e0', borderRadius:7, padding:'8px 16px', fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }} onClick={() => nav('/dashboard')}>
              <Activity size={13}/> View All Certs
            </button>
            <button style={{ display:'inline-flex', alignItems:'center', gap:6, background:'#00b48a', color:'white', border:'none', borderRadius:7, padding:'8px 16px', fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }} onClick={() => nav('/buy')}>
        ...
        <div style={{ flex:1, padding:'24px 28px 60px', overflowY:'auto' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:16, marginBottom:24 }}>
            {[
              ...
              <div key={label} style={{ background:'white', borderRadius:10, padding:'18px 20px', border:'1px solid #e8edf2', display:'flex', alignItems:'center', gap:14, cursor:'pointer', boxShadow:'0 1px 3px rgba(0,0,0,0.04)' }} onClick={() => nav('/dashboard')}>
          ...
          <div style={{ background:'white', borderRadius:10, border:'1px solid #e8edf2', boxShadow:'0 1px 3px rgba(0,0,0,0.04)', overflow:'hidden', marginBottom:20 }}>
          ...
          <div style={{ background:'white', borderRadius:10, border:'1px solid #e8edf2', boxShadow:'0 1px 3px rgba(0,0,0,0.04)', overflow:'hidden' }}>
            <div style={{ padding:'16px 20px 12px', borderBottom:'1px solid #f1f5f9', display:'flex', alignItems:'flex-start', justifyContent:'space-between' }}>
              ...
              <button style={{ fontSize:11, fontWeight:600, color:'#00b48a', background:'none', border:'none', cursor:'pointer', display:'flex', alignItems:'center', gap:4, fontFamily:'inherit' }} onClick={() => nav('/dashboard')}>
                View all <ChevronRight size={11}/>
              </button>
            </div>
            {recent.length === 0 ? (
              <div style={{ padding:'40px 20px', textAlign:'center' }}>
                <div style={{ fontSize:36, marginBottom:12 }}>SSL</div>
                ...
                <button style={{ display:'inline-flex', alignItems:'center', gap:7, background:'#00b48a', color:'white', border:'none', borderRadius:8, padding:'10px 20px', fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }} onClick={() => nav('/buy')}>
              ...
              <div style={{ overflowX:'auto' }}>
                <table style={{ width:'100%', borderCollapse:'collapse' }}>
                  <thead>
                    <tr>
                      {['Domain','Status','Expires','Type'].map(h => (
                  ...
                  <tbody>
                    {recent.map(cert => {
                      ...
                      return (
                        <tr key={cert.id} onClick={() => nav('/dashboard')} style={{ cursor:'pointer' }}>
