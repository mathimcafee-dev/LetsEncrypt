...
function agentStatus(last_seen_at, status) {
  if (!last_seen_at) return { label: 'Never seen', color: 'var(--v2-text-3)', dot: 'rgba(240,237,232,0.15)', bg: '#f0ede8' }
  ...
  if (mins > 6) return { label: 'Idle', color: '#e07060', dot: '#f0ede8', bg: 'rgba(239,68,68,0.08)' }
  return { label: 'Online', color: '#f0ede8', dot: '#f0ede8', bg: 'transparent' }
}
      ...
      >
        ...
        <div style={{ minWidth:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:3 }}>
            ...
            {agent.is_sandbox && <span style={{ fontSize:9, color:'#e07060', background:'rgba(239,68,68,0.08)', border:'0.5px solid #F2C4BC', borderRadius:3, padding:'1px 5px', fontWeight:500, flexShrink:0 }}>SANDBOX</span>}
          </div>
        ...
        {/* Version */}
        <div style={{ fontSize:11, color:'rgba(240,237,232,0.4)', fontFamily:'var(--font-mono, monospace)' }}>
          ...
          <div>
            ...
            <div style={{ marginTop:12 }}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:8, background:'var(--v2-surface)', border:'0.5px solid var(--v2-border)', borderRadius:6, padding:'8px 10px' }}>
                <code style={{ fontSize:10, color:'rgba(240,237,232,0.6)', fontFamily:'var(--font-mono, monospace)', flex:1, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                  {agent.agent_token ? `${agent.agent_token.substring(0,24)}…` : '—'}
                </code>
                <button onClick={copyToken} style={{ flexShrink:0, fontSize:10, fontWeight:500, color:'rgba(240,237,232,0.6)', padding:'4px 8px', border:'0.5px solid var(--v2-border)', borderRadius:4, background:'var(--v2-surface)', cursor:'pointer', display:'inline-flex', alignItems:'center', gap:4, fontFamily:'inherit' }}>
...
function InstallModal({ onClose }) {
  ...
  const CmdBlock = ({ cmd, id }) => (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', background:'var(--v2-text)', borderRadius:6, padding:'10px 14px', marginBottom:8 }}>
      <code style={{ fontSize:12, color:'#f0ede8', fontFamily:'var(--font-mono, monospace)', flex:1, overflow:'auto' }}>{cmd}</code>
      <button onClick={() => copy(cmd, id)} style={{ flexShrink:0, marginLeft:12, fontSize:10, fontWeight:500, color:'rgba(240,237,232,0.6)', padding:'4px 8px', border:'0.5px solid rgba(240,237,232,0.14)', borderRadius:4, background:'none', cursor:'pointer', display:'inline-flex', alignItems:'center', gap:4, fontFamily:'inherit' }}>
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(240,237,232,0.45)', zIndex:200, display:'flex', alignItems:'center', justifyContent:'center', padding:24 }}>
      <div style={{ background:'var(--v2-surface)', borderRadius:10, width:'100%', maxWidth:540, boxShadow:'0 20px 60px rgba(0,0,0,0.2)', overflow:'hidden' }}>
        <div style={{ padding:'18px 22px', borderBottom:'0.5px solid var(--v2-border)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          ...
          <button onClick={onClose} style={{ width:28, height:28, border:'0.5px solid var(--v2-border)', borderRadius:6, background:'var(--v2-surface)', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', color:'rgba(240,237,232,0.6)', fontSize:16 }}>×</button>
        </div>
        <div style={{ padding:'20px 22px' }}>
          <div style={{ fontSize:11, fontWeight:500, color:'rgba(240,237,232,0.6)', marginBottom:8 }}>1. Install the agent</div>
          <CmdBlock cmd={cmd1} id="install"/>
          <div style={{ fontSize:11, fontWeight:500, color:'rgba(240,237,232,0.6)', margin:'14px 0 8px' }}>2. Verify it's running</div>
  ...
  return (
    <div style={{ padding:'24px 28px 60px' }}>
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:24, gap:16 }}>
        <div>
          <div style={{ fontSize:14, color:'rgba(240,237,232,0.6)', marginBottom:2 }}>
            {loading ? '—' : `${agents.length} server${agents.length !== 1 ? 's' : ''} connected`}
            {!loading && online > 0 && <span style={{ marginLeft:8, fontSize:11, color:'#f0ede8', fontWeight:500 }}>· {online} online</span>}
          </div>
          <div style={{ fontSize:11, color:'var(--v2-text-3)' }}>Persistent agents auto-renew certificates every 5 minutes</div>
        </div>
        <div style={{ display:'flex', gap:8, flexShrink:0 }}>
          <button onClick={refresh} style={{ display:'inline-flex', alignItems:'center', gap:6, background:'var(--v2-surface)', color:'rgba(240,237,232,0.6)', border:'0.5px solid var(--v2-border)', padding:'7px 12px', borderRadius:6, fontSize:12, fontWeight:500, cursor:'pointer', fontFamily:'inherit' }}>
