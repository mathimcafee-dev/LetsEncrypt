...
const GLOBAL_TIMELINE = [
  ...
  { year:1999, org:'webtrust', event:'WebTrust for CAs launched by AICPA/CPA Canada', desc:'First formal CA audit programme launched. Gives browser vendors an independent assurance mechanism to evaluate CA trustworthiness before root store inclusion.', type:'milestone', color:'rgba(255,255,255,0.6)' },
  ...
  { year:2014, org:'eidas', event:'eIDAS Regulation — EU digital identity framework created', desc:'EU Regulation 910/2014 creates pan-European framework for electronic signatures, seals, and timestamps. Makes qualified electronic signatures legally binding across all EU member states.', type:'milestone', color:'rgba(255,255,255,0.7)' },
  ...
  { year:2024, org:'eidas', event:'eIDAS 2.0 Regulation 2024/1183 — EU Digital Identity Wallet', desc:'eIDAS 2.0 published. Introduces EU Digital Identity Wallet (EUDIW) and mandates browsers trust government-issued QWAC certificates. Browser governance battle begins.', type:'milestone', color:'rgba(255,255,255,0.7)' },
...
const statusDot = s => {
  ...
  if (['draft','selected','planning'].some(x=>l.includes(x))) return '#ffffff'
  return '#a3a3a3'
}
            ...
            <div className="detail-panel">
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:12 }}>
                <div style={{ display:'flex', gap:12, alignItems:'flex-start' }}>
                  <div className="org-badge" style={{ background: selectedOrg.logo_color||'#ffffff', width:44, height:44, fontSize:11 }}>{selectedOrg.acronym||selectedOrg.short_name?.slice(0,4)}</div>
                  <div>
              ...
              {filteredOrgs.map(org => (
                <div key={org.id} className={`org-card${selectedOrg?.id===org.id?' active':''}`} onClick={() => setSelectedOrg(selectedOrg?.id===org.id ? null : org)}>
                  <div className="org-header">
                    <div className="org-badge" style={{ background: org.logo_color||'#ffffff' }}>{org.acronym||org.short_name?.slice(0,4)}</div>
                ...
                return (
                  <div key={s.identifier} className={`std-row${selectedStd?.identifier===s.identifier?' active':''}`} onClick={() => setSelectedStd(selectedStd?.identifier===s.identifier ? null : s)}>
                    <div className="std-id-col">
                      ...
                      {org && <div style={{ marginTop:4 }}><div className="org-badge" style={{ background:org.logo_color||'#ffffff', width:20, height:20, fontSize:7, borderRadius:4, display:'inline-flex' }}>{org.acronym?.slice(0,4)||org.short_name?.slice(0,4)}</div></div>}
                    </div>
                    <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <div key={e.id} className="event-card">
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      ...
                      <div style={{ display:'flex', gap:6, flexWrap:'wrap', marginBottom:6 }}>
                        {org && <span className="org-badge" style={{ background:org.logo_color||'#ffffff', width:20, height:20, fontSize:7, borderRadius:4, display:'inline-flex', alignItems:'center', justifyContent:'center', flexShrink:0, color:'#fff', fontWeight:800 }}>{org.acronym?.slice(0,4)}</span>}
