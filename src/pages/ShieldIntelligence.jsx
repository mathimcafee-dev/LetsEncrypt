...
function gradeStyle(g) {
  ...
  if (g==='B')       return { color:'#0F6E56', bg:'#E1F5EE', border:'#A8E6DE' }
  if (g==='A')       return { color:'#0F6E56', bg:'#E1F5EE', border:'#A8E6DE' }
  if (g==='A+')      return { color:'#1A7A72', bg:'#E1F5EE', border:'#6ee7b7' }
...
function Tick({ ok, label }) {
  return (
    <div style={{ display:'flex', alignItems:'center', gap:4 }}>
      {ok ? <CheckCircle size={11} color="#1A7A72"/> : <XCircle size={11} color="#A32D2D"/>}
      <span style={{ fontSize:11, color:ok?'#1A7A72':'#A32D2D' }}>{label}</span>
    </div>
  )
}
...
function ScoreBar({ score }) {
  ...
  const color = pct>=80?'#1A7A72':pct>=60?'#0F6E56':pct>=50?'#854F0B':'#A32D2D'
  return (
...
function OverviewTab({ user }) {
  ...
  const gradeEntries = [['A+','#1A7A72'],['A','#0F6E56'],['B','#0F6E56'],
    ...
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8, marginBottom:16 }}>
        ...
        <StatCard label="Deployed to servers" val={deployed} sub="agent or cPanel"
          color={deployed>0?'#1A7A72':'var(--v2-text-1)'}/>
          ...
          <div style={{ background:'var(--v2-surface)', border:'0.5px solid var(--v2-border)',
            ...
            {Object.entries(bySource).length === 0 ? (
              ...
              <div key={src} style={{ display:'flex', alignItems:'center', justifyContent:'space-between',
                ...
                <span style={{ fontSize:13, fontWeight:700, color:'#1A7A72', fontFamily:'monospace' }}>{count}</span>
              </div>
            ))}
          </div>
          <div style={{ background:'var(--v2-surface)', border:'0.5px solid var(--v2-border)',
            ...
            <div style={{ display:'flex', gap:16 }}>
              <div>
                <div style={{ fontSize:20, fontWeight:700, color:'#1A7A72', fontFamily:'monospace' }}>{thisMonth}</div>
        ...
        <div style={{ background:'var(--v2-surface)', border:'0.5px solid var(--v2-border)',
          ...
          <div style={{ display:'flex', height:12, borderRadius:8, overflow:'hidden', gap:1 }}>
            {[
              ...
              { n:active-expiring30,     color:'#1A7A72', label:'Healthy' },
          ...
          <div style={{ display:'flex', gap:16, marginTop:8 }}>
            {[
              ...
              { label:'Healthy',        n:active-expiring30, color:'#1A7A72' },
            ].map(({ label, n, color }) => (
      ...
      {scores.length > 0 && (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8, marginBottom:16 }}>
          ...
          <StatCard label="Need attention" val={issues.length}
            color={issues.length>0?'#A32D2D':'#1A7A72'}
        ...
        <div style={{ display:'flex', gap:8, alignItems:'center' }}>
          ...
          <button onClick={addDomain} disabled={adding || !newDomain.trim()}
            style={{ display:'flex', alignItems:'center', gap:5, fontSize:11, padding:'5px 12px',
              borderRadius:7, border:'none', background:'var(--v2-green)', color:'white',
...
const CT_STATUS = {
  ...
  known:      { label:'Known',      color:'#1A7A72', bg:'#E1F5EE', border:'#A8E6DE' },
}
  ...
  return (
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8, marginBottom:16 }}>
        ...
        <StatCard label="Confirmed safe" val={known.length} color="#1A7A72"/>
      </div>
      ...
      <div style={{ background:'#E1F5EE', border:'0.5px solid #A8E6DE', borderRadius:10,
        ...
        <ShieldAlert size={16} color="#1A7A72" style={{ flexShrink:0, marginTop:1 }}/>
                      ...
                      <div style={{ display:'flex', gap:6 }}>
                        <button onClick={() => mark(s.id,'known')} disabled={marking===s.id}
                          style={{ fontSize:11, padding:'5px 12px', borderRadius:6,
                            border:'0.5px solid #A8E6DE', background:'#E1F5EE',
